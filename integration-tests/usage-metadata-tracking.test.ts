...
test.describe('Token usage tracking', () => {
  test('should track and display token usage with OpenAI provider', async ({ page }) => {
    ...
    await expect(page.locator('text=Switched from gemini to openai')).toBeVisible();
    ...
    const contextText = await page.locator('text=/\\d+% context left/').textContent();
  ...
  test('should accumulate token usage across multiple messages', async ({ page }) => {
    await page.goto('/');
    ...
    const firstContextText = await page.locator('text=/\\d+% context left/').textContent();
    ...
    const secondContextText = await page.locator('text=/\\d+% context left/').textContent();
...
test.describe('Usage metadata events', () => {
  test('should emit usage metadata events from provider', async ({ page }) => {
    ...
    const usageDataLog = consoleLogs.find(log => 
    ...
    const usageEventLog = consoleLogs.find(log =>
      log.includes('UsageMetadata') ||
      log.includes('usage_metadata')
    );
