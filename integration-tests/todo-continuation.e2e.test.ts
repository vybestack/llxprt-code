 ...
 */
it.skipIf(skipTodoTests)('basic todo continuation flow', async () => {
  ...
  const createResult = await rig.run({
    args: 'Create a todo list with these tasks: 1. Implement auth system (in_progress), 2. Write tests (pending)',
  });
  ...
  if (!writeToolCall) {
    printDebugInfo(rig, createResult, {
  ...
  const continuationResult = await rig.run({
    args: 'I need to think about this approach',
  });
...
it.skipIf(skipTodoTests)(
  ...
  async () => {
    ...
    // Create todo list
    await rig.run({
      args: 'Create a todo: Implement auth system (in_progress)',
    });
    ...
    const toolResult = await rig.run({
      args: 'Let me check what files exist in this directory',
    });
...
it.skipIf(skipTodoTests)(
  ...
  async () => {
    ...
    await rig.run({
      args: 'Create a todo list with one item: Build login form (in_progress)',
    });
    const writeToolCall = await rig.waitForToolCall('todo_write');
    assert.ok(writeToolCall, 'Expected to find a todo_write tool call');

    const result = await rig.run({
      args: 'List the files in the project root and then reply immediately with a summary.',
    });
 ...
 */
it.skipIf(skipTodoTests)('todo pause tool usage', async () => {
  const rig = new TestRig();
  ...
  // Create todo
  await rig.run({
    args: 'Create a todo: Implement database schema (in_progress)',
  });
  ...
  const pauseResult = await rig.run({
    args: 'The database configuration file is missing and I cannot proceed without it',
  });
 ...
 * @then NO continuation prompt appears
 */
it.skipIf(skipTodoTests)('setting toggle disables continuation', async () => {
  const rig = new TestRig();
  ...
  // Create todo
  await rig.run({ args: 'Create a todo: Write documentation (in_progress)' });
  ...
  const result = await rig.run({
    args: 'I need to think about this documentation approach',
  });
 ...
 */
it.skip('YOLO mode enhanced continuation prompts', async () => {
  ...
  await rig.run({
    args: 'Create a todo: Optimize performance (in_progress)',
  });
  ...
  const yoloResult = await rig.run({
    args: 'Let me analyze the current situation',
  });
...
it.skipIf(skipTodoTests)(
  ...
  async () => {
    ...
    await rig.run({
      args: 'Create todos: 1. Fix critical bug (in_progress), 2. Update docs (in_progress), 3. Code review (pending)',
    });
    ...
    const priorityResult = await rig.run({
      args: 'I need to step back and think about priorities',
    });
 ...
 * @then NO continuation prompt appears
 */
it.skipIf(skipTodoTests)('no continuation without active todos', async () => {
  const rig = new TestRig();
  ...
  await rig.run({
    args: 'Create todos: 1. Setup project (completed), 2. Plan architecture (pending)',
  });
  ...
  const result = await rig.run({
    args: 'I need to think about the next steps',
  });
 ...
 */
it.skipIf(skipTodoTests)('state validation and consistency', async () => {
  const rig = new TestRig();
  ...
  await rig.run({ args: 'Create a todo: Build user interface (in_progress)' });
  ...
  await rig.run({ args: 'Let me think about this' });

  // Update state - mark as completed and add new task
  await rig.run({
    args: 'Update todos: 1. Build user interface (completed), 2. Add authentication (in_progress)',
  });
  ...
  const secondContinuation = await rig.run({
    args: 'I need to pause and think',
  });
...
it.skipIf(skipTodoTests)(
  ...
  async () => {
    ...
    // Create todo
    await rig.run({
      args: 'Create a todo: Process data pipeline (in_progress)',
    });
    ...
    const quickMessage1 = await rig.run({ args: 'I need to analyze this' });
    const quickMessage2 = await rig.run({
      args: 'Let me consider the options',
    });
