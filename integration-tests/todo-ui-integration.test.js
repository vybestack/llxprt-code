...
test('todo ui integration in interactive mode', async () => {
  ...
  const hasStructuredUI = 
    (result.includes('- [→]') || result.includes('- [ ]') || result.includes('- [x]')) && 
...
test('todo ui integration in non-interactive mode', async () => {
  ...
  const hasSimplifiedMarkdown = 
    result.includes('## Todo List') && 
    (result.includes('- [→] ← current') || result.includes('- [ ]') || result.includes('- [x]'));
  ...
  assert.ok(hasSimplifiedMarkdown, 'Expected simplified Markdown in non-interactive mode');
});
