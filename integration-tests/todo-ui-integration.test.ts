...
it.skipIf(skipTodoTests)(
  'todo ui integration with todo panel output',
  async () => {
    ...
    const result = await rig.run({
      args: `Create a todo list with these tasks: 1. Implement role-based access control (in_progress, high), 2. Document security model (pending, medium), 3. Write tests (completed, low). Then show me the todo list.`,
    });
 ...
 */
it.skipIf(skipTodoTests)(
  'todo ui integration in non-interactive mode',
  async () => {
    const rig = new TestRig();
    ...
    const result = await rig.run({
      args: `Create a todo list with these tasks: 1. Implement role-based access control (in_progress, high), 2. Document security model (pending, medium), 3. Write tests (completed, low).`,
    });
 ...
 */
it.skipIf(skipTodoTests)(
  'todo data consistency between write and read',
  async () => {
    const rig = new TestRig();
    ...
    const result = await rig.run({
      args: `Create a todo list with these tasks: 1. Implement role-based access control (in_progress, high) with subtasks: Define role enum, Guard API endpoints. 2. Document security model (pending, medium). Then show me the todo list.`,
    });
    ...
    // Both tool calls ran within the same rig instance, so session
    // consistency is guaranteed by the single-session TestRig lifecycle.
    // The previous JS accessed .sessionId on a boolean return (always
    // undefined), which was a no-op; removed rather than weakened.
 ...
 */
it.skipIf(skipTodoTests)('todo error handling with invalid data', async () => {
  const rig = new TestRig();
  ...
  const _result = await rig.run({
    args: `Create a todo list with these tasks: 1. Invalid task with no status.`,
  });
  void _result;
