...
> **Windows note (Ctrl+Enter steering):** Windows consoles deliver `Ctrl+Enter` as a bare line feed, indistinguishable from `Ctrl+J`. So on Windows `Ctrl+Enter` and `Ctrl+J` are the same key: while the agent is streaming they **steer** the active turn (see "Steer the active agent…" in the Text Input table below) — or, on an empty input with queued messages, steer all queued messages into the active turn — and they insert a newline when the agent is idle, or when there is nothing to steer (empty input with no queued messages). On macOS and Linux `Ctrl+J` and `Ctrl+Enter` are distinct keys and behave exactly as listed. This platform alias is applied at runtime only; the binding table below is intentionally platform-neutral.
