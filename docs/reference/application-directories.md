...
3. **OAuth advisory locks** — short-lived, credential-free lock files under `<log>/oauth/locks/` that coordinate concurrent refresh and interactive login attempts. Versioned lock metadata records a random owner token, PID, hostname, and process-start identity. LLxprt reclaims a lock only when it can prove that its local owner process is dead; live owners, different-host owners, malformed metadata, and unverifiable process identities are never age-stolen. Recovery uses a fenced takeover so concurrent waiters cannot unlink a successor lock. If manual cleanup is unavoidable, stop every LLxprt process first.
