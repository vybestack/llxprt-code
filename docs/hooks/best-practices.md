# Hooks on LLxprt Code: Best practices
...
docker run --rm \
  -v "$LLXPRT_PROJECT_DIR:/workspace:ro" \
...
const CACHE_FILE = '.llxprt/hook-cache.json';
...
```bash
#!/usr/bin/env bash
LOG_FILE=".llxprt/hooks/debug.log"
...
# Test the hook
cat test-input.json | .llxprt/hooks/my-hook.sh
...
input=$(cat)
echo "$input" >> .llxprt/hook-inputs.log
echo "Logged input"
```
...
```bash
git add .llxprt/hooks/
git add .llxprt/settings.json
...
.llxprt/hook-cache.json
.llxprt/hook-debug.log
.llxprt/memory/session-*.jsonl
...
```bash
ls -la .llxprt/hooks/my-hook.sh
chmod +x .llxprt/hooks/my-hook.sh
```
...
echo "$LLXPRT_PROJECT_DIR/.llxprt/hooks/my-hook.sh"

# Verify file exists
test -f "$LLXPRT_PROJECT_DIR/.llxprt/hooks/my-hook.sh" && echo "File exists"
```
...
if [ -z "$LLXPRT_PROJECT_DIR" ]; then
  echo "LLXPRT_PROJECT_DIR not set" >&2
  exit 1
fi
...
env > .llxprt/hook-env.log

# Check specific variables
echo "LLXPRT_PROJECT_DIR: $LLXPRT_PROJECT_DIR" >> .llxprt/hook-env.log
echo "LLXPRT_SESSION_ID: $LLXPRT_SESSION_ID" >> .llxprt/hook-env.log
echo "LLXPRT_API_KEY: ${LLXPRT_API_KEY:+<set>}" >> .llxprt/hook-env.log
```
...
| **User** (your [settings.json](../reference/application-directories.md)) | Configured by you. You are responsible for ensuring they are safe.                                                          |
...
1. **Detection**: LLxprt Code detects the hooks.
...
> hook is changed (e.g., by a `git pull`), its identity changes. LLxprt Code will
...
Hooks inherit the environment of the LLxprt Code process, which may include
sensitive API keys. LLxprt Code attempts to sanitize sensitive variables, but you
should be cautious.
...
default in the system configuration (e.g., `/etc/llxprt-code/settings.json`):
...
Prevent denial-of-service (hanging agents) by enforcing timeouts. LLxprt Code
