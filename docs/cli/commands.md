...
- **`/bug`**
  - **Description:** File an issue about LLXprt CLI. By default, the issue is filed within the GitHub repository for LLXprt CLI. The string you enter after `/bug` will become the headline for the bug being filed. The default `/bug` behavior can be modified using the `bugCommand` setting in your `.llxprt/settings.json` files.
  ...
  - **Sub-commands:**
    - **`save`**
      ...
      - **Details on Checkpoint Location:** The default locations for saved chat checkpoints are:
        - Linux/macOS: `~/.llxprt/tmp/<project_hash>/`
        - Windows: `C:\Users\<YourUsername>\.llxprt\tmp\<project_hash>`
        - When you run `/chat list`, the CLI only scans these specific directories to find available checkpoints.
        - **Note:** These checkpoints are for manually saving and resuming conversation states. For automatic checkpoints created before file modifications, see the [Continuation and Checkpointing documentation](../checkpointing.md).
    - **`resume`**
...
- **`/help`** (or **`/?`**)
  - **Description:** Display help information about LLxprt Code, including available commands and their usage.
...
- **`/restore`**
  ...
  - **Note:** Only available if the CLI is invoked with the `--checkpointing` option or configured via [settings](./configuration.md). See [Continuation and Checkpointing documentation](../checkpointing.md) for more details.

- **`/settings`**
  - **Description:** Open the settings editor to view and modify LLxprt CLI settings.
  - **Details:** This command provides a user-friendly interface for changing settings that control the behavior and appearance of LLXprt CLI. It is equivalent to manually editing the `.llxprt/settings.json` file, but with validation and guidance to prevent errors.
...
- If the path specified after `@` is not found or is invalid, an error message will be displayed, and the query might not be sent to the active model, or it will be sent without the file content.
...
- **Environment Variable:** When a command is executed via `!` or in shell mode, the `LLXPRT_CLI=1` environment variable is set in the subprocess's environment. This allows scripts or tools to detect if they are being run from within the LLxprt Code.
