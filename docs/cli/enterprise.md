# LLxprt Code for the Enterprise

This document outlines configuration patterns and best practices for deploying
and managing LLxprt Code in an enterprise environment. By leveraging system-level
...
> using LLxprt Code. However, they should not be considered a foolproof security
...
- **Workspace `settings.json` (`<project>/.llxprt/settings.json`):**
...
This approach ensures that no matter how the user calls the `llxprt` command,
the enterprise settings are always loaded with the highest precedence.

**Example wrapper script:**

Administrators can create a script named `llxprt` and place it in a directory
that appears earlier in the user's `PATH` than the actual LLxprt Code binary
(e.g., `/usr/local/bin/llxprt`).
...
REAL_LLXPRT_PATH=$(type -aP llxprt | grep -v "^$(type -P llxprt)$" | head -n 1)

if [ -z "$REAL_LLXPRT_PATH" ]; then
  echo "Error: The original 'llxprt' executable was not found." >&2
  exit 1
fi

# Pass all arguments to the real LLxprt Code executable.
...
You can significantly enhance security by controlling which tools the LLxprt
...
LLxprt Code loads `settings.json` files from three levels: System, Workspace, and
...
In corporate environments with strict network policies, you can configure LLxprt Code
...
control that is configured on a proxy server, not within LLxprt Code itself. It
...
patterns discussed above to create a secure, controlled environment for LLxprt Code
CLI.
...
```json
{
  ...
  "mcpServers": {
    "corp-tools": {
      "command": "/opt/llxprt-tools/start.sh",
      "timeout": 5000
    }
  },
  "telemetry": {
