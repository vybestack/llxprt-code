...
| Dimension                         | Upstream (`route_severity_below`, `route_categories`)                                                                                                                                                                                                                                                                                                                                                                                                                                   | Ours (routing + shadow mode + inline cap + coverage threshold)                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                             |
| --------------------------------- | --------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------- | -------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------- |
| Configuration surface             | Two string inputs, both optional and empty by default. `route_severity_below` is one of `critical`, `high`, `medium`, `low` and routes at-or-below that severity. `route_categories` is a case-insensitive comma list drawn from the eight documented categories (`bug`, `security`, `performance`, `maintainability`, `test`, `style`, `documentation`, `other`). An empty input disables that axis.                                                                                   | Three repository variables, each with a separate role. `OCR_ROUTING_SHADOW_MODE` controls whether routing is applied. `OCR_INLINE_COMMENT_CAP` (default 50) applies after deduplication and moves overflow to the sticky summary. `OCR_COVERAGE_THRESHOLD` (default 90) controls the incomplete-review warning, not finding placement. The routing policy stays fixed: `bug`/`security`/`correctness`, `high`/`medium` severity, and unknown category or severity stay inline; `info` and `low` severity in `maintainability`/`test`/`style`/`other` route to the summary. |
| Invalid input                     | An unknown severity disables severity routing. Unknown category tokens are ignored while valid tokens in the same list remain active. `buildPolicy` returns `NO_ROUTING` only when neither axis has a valid value. The two axes are independent and OR-combined: missing or unknown metadata on a finding cannot match its own axis, but another active axis with known matching metadata can still route it. A line-addressable finding stays inline only when no active axis matches. | Only the literal `false` turns shadow mode off; unset or any other value leaves it on. The inline cap accepts positive integers and otherwise falls back to 50. The coverage threshold accepts finite values from 0 through 100 and otherwise falls back to 90. Unknown or absent finding metadata takes an explicit fail-safe inline branch.                                                                                                                                                                                                                              |
| Non-destructive                   | Routed findings are pushed to `commentsRouted` and rendered in the summary with the policy reason; nothing is deleted.                                                                                                                                                                                                                                                                                                                                                                  | Policy-routed and cap-overflow findings go to the sticky summary with the original finding object preserved; nothing is deleted.                                                                                                                                                                                                                                                                                                                                                                                                                                           |
| Inline publication errors         | `publishBatch` catches a failed batch `createReview`, reconciles it, retries per comment, counts failures in `comments_failed`, and appends failed findings to the summary. The run continues through this path.                                                                                                                                                                                                                                                                        | A failed batch warns and falls back to per-comment posting. Ambiguous writes are recorded and reconciled. Individual failures are retained for the sticky summary, and the workflow continues.                                                                                                                                                                                                                                                                                                                                                                             |
| Sticky-summary publication errors | Summary reads fail soft, writes fail hard. `ensureSummaryAnchor` logs and returns null when its existence-check read fails; finalization then gets another opportunity. An attempted anchor create write rejects before inline publication. `finalizeSummary` logs and returns null when its existence-check read fails and there is no anchor. An attempted final update or create write rejects after inline comments may already have landed. Neither case deletes findings.         | `createOrUpdateMarkerComment` failures are caught. The workflow warns, leaves publication ambiguous, continues without failing, and retains the raw review and routing-decision artifacts.                                                                                                                                                                                                                                                                                                                                                                                 |
| Rollout safety                    | A configured policy takes effect immediately. There is no dry-run mode.                                                                                                                                                                                                                                                                                                                                                                                                                 | Shadow mode is on unless `OCR_ROUTING_SHADOW_MODE=false`. Routing decisions are computed and recorded, but they are not applied while shadow mode is on.                                                                                                                                                                                                                                                                                                                                                                                                                   |
| Observability                     | Routing observability is the aggregate `comments_routed` action output. There is no finding-level routing-decision output or artifact.                                                                                                                                                                                                                                                                                                                                                  | Every pre-dedup routing decision (finding, destination, reason) is persisted to `ocr-routing-decisions.json` and uploaded as an artifact. The workflow also exposes `comments_routed_summary` and renders a shadow-mode preview in the sticky summary.                                                                                                                                                                                                                                                                                                                     |

**Decision: DEFER, keep ours.** Upstream's controls provide a configurable routing
policy, and its routing path is fail-open and non-destructive. Our fixed policy
provides the behavior this repository currently needs, including default-on shadow
mode, a finding-level audit artifact, and broader handling of sticky-summary write
failures. The policies are not interchangeable: ours can route only `info` and `low`
findings in `maintainability`/`test`/`style`/`other`, while upstream callers can choose
the threshold and categories.

Upstream's publication controls live only in `action.yml` and
`scripts/github-actions/post-review-comments.js`; the npm version bump does not import
them. Consuming the composite action would replace our checkout, install, review, and
publication sequencing. Its surrounding checkpoint, suspension, manifest, coverage,
routing/shadow, and trusted-marker behavior would have to be rebuilt around the
action.

This issue adopts no workflow behavior. None of our routing, cap, coverage, telemetry,
or artifact controls change, so the conditional acceptance criterion requiring new
behavioral tests does not apply.
