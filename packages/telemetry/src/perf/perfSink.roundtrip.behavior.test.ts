...
import type { PerfOperationRecord, PerfRecord } from './perfRecords.js';
...
describe('PerfSink round-trip behavior', () => {
  beforeEach(() => {
  ...
  describe('PerfSink round-trip (AC-1)', () => {
    it('writes a record that round-trips through the real reader with exact field values', async () => {
      ...
      const { records, counts } = await readPerfRecords(
      ...
      expect(result.record_type).toBe('operation');
      const parsed = requireOperationRecord(result);
