...
describe('Token usage reader behavior', () => {
  beforeEach(() => {
  ...
  function requireTurnEntry(entry: TokenUsageStreamEntry | undefined) {
    if (entry?.kind !== 'turn') {
      throw new Error('expected a token usage turn');
    }
    return entry;
  }
  ...
  async function summarizeTurns(
    filePath: string,
  ): Promise<{ readonly count: number; readonly firstId: string }> {
    let count = 0;
    let firstId = '';
    for await (const entry of streamTokenUsageRecords(filePath)) {
      if (entry.kind !== 'turn') continue;
      count += 1;
      if (count === 1) {
        firstId = entry.row.promptId;
      }
    }
    return { count, firstId };
  }

  async function collectTurnSources(dir: string): Promise<string[]> {
    const sources: string[] = [];
    for await (const entry of streamTokenUsageDirectory(dir)) {
      if (entry.entry.kind === 'turn') {
        sources.push(entry.sourceFile);
      }
    }
    return sources;
  }
  ...
  describe('streamTokenUsageRecords — real file classification', () => {
    it('yields one turn entry per accepted turn row', async () => {
      ...
      expect(entries.map((e) => e.kind)).toStrictEqual(['turn', 'turn']);
      const first = requireTurnEntry(entries[0]);
    ...
    it('classifies a final truncated line as truncated', async () => {
      ...
      expect(entries.map((e) => e.kind)).toStrictEqual(['turn', 'truncated']);
    });
    ...
    it('treats a non-object JSON value as malformed', async () => {
      ...
      expect(entries.slice(1).map((e) => e.kind)).toStrictEqual([
        'malformed',
        'malformed',
        'malformed',
        'malformed',
    ...
    it('structurally accepts a future-version turn row with the right fields', async () => {
      ...
      expect(entries.map((e) => e.kind)).toStrictEqual(['turn']);
      const first = requireTurnEntry(entries[0]);
  ...
  describe('consumeTokenUsageDirectory — multi-file sorted reading + self-health', () => {
    it('reads multiple sorted files and accumulates rows + counts', async () => {
      ...
      expect(rows.map((r) => r.promptId)).toStrictEqual(['aaa', 'bbb']);
    });
    ...
    it('visits files in sorted name order for deterministic reading', async () => {
      ...
      expect(rows.map((r) => r.promptId)).toStrictEqual(['a', 'm', 'z']);
    });
  ...
  describe('streamTokenUsageFromReadable — incremental yield proof', () => {
    ...
    it('processes a large file incrementally without accumulating it all first', async () => {
      ...
      const summary = await summarizeTurns(path.join(dir, 'big.jsonl'));
      expect(summary.count).toBe(N);
      expect(summary.firstId).toBe('p-0');
    });
  ...
  describe('streamTokenUsageDirectory — per-file source attribution', () => {
    it('annotates each entry with its source file name', async () => {
      ...
      const sources = await collectTurnSources(dir);
      expect(sources).toStrictEqual(['a.jsonl', 'b.jsonl']);
    });
  });
});
