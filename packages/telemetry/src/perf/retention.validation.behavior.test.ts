...
describe('PerfRetention constructor tuning validation (D-LC-5)', () => {
  ...
  it('rejects a runUuid with path separators (fail-fast internally)', () => {
    ...
    expect(() => new PerfRetention({ dir, runUuid: `a${backslash}b` })).toThrow(
      TypeError,
    );
    expect(() => new PerfRetention({ dir, runUuid: `a${backspace}b` })).toThrow(
      TypeError,
    );
    expect(() => new PerfRetention({ dir, runUuid: `a${nullByte}b` })).toThrow(
      TypeError,
    );
