...
describe('PerfRetention claim behavior', () => {
  beforeEach(() => {
  ...
  function requireSchedulerCallback(
    callback: (() => Promise<void>) | null,
  ): () => Promise<void> {
    if (callback === null) {
      throw new Error('expected the retention scheduler callback');
    }
    return callback;
  }

  function isPerfJsonlFile(fileName: string): boolean {
    return fileName.startsWith('perf-') && fileName.endsWith('.jsonl');
  }
  ...
  describe('PerfRetention claim lifecycle (AC-7, D3)', () => {
    ...
    it('start() creates only the claim — no perf JSONL', async () => {
      ...
      expect(files).toStrictEqual([
        '00000000-0000-4000-8000-000000000002.claim',
      ]);
      await retention.dispose();
    });
    ...
    it('dispose() stops the interval (no further ticks)', async () => {
      ...
      await requireSchedulerCallback(scheduler.callback)();
      expect(listFiles()).not.toContain(
        '00000000-0000-4000-8000-000000000005.claim',
      );
    });
  ...
  describe('PerfRetention — claims never parsed as JSONL', () => {
    it('a .claim file is not picked up by readPerfRecords', async () => {
      ...
      const perfFiles = listFiles().filter(isPerfJsonlFile);
      expect(perfFiles).toStrictEqual([
        'perf-20260808-00000000-0000-4000-8000-00000000001c.jsonl',
      ]);
