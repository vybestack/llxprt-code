...
describe('Qwen3FireworksProvider', () => {
  ...
  it('should clean Qwen3 control tokens from content', async () => {
    ...
    const cleanMethod = (provider as unknown as { cleanQwen3Content: (content: string) => string }).cleanQwen3Content.bind(provider);
  ...
  it('should fix missing spaces between words', () => {
    ...
    const input = 'findaProviderContext.tsfile,whichprobably indicatesa different namingconventionforprovider-relatedcode';
  ...
  it('should keep think tags visible', () => {
    ...
    const input = '✦ <think>Okay, I couldn\'t find a file</think>Here is the actual response';
    const cleaned = cleanMethod(input);
    
    // Think tags should be kept visible
    expect(cleaned).toBe('✦ <think>Okay, I couldn\'t find a file</think>Here is the actual response');
  });
  ...
  it('should remove duplicate tool calls', () => {
    ...
    const input = '<tool_call>{"name": "test"}</tool_call> <tool_call>{"name": "test"}</tool_call>';
    const cleaned = cleanMethod(input);
  ...
  it('should set model correctly', () => {
    ...
    expect(provider.getCurrentModel()).toBe('accounts/fireworks/models/qwen3-235b-a22b');
    
    // Should accept a different Qwen3 model
    provider.setModel('accounts/fireworks/models/qwen3-70b');
    expect(provider.getCurrentModel()).toBe('accounts/fireworks/models/qwen3-70b');
  });
