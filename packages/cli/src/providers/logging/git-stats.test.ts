...
describe('Git Statistics Tracking', () => {
  ...
  describe('Statistics Calculation', () => {
    ...
    it('should handle empty content correctly', async () => {
      ...
      // Empty to content
      const stats1 = await tracker.trackFileEdit('file1.ts', '', 'line1\nline2');
      expect(stats1?.linesAdded).toBe(2);
      expect(stats1?.linesRemoved).toBe(0);
      
      // Content to empty
      const stats2 = await tracker.trackFileEdit('file2.ts', 'line1\nline2', '');
  ...
  describe('Error Handling', () => {
    ...
    it('should handle invalid file paths', async () => {
      ...
      const invalidPaths = ['', null, undefined, '/path/with/../traversal', 'path with spaces'];
  ...
  describe('Performance Characteristics', () => {
    it('should have minimal overhead when disabled', async () => {
      ...
      for (let i = 0; i < 100; i++) {
        await tracker.trackFileEdit(`file${i}.ts`, 'old content', 'new content');
      }
    ...
    it('should scale reasonably with file size', async () => {
      ...
      const largeContent = Array(1000).fill('line').map((line, i) => `${line}${i}`).join('\n');
      const modifiedContent = largeContent + '\nextra line';
      
      const startTime = process.hrtime.bigint();
      const stats = await tracker.trackFileEdit('large-file.ts', largeContent, modifiedContent);
      const endTime = process.hrtime.bigint();
