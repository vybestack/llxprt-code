...
describe('saveTrustedFolders filesystem permissions', () => {
  ...
  describe.skipIf(process.platform === 'win32')(
    ...
    () => {
      it('repairs an existing permissive file to mode 0600', () => {
      ...
      it('persists and resolves a workspace symlink under its canonical identity', () => {
        const directory = fs.mkdtempSync(
          path.join(os.tmpdir(), 'llxprt-trusted-folders-'),
        );
        temporaryDirectories.push(directory);
      ...
      it('does not inherit TRUST_PARENT through a symlink that escapes the canonical parent', () => {
        const directory = fs.mkdtempSync(
          path.join(os.tmpdir(), 'llxprt-trusted-folders-'),
        );
        temporaryDirectories.push(directory);
      ...
      it('replaces duplicate textual aliases and their denial with the explicit selection', () => {
        const directory = fs.mkdtempSync(
          path.join(os.tmpdir(), 'llxprt-trusted-folders-'),
        );
        temporaryDirectories.push(directory);
      ...
      it('snapshots and restores every exact textual alias after an explicit update', () => {
        const directory = fs.mkdtempSync(
          path.join(os.tmpdir(), 'llxprt-trusted-folders-'),
        );
        temporaryDirectories.push(directory);
      ...
      it('deletes every textual alias for the same canonical identity', () => {
        const directory = fs.mkdtempSync(
          path.join(os.tmpdir(), 'llxprt-trusted-folders-'),
        );
        temporaryDirectories.push(directory);
...
describe('LoadedTrustedFolders rule removal via a symlinked spelling', () => {
  ...
  describe.skipIf(process.platform === 'win32')(
    'on non-Windows systems',
    () => {
      it('removes the canonical rule when asked with a symlinked path spelling', () => {
