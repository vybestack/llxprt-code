...
describe('interactive perf runtime test lifecycle', () => {
  beforeEach(() => {
  ...
  describe('interactivePerfRuntime — live snapshot (AC-12)', () => {
    ...
    it('snapshot capability returns null active operation when idle', async () => {
      const runtime = createInteractivePerfRuntime(makeOptions());
      await runtime!.start();
      expect(runtime!.snapshotCapability.getActiveOperationSummary()).toBe(
        null,
      );
      await runtime!.dispose();
    });
    ...
    it('snapshot capability returns memory samples when memory enabled', async () => {
      ...
      expect(runtime!.snapshotCapability.getMemorySnapshot()).toStrictEqual([]);
      await runtime!.dispose();
    });
  });
  ...
  describe('resolveRuntimeVersion — canonical format (no leading v)', () => {
    const observeRuntimeVersionFormat = (): {
      readonly hasRuntimePrefix: boolean;
      readonly versionSegment: string;
      readonly hasLeadingV: boolean;
    } => {
      const value = resolveRuntimeVersion();
      const versionSegment = value.split('-').slice(1).join('-');
      return {
        hasRuntimePrefix: value.startsWith('bun-') || value.startsWith('node-'),
        versionSegment,
        hasLeadingV: versionSegment.startsWith('v'),
      };
    };

    it('produces a runtime string without a leading v on the version segment', () => {
      const runtimeVersion = observeRuntimeVersionFormat();
      expect(runtimeVersion.hasRuntimePrefix).toBe(true);
      expect(runtimeVersion.versionSegment.length).toBeGreaterThan(0);
      expect(runtimeVersion.hasLeadingV).toBe(false);
    });
  });
});
