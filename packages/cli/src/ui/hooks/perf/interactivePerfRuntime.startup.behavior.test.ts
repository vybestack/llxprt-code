...
describe('InteractivePerfRuntime.start — startup rollback (Item 2)', () => {
  it('rolls back and rejects on sink.start failure (non-errno internal error)', async () => {
    ...
    const messages =
      err instanceof AggregateError
        ? (err.errors as unknown[]).map((e) => (e as Error).message)
        : [err.message];
  ...
  it('rolls back and rejects on observer-install conflict (owner B)', async () => {
    ...
    const bErr = bStartError as Error;
    const bMessages =
      bErr instanceof AggregateError
        ? (bErr.errors as unknown[]).map((e) => (e as Error).message)
        : [bErr.message];
    expect(bMessages.some((m) => m.includes('observer is already'))).toBe(true);
    ...
    expect(claimsAfterB.length).toBe(1);
