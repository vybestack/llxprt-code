...
describe('dispatchStreamEvent - stream buffer lifecycle', () => {
  it.each([
    ...
    expect(result.agentMessageBuffer).toBe('');
  });
  ...
  it('does not prepend content from a finished turn to the next turn', () => {
    ...
    const next = dispatchStreamEvent(
      ...
      deps,
      finished.agentMessageBuffer,
      1001,
    );

    expect(next.agentMessageBuffer).toBe('second');
  });

  it('flushes pending AI content before clearing buffer on Finished', () => {
