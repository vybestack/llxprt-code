 ...
 * `getContentPrefixIdentity` dep into the AI history item's `profileName`
 * field, which is display-only (consumed by AiMessage.tsx's [profileName]
 * prefix rendering).
 */
...
describe('processContentEvent content-prefix identity (issue #2263)', () => {
  it('threads profileName:modelName into the pending AI item', () => {
    ...
    // ensureAiPendingItem creates the new pending item via the first call
