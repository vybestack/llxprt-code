...
describe('validation', () => {
  ...
  describe('validateBaseUrl', () => {
    it('rejects empty and whitespace-only URLs with the required message', () => {
      for (const url of ['', '   ']) {
        expect(validateBaseUrl(url)).toStrictEqual({
          valid: false,
    ...
    it('rejects URLs whose protocol is not http or https', () => {
      expect(validateBaseUrl('ftp://host')).toStrictEqual({
        valid: false,
    ...
    it('rejects strings that are not valid URLs', () => {
      expect(validateBaseUrl('not a url')).toStrictEqual({
        valid: false,
    ...
    it('accepts http and https URLs', () => {
      expect(validateBaseUrl('http://localhost:1234')).toStrictEqual({
        valid: true,
      });
      expect(validateBaseUrl('https://api.example.com/v1')).toStrictEqual({
        valid: true,
      });
    });
  });
  ...
  describe('validateProfileName', () => {
    it('rejects empty and whitespace-only names', async () => {
      for (const name of ['', '   ']) {
        expect(await validateProfileName(name, [])).toStrictEqual({
          valid: false,
    ...
    it('rejects a name that already exists', async () => {
      expect(await validateProfileName('dupe', ['dupe'])).toStrictEqual({
        valid: false,
    ...
    it('accepts a fresh name without mutating the existing list', async () => {
      ...
      await expect(
        validateProfileName('gamma', existing),
      ).resolves.toStrictEqual({
        valid: true,
      });
      expect(existing).toStrictEqual(snapshot);
    });
  });
  ...
  describe('validateKeyFile', () => {
    ...
    it('accepts an existing readable key file', async () => {
      ...
      await expect(validateKeyFile(filePath)).resolves.toStrictEqual({
        valid: true,
      });
    });
    ...
    it('expands a tilde-prefixed path against the home directory', async () => {
      ...
      await expect(validateKeyFile('~/tilde-key.txt')).resolves.toStrictEqual({
        valid: true,
      });
    });
    ...
    it('reports not-found for a tilde path whose target is absent from the home directory', async () => {
      ...
      await expect(validateKeyFile('~/absent-key.txt')).resolves.toStrictEqual({
        valid: false,
    ...
    describe('temperature', () => {
      it('rejects values outside the 0.0 to 2.0 range', () => {
        for (const value of [-0.1, 2.1]) {
          expect(PARAM_VALIDATORS.temperature(value)).toStrictEqual({
            valid: false,
    ...
    describe('maxTokens', () => {
      it('rejects non-positive or non-integer token counts', () => {
        for (const value of [0, -1, 1.5]) {
          expect(PARAM_VALIDATORS.maxTokens(value)).toStrictEqual({
            valid: false,
      ...
      it('rejects token counts above the maximum', () => {
        expect(PARAM_VALIDATORS.maxTokens(1_000_001)).toStrictEqual({
          valid: false,
      ...
      it('accepts token counts at the boundaries', () => {
        expect(PARAM_VALIDATORS.maxTokens(1)).toStrictEqual({ valid: true });
        expect(PARAM_VALIDATORS.maxTokens(1_000_000)).toStrictEqual({
          valid: true,
        });
      });
    });
    ...
    describe('contextLimit', () => {
      it('rejects non-positive or non-integer context limits', () => {
        for (const value of [0, -1, 2.5]) {
          expect(PARAM_VALIDATORS.contextLimit(value)).toStrictEqual({
            valid: false,
            error: 'Must be a positive integer',
          });
        }
      ...
      it('accepts a positive integer context limit', () => {
        expect(PARAM_VALIDATORS.contextLimit(1)).toStrictEqual({ valid: true });
      });
    });
  });
});
