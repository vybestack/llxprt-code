...
const formatTokenCount = (tokens?: { input?: number; output?: number }): string => {
  if (!tokens) return '';
  const parts: string[] = [];
...
export const LoggingDialog: React.FC<LoggingDialogProps> = ({
  ...
  useInput((input, key) => {
    ...
    if (key.downArrow || input === 'j') {
      setSelectedIndex(prev => Math.min(reversedEntries.length - 1, prev + 1));
    }
    
    if (key.pageUp) {
      setSelectedIndex(prev => Math.max(0, prev - Math.floor(contentHeight / 2)));
    }
    
    if (key.pageDown) {
      setSelectedIndex(prev => 
        Math.min(reversedEntries.length - 1, prev + Math.floor(contentHeight / 2))
      );
    }
    ...
    return (
      ...
      >
        ...
        {mainContent && (
          <Box paddingLeft={2}>
            <Text 
              color={entry.error ? SemanticColors.status.error : SemanticColors.text.secondary}
              wrap="wrap"
            >
              {mainContent}
            </Text>
  ...
  const scrollPercentage = hasScroll
    ? Math.round(((scrollOffset + contentHeight) / reversedEntries.length) * 100)
    : 100;
  
  return (
    <Box flexDirection="column" width={dialogWidth} borderStyle="round" borderColor={SemanticColors.border.default}>
      {/* Header */}
      <Box 
        paddingX={1} 
        paddingY={0}
