...
describe('/key subcommands', () => {
  ...
  describe('/key — Table-Driven Parsing (R27.2)', () => {
     ...
     */
    // Subcommand stubs return 'not yet implemented' during P14; after P15 they
    // return real results. Either way the input dispatches to a subcommand
    // handler and yields a defined message content.
    const subcommandCases = [
      { input: 'save mykey sk-abc', description: 'dispatches to save handler' },
      { input: 'load mykey', description: 'dispatches to load handler' },
      { input: 'show mykey', description: 'dispatches to show handler' },
      { input: 'list', description: 'dispatches to list handler' },
      { input: 'delete mykey', description: 'dispatches to delete handler' },
      {
    ...
    // Non-subcommand tokens fall through to the legacy path which calls
    // runtime.updateActiveProviderApiKey.
    const legacyKeyCases = [
      {
        input: 'sk-abc123',
    ...
    for (const tc of subcommandCases) {
    ...
    for (const tc of legacyKeyCases) {
      it(`${tc.description}: "${tc.input}"`, async () => {
        const result = await runKey(tc.input);
        expect(result.type).toBe('message');
    ...
    it('no args shows status via legacy path (R12.4)', async () => {
  ...
  describe('/key — Secure Input Masking (R20)', () => {
    ...
    it('sanitizes /key save for history with value masked', () => {
      const handler = new SecureInputHandler();
      const sanitized = handler.sanitizeForHistory(
        '/key save mykey sk-secret99',
      );
