...
import {
  ...
  buildLockUnlockSchemaEntries,
} from './authLockHelpers.js';
import {
  handleDiscoverBrowserProfiles,
  handleManageBrowserProfile,
} from './authBrowserProfileHelpers.js';
import {
...
export class AuthCommandExecutor {
  ...
  async execute(
    ...
    if (action === 'lock') {
      return handleLockCommandFn(this.oauthManager, provider, parts.slice(2));
    }

    if (action === 'unlock') {
      return handleUnlockCommandFn(this.oauthManager, provider, parts.slice(2));
    }

    return {
      type: 'message',
      messageType: 'error',
      content: `Invalid action: ${action}. Use create, disable, enable, lock, login, logout, profile, status, switch, or unlock`,
    };
  }
      ...
      if (bucket) {
        try {
          ...
          return {
            type: 'message',
            messageType: 'error',
            content: `Authenticated ${provider}, but failed to activate bucket ${bucket}: ${errorMessage}. Run /auth ${provider} switch ${bucket}.`,
          };
        }
      }
