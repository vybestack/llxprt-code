  ...
  describe('media block sanitization (issue #1889)', () => {
    it('does not send raw media bytes to the compression provider for compressed messages with images', async () => {
      const {
        ...
        await observeDoesNotSendRawMediaBytesToTheCompressionProviderForCompressed();
      expect(
    ...
    const observeDoesNotSendRawMediaBytesToTheCompressionProviderForCompressed =
      async () => {
    ...
    it('preserves media blocks in the kept tail that are not sent to the LLM', async () => {
      const { tailMediaBlocks, mediaInRequest } =
        await observePreservesMediaBlocksInTheKeptTailThatAreNotSentTo();
    ...
    const observePreservesMediaBlocksInTheKeptTailThatAreNotSentTo =
      async () => {
        ...
        return { tailMediaBlocks, mediaInRequest };
      };
  });
});
