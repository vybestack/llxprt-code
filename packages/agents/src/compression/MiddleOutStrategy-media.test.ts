  ...
  describe('media block sanitization (issue #1889)', () => {
    it('does not send raw media bytes to the compression provider for middle messages with images', async () => {
      const {
        ...
        await observeDoesNotSendRawMediaBytesToTheCompressionProviderForMiddle();
      expect(
        providerTexts.some((text) => text.includes('not-valid-base64===')),
      ).toBe(false);
      expect(providerTexts.some((text) => text.includes('JVBERi0xLjQ='))).toBe(
        false,
      );
    ...
    const observeDoesNotSendRawMediaBytesToTheCompressionProviderForMiddle =
      async () => {
        ...
        const providerTexts = textBlocks.map((block) => block.text);
        ...
        const hasPdfPlaceholder = placeholderTexts.some(
          (t) =>
            t.includes('[Attached PDF: report.pdf]') ||
    ...
    it('preserves media blocks in top and bottom sections that are not sent to the LLM', async () => {
      const { topMediaBlocks, bottomMediaBlocks, mediaInRequest } =
        await observePreservesMediaBlocksInTopAndBottomSectionsThatAreNotSent();
    ...
    const observePreservesMediaBlocksInTopAndBottomSectionsThatAreNotSent =
      async () => {
        const history: IContent[] = [];
        for (let i = 0; i < 20; i++) {
          if (i === 0) {
        ...
        return { topMediaBlocks, bottomMediaBlocks, mediaInRequest };
      };
  });
  ...
  describe('largeLastPromptInjection media sanitization (issue #1889)', () => {
    it('uses text placeholders for media blocks in the largeLastPromptInjection (no raw media in provider request)', async () => {
      const {
        ...
        await observeUsesTextPlaceholdersForMediaBlocksInTheLargeLastPromptInjectionNoRawMedia();
      expect(
        providerTexts.some((text) => text.includes('malformed-base64===')),
      ).toBe(false);
      expect(result.metadata.llmCallMade).toBe(true);
      expect(mediaBlocks).toHaveLength(0);
    ...
    const observeUsesTextPlaceholdersForMediaBlocksInTheLargeLastPromptInjectionNoRawMedia =
      async () => {
    ...
    it('uses placeholders when the large last human prompt in compress range includes media', async () => {
      const { result, mediaBlocks, hasDiagramPlaceholder, providerTexts } =
        await observeUsesPlaceholdersWhenTheLargeLastHumanPromptInCompressRangeIncludes();
      expect(providerTexts.some((text) => text.includes('AAAA='))).toBe(false);
      expect(result.metadata.llmCallMade).toBe(true);
      expect(mediaBlocks).toHaveLength(0);
      expect(hasDiagramPlaceholder).toBe(true);
    });

    const observeUsesPlaceholdersWhenTheLargeLastHumanPromptInCompressRangeIncludes =
      async () => {
        ...
        return { result, mediaBlocks, hasDiagramPlaceholder, providerTexts };
      };

    it('no media blocks reach the provider when short prompt with media is moved to bottom', async () => {
      const { result, mediaInRequest, ackText, providerTexts } =
        await observeNoMediaBlocksReachTheProviderWhenShortPromptWithMediaIs();
      expect(
        providerTexts.some((text) => text.includes('base64imagedata==')),
      ).toBe(false);
      expect(result.metadata.llmCallMade).toBe(true);
      expect(mediaInRequest).toHaveLength(0);
    ...
    const observeNoMediaBlocksReachTheProviderWhenShortPromptWithMediaIs =
      async () => {
        const history: IContent[] = [];
        for (let i = 0; i < 20; i++) {
          if (i === 9) {
        ...
        return { result, mediaInRequest, ackText, providerTexts };
      };

    it('lastUserPromptContext string contains placeholder text, not raw media data, for messages with media', async () => {
      const { ackText } =
        await observeLastUserPromptContextStringContainsPlaceholderTextNotRawMediaDataForMessagesWith();
      expect(ackText).not.toContain('rawchartdata=');
      expect(ackText).toContain('[Attached image: Revenue chart Q4]');
    });

    const observeLastUserPromptContextStringContainsPlaceholderTextNotRawMediaDataForMessagesWith =
      async () => {
        const history: IContent[] = [];
        for (let i = 0; i < 20; i++) {
          if (i === 19) {
