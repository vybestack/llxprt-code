...
describe('agent.tasks undefined-safe async-task control @plan:PLAN-20260622-COREAPIGAP.P07 @requirement:REQ-003', () => {
  ...
  it('PROP completed/failed optional-field projection: a terminated task carries completedAt, a failed task carries its error, and a never-terminated control task omits BOTH @requirement:REQ-003 @scenario:property-terminal-projection @given:a discriminator in {complete, fail} and a unique id, with a real manager seeded with a terminal task (per discriminator) plus a never-terminated control task @when:agent.tasks.get() is called for the terminal id and the control id @then:the terminal view has completedAt:number>0; if fail, error===the seed; the control view OMITS both completedAt and error keys', async () => {
    const {
      pROPCompletedFailedOptionalFieldProjectionATerminatedTaskCarriesCompletedAtAProperty,
      observations,
    } =
      await observePROPCompletedFailedOptionalFieldProjectionATerminatedTaskCarriesCompletedAtA();
    await fc.assert(
      pROPCompletedFailedOptionalFieldProjectionATerminatedTaskCarriesCompletedAtAProperty,
    );
    expect(observations.map(({ ok }) => ok)).toStrictEqual(
      observations.map(() => true),
    );
    expect(
      observations.map(({ completedAtType }) => completedAtType),
    ).toStrictEqual(observations.map(() => 'number'));
    expect(
      observations.map(({ completedAtIsPositive }) => completedAtIsPositive),
    ).toStrictEqual(observations.map(() => true));
    expect(
      observations.map(({ hasCompletedAt }) => hasCompletedAt),
    ).toStrictEqual(observations.map(() => true));
    expect(observations.map(({ hasError }) => hasError)).toStrictEqual(
      observations.map(({ expectedHasError }) => expectedHasError),
    );
    expect(observations.map(({ actualError }) => actualError)).toStrictEqual(
      observations.map(({ expectedError }) => expectedError),
    );
    expect(
      observations.map(({ controlHasCompletedAt }) => controlHasCompletedAt),
    ).toStrictEqual(observations.map(() => false));
    expect(
      observations.map(({ controlHasError }) => controlHasError),
    ).toStrictEqual(observations.map(() => false));
    expect(
      observations.filter(({ controlKeys }) =>
        controlKeys.includes('completedAt'),
      ),
    ).toStrictEqual([]);
    expect(
      observations.filter(({ controlKeys }) => controlKeys.includes('error')),
    ).toStrictEqual([]);
  });

  const observePROPCompletedFailedOptionalFieldProjectionATerminatedTaskCarriesCompletedAtA =
    async () => {
      const observations: Array<{
        ok: boolean;
        completedAtType: string;
        completedAt: number | undefined;
        completedAtIsPositive: boolean;
        hasCompletedAt: boolean;
        hasError: boolean;
        expectedHasError: boolean;
        actualError: string | undefined;
        expectedError: string | undefined;
        controlHasCompletedAt: boolean;
        controlHasError: boolean;
        controlKeys: string[];
      }> = [];
      const pROPCompletedFailedOptionalFieldProjectionATerminatedTaskCarriesCompletedAtAProperty =
        fc.asyncProperty(
          ...
          async (kind, suffix) => {
            ...
            try {
              ...
              const expectedError =
                kind === 'fail' ? `err-${suffix}` : undefined;
              const ok =
                kind === 'complete'
              ...
              observations.push({
                ok,
                completedAtType: typeof termView.completedAt,
                completedAt: termView.completedAt,
                completedAtIsPositive:
                  typeof termView.completedAt === 'number' &&
                  termView.completedAt > 0,
                hasCompletedAt: 'completedAt' in termView,
                hasError: 'error' in termView,
                expectedHasError: expectedError !== undefined,
                actualError: termView.error,
                expectedError,
                controlHasCompletedAt: 'completedAt' in controlView,
                controlHasError: 'error' in controlView,
                controlKeys: Object.keys(controlView),
              });
              const terminatedFieldsAreValid =
                ok &&
                typeof termView.completedAt === 'number' &&
                termView.completedAt > 0 &&
                'completedAt' in termView;
              const errorFieldsAreValid =
                'error' in termView === (expectedError !== undefined) &&
                termView.error === expectedError;
              const controlFieldsAreAbsent =
                !('completedAt' in controlView) && !('error' in controlView);
              const controlKeysAreAbsent =
                !Object.keys(controlView).includes('completedAt') &&
                !Object.keys(controlView).includes('error');
              return (
                terminatedFieldsAreValid &&
                errorFieldsAreValid &&
                controlFieldsAreAbsent &&
                controlKeysAreAbsent
              );
            } finally {
              await cleanup();
            }
