...
describe('System prompt provider — non-foreground subagent (issue #3176, D5)', () => {
  ...
  it('sends the real template for the provider executing the subagent request', async () => {
    const { chat, capturedCalls, sentPrompt } =
      await observeSendsTheRealTemplateForTheProviderExecutingTheSubagentRequest();
  ...
  const observeSendsTheRealTemplateForTheProviderExecutingTheSubagentRequest =
    async () => {
      const settings = new SettingsService();
      settings.set('activeProvider', AMBIENT_PROVIDER);
      ...
      return { chat, capturedCalls, sentPrompt };
    };
});
