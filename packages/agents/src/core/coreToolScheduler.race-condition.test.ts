...
describe('CoreToolScheduler race conditions and recovery', () => {
  it('should handle race condition when later tools complete while publishBufferedResults is exiting', async () => {
    const { publishOrder, completionOrder, resolverCount } =
      await observeHandleRaceConditionWhenLaterToolsCompleteWhilePublishBufferedResultsIsExiting();
    expect(resolverCount).toBe(5);
    expect(completionOrder).toHaveLength(5);
    expect(publishOrder).toHaveLength(5);
    expect(publishOrder).toStrictEqual([1, 2, 3, 4, 5]);
  });

  const observeHandleRaceConditionWhenLaterToolsCompleteWhilePublishBufferedResultsIsExiting =
    async () => {
      ...
      await waitFor(
        () => {
          if (resolvers.size !== 5) {
            throw new Error('Waiting for all tool calls to start');
          }
        },
        { timeout: 1000 },
      );
      const resolverCount = resolvers.size;
      ...
      await waitFor(
        () => {
          if (completionOrder.length !== 5 || publishOrder.length !== 5) {
            throw new Error(
              'Waiting for all tool calls to complete and publish',
            );
          }
        },
        { timeout: 2000 },
      ...
      return { publishOrder, completionOrder, resolverCount };
    };

  it('should recover when all later tools complete before first tool', async () => {
    const { completionOrder, publishOrder } =
      await observeRecoverWhenAllLaterToolsCompleteBeforeFirstTool();
    expect(completionOrder).toHaveLength(5);
    expect(publishOrder).toHaveLength(5);
  ...
  const observeRecoverWhenAllLaterToolsCompleteBeforeFirstTool = async () => {
    ...
    await waitFor(
      () => {
        if (completionOrder.length !== 5 || publishOrder.length !== 5) {
          throw new Error('Waiting for all tool calls to complete and publish');
        }
      },
      { timeout: 2000 },
    );
    ...
    return { completionOrder, publishOrder };
  };
});
