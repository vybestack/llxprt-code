...
import { tokenLimit } from '@vybestack/llxprt-code-core/core/tokenLimits.js?context-limit-resolver-suite';

const resolveTokenLimit: typeof tokenLimit = (model, configuredLimit) =>
  configuredLimit ?? tokenLimit(model);
...
describe('getTokenLimitForConfiguredContext', () => {
  ...
  it('falls back to the model-name window when neither override nor provider limit is set', () => {
    ...
    expect(
      getTokenLimitForConfiguredContext('gpt-4o', config, resolveTokenLimit),
    ).toBe(128_000);
  });
