...
describe('CoreToolScheduler suggest edit and abort', () => {
  it('should publish suggest edit response as not confirmed and execute with edited command', async () => {
    const {
      ...
      await observePublishSuggestEditResponseAsNotConfirmedAndExecuteWithEditedCommand();
  ...
  const observePublishSuggestEditResponseAsNotConfirmedAndExecuteWithEditedCommand =
    async () => {
      ...
      const latestUpdate = onToolCallsUpdate.mock.calls.at(
        -1,
      )?.[0] as ToolCall[];
