...
describe('AgenticLoop pause loop-break (issue #2653)', () => {
  ...
  it('emits exactly one terminal done AFTER the todo_pause tool-result through mapLoopStream (issue #3087 ordering)', async () => {
    const {
      ...
      await observeEmitsExactlyOneTerminalDoneAFTERTheTodoPauseToolResultThrough();
  ...
  const observeEmitsExactlyOneTerminalDoneAFTERTheTodoPauseToolResultThrough =
    async () => {
      ...
      const isErrorObservation = pauseResult?.result.isError;
      return {
        pauseResult,
        doneEvents,
        pauseResultIndex,
