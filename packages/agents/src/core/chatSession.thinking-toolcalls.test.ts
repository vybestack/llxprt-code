...
describe('Issue #1150: Thinking blocks must be attached to tool call messages', () => {
  ...
  it('should combine thinking block with subsequent tool calls in same history entry when yielded separately', async () => {
    const {
      ...
      await observeCombineThinkingBlockWithSubsequentToolCallsInSameHistoryEntryWhen();
  ...
  const observeCombineThinkingBlockWithSubsequentToolCallsInSameHistoryEntryWhen =
    async () => {
  ...
  it('should preserve thinking signature when combining with tool calls', async () => {
    const { thinkingBlock, testSignature, signatureObservation } =
      await observePreserveThinkingSignatureWhenCombiningWithToolCalls();
    expect(thinkingBlock).toBeDefined();
    expect(signatureObservation).toBe(testSignature);
  });

  const observePreserveThinkingSignatureWhenCombiningWithToolCalls =
    async () => {
      const calls: GenerateChatOptions[] = [];
      ...
      return { thinkingBlock, testSignature, signatureObservation };
    };

  it('should handle multiple tool calls with thinking block in multi-turn conversation', async () => {
    const { calls, aiWithToolCall, hasThinking } =
      await observeHandleMultipleToolCallsWithThinkingBlockInMultiTurnConversation();
    expect(calls.length).toBe(2);
    expect(aiWithToolCall).toBeDefined();
    expect(hasThinking).toBe(true);
  });

  const observeHandleMultipleToolCallsWithThinkingBlockInMultiTurnConversation =
    async () => {
      let callCount = 0;
      const calls: GenerateChatOptions[] = [];
      ...
      return { calls, aiWithToolCall, hasThinking };
    };

  it('should NOT create separate history entries for thinking and tool calls', async () => {
    const { messagesWithBoth, thinkingOnlyMessages } =
      await observeNOTCreateSeparateHistoryEntriesForThinkingAndToolCalls();
    expect(messagesWithBoth.length).toBe(1);
    expect(thinkingOnlyMessages.length).toBe(0);
  });

  const observeNOTCreateSeparateHistoryEntriesForThinkingAndToolCalls =
    async () => {
      const generateChatCompletionMock = vi.fn(async function* () {
      ...
      return { messagesWithBoth, thinkingOnlyMessages };
    };
});
