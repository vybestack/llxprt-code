 ...
 *
 * These exercise the real `SubagentOrchestrator` assembly boundary: real
 ...
 * inspected without a full agent runtime.
 */
...
describe('SubagentOrchestrator — session dumpcontext inheritance (#3151)', () => {
  ...
  it('inherits the foreground dumpcontext for a load-balancer subagent', () => {
    // The LB activation path skips populatePreActivationSettings — the child
    // store never receives a profile-local dumpcontext — so the session-source
    // overlay is the sole supplier. Reproduce that assembly directly to avoid
    // the real LB provider activation (which intermittently hangs on agent
    // client initialisation) while still proving the overlay contract.
    const foreground = new SettingsService();
    foreground.setSessionScoped('dumpcontext', 'on');

    const isolatedSettings = new SettingsService({ sessionSource: foreground });
    isolatedSettings.setCurrentProfileName('lb-profile');
    isolatedSettings.set('activeProvider', 'load-balancer');
    isolatedSettings.set('providers.load-balancer.model', 'load-balancer');
