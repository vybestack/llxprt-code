...
describe('CoreToolScheduler cancelled tool responseParts', () => {
  it('should populate responseParts for cancelled tools when cancelAll is called', async () => {
    ...
    // Cancelled tools should have responseParts populated with tool_response only.
    // The tool_call is already in history from the model's assistant message;
    ...
    // responseParts should contain only tool_response (no tool_call)
    ...
    expect((toolResponsePart.result as { error: string }).error).toContain(
