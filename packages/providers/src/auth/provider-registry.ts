...
export class ProviderRegistry {
  ...
  constructor(private settings?: IOAuthSettingsProvider) {}

  /**
   * Register an OAuth provider with the registry
   * @param provider - The OAuth provider to register
   */
  registerProvider(provider: OAuthProvider | null | undefined): void {
    if (provider == null) {
  ...
  isOAuthEnabled(providerName: string): boolean {
    ...
    if (this.settings) {
      ...
      const oauthEnabledProviders = this.settings.getOAuthEnabledProviders();
  ...
  setOAuthEnabledState(providerName: string, enabled: boolean): void {
    ...
    if (this.settings) {
      this.settings.setOAuthEnabled(providerName, enabled);
    }
