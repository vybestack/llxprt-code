...
function tokenMapKey(provider: string, bucket?: string): string {
  return `${provider}:${bucket ?? 'default'}`;
}

function createTokenMapAccess(
  savedTokens: Map<string, OAuthToken>,
): Pick<TokenStore, 'saveToken' | 'getToken'> {
  return {
    saveToken: async (provider, token, bucket) => {
      savedTokens.set(tokenMapKey(provider, bucket), token);
    },
    getToken: async (provider, bucket) =>
      savedTokens.get(tokenMapKey(provider, bucket)) ?? null,
  };
}
...
describe('OAuthManager', () => {
    ...
    () => {
      ...
      describe.skipIf(skipInCI)('OAuth Enablement Management', () => {
        ...
        it('should check OAuth enablement status without triggering auth', async () => {
          ...
          expect(
            managerWithSettings.isOAuthEnabled('unpersisted-provider'),
          ).toBe(false);
      ...
      describe.skipIf(skipInCI)(
        'Status Reporting with OAuth Enablement',
        () => {
          /**
          ...
          it('should report OAuth enablement status for all providers', async () => {
            ...
            const mockIsEnabled = vi
              .fn()
              .mockImplementation(
                (provider) => provider === 'device-code-test',
              );
            (
              manager as unknown as { isOAuthEnabled: typeof mockIsEnabled }
            ).isOAuthEnabled = mockIsEnabled;
      ...
      describe.skipIf(skipInCI)('Provider Discovery', () => {
        ...
        it('prefers an explicit bucket over a single configured profile bucket during auth fallback', async () => {
          ...
          const tokenMapAccess = createTokenMapAccess(savedTokens);
          const tokenStore: TokenStore = {
            saveToken: vi.fn(tokenMapAccess.saveToken),
            getToken: vi.fn(tokenMapAccess.getToken),
  ...
  describe('OAuthManager.attachAddItemToProviders', () => {
    ...
    it('safely skips registered providers that do not implement setAddItem', () => {
      ...
      const tokenStore = new MockTokenStore();
      const manager = new OAuthManager(tokenStore);
      const recordingProvider = new AddItemRecordingProvider(
        'device-code-test',
      );
