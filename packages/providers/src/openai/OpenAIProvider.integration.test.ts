...
function createIntegrationProvider(
...
function isConfiguredDefaultModelAvailable(
  defaultModel: string | undefined,
  modelIds: readonly string[],
): boolean {
  return !defaultModel || modelIds.includes(defaultModel);
}

function getToolCallBlocks(content: IContent | undefined): IContent['blocks'] {
  return content?.blocks.filter((block) => block.type === 'tool_call') ?? [];
}

describe('OpenAIProvider integration test registration', () => {
  describe.skipIf(skipTests)('OpenAIProvider Integration Tests', () => {
    let provider: OpenAIProvider | null = null;

    beforeEach(() => {
      provider = createIntegrationProvider(OPENAI_API_KEY, OPENAI_BASE_URL);
    });
    ...
    it('should fetch real models from OpenAI API', async () => {
      ...
      const isDefaultModelValid = isConfiguredDefaultModelAvailable(
        defaultModel,
        modelIds,
      );
    ...
    it('should handle tool calls', { timeout: 10000 }, async () => {
      ...
      const toolCallBlocks = getToolCallBlocks(toolCallMessage);
