...
describe('Codex Responses WebSocket transport', () => {
  ...
  it('serializes each request value once before sending the bounded frame', async () => {
    let serializationCount = 0;
    const requestWithSerializationHook = {
      ...request(),
      metadata: {
        toJSON(): { readonly sequence: number } {
          serializationCount += 1;
          return { sequence: serializationCount };
        },
      },
    };
    const harness = new SocketHarness([completingScript()]);
    ...
    await drain(
      transport.streamResponse(requestWithSerializationHook, options()),
    );

    const payload = parsedObject(harness.sockets[0].sent[0]);
    expect(Reflect.get(payload, 'metadata')).toStrictEqual({ sequence: 1 });
