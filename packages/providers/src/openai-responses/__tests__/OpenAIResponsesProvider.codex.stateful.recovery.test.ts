...
async function requestBodyText(init?: RequestInit): Promise<string> {
  ...
  return body instanceof Blob ? body.text() : String(body ?? '');
}
...
function textOfBlock(block: IContent['blocks'][number]): string {
  return block.type === 'text' ? block.text : '';
}

class ParentRecoveryFetch {
  calls = 0;
  ...
  readonly fetch = async (
    ...
    this.calls += 1;
    this.bodies.push(await requestBodyText(init));
    if (this.calls === 1) {
...
function createCapturingFetch(
  ...
  return async (_input: unknown, init?: RequestInit): Promise<Response> => {
    capture(await requestBodyText(init));
    return sseResponse(responseId, responseText);
  };
}
...
describe('OpenAIResponsesProvider Codex stateful — parent rejection recovery and endpoint scoping @issue:3134', () => {
  ...
  describe('Fix 1 — one-shot recovery when previous_response_id is rejected', () => {
    it('retries once with full history when the API rejects a stored parent', async () => {
      const fetchHarness = new ParentRecoveryFetch();
      const originalFetch = globalThis.fetch;
      (globalThis as { fetch: unknown }).fetch = fetchHarness.fetch;
      try {
        ...
        const secondReq = JSON.parse(fetchHarness.bodies[1]) as Record<
          string,
          unknown
        >;
    ...
    it('re-establishes a new chain on the next turn instead of degrading for the session', async () => {
      const fetchHarness = new ChainRecoveryFetch();
      const originalFetch = globalThis.fetch;
      (globalThis as { fetch: unknown }).fetch = fetchHarness.fetch;
      try {
        const rejected = new Set<string>();
        ...
        fetchHarness.calls = 0;
        let turn2Body: string | undefined;
        (globalThis as { fetch: unknown }).fetch = createCapturingFetch(
          (body) => {
            turn2Body = body;
          },
          'resp_new2',
