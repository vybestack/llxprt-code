...
/**
 * Behavioral tests for the OpenAI prompt-envelope projections (issue #2817).
 *
 * Each provider builds a finalized request body in its preparation path. The
 * projections count tokens against ONLY the prompt-bearing typed fields
 * (messages/tools for OpenAI Chat; instructions/input/tools for OpenAI
 * Responses) — never the full HTTP body, which would inflate counts with
 * transport controls (stream, max_tokens, tool_choice) and raw base64 media.
 *
 * Cross-provider contracts (consistency, fail-fast, immutability) live in
 * promptEnvelopeProjections.test.ts; the Anthropic projection in
 * promptEnvelopeProjections.anthropic.test.ts.
 *
