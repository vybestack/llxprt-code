...
describe('MessageBus', () => {
  ...
  describe('requestConfirmation', () => {
    ...
    describe('DENY policy', () => {
      it('returns false immediately for DENY decision', async () => {
        ...
        const toolCall: PolicyFunctionCall = { name: 'shell', args: {} };
        const result = await messageBus.requestConfirmation(toolCall, {});
    ...
    describe('ASK_USER policy', () => {
      ...
      it('resolves false when user selects suggest edit', async () => {
        ...
        const toolCall: PolicyFunctionCall = {
        ...
        messageBus.respondToConfirmation(
          request.correlationId,
          ConfirmationOutcome.SuggestEdit,
          {
            editedCommand: 'ls -la',
          },
        );
    ...
    it('publishes confirmation response message', () => {
      ...
      expect(handler).toHaveBeenCalledWith(
        expect.objectContaining({
          ...
          correlationId: 'test-id',
          outcome: ConfirmationOutcome.ProceedOnce,
          confirmed: true,
        }),
      );
    });
  ...
  describe('debug mode', () => {
    it('logs messages when debug mode enabled', () => {
      const logs: unknown[][] = [];
      const logger: PolicyLogger = {
        log: (...args) => logs.push(args),
      };

      const debugBus = new MessageBus(policyEngine, true, logger);
      ...
      expect(logs).toStrictEqual([
        [expect.stringContaining('[MessageBus] Publishing'), message],
      ]);
    });
    ...
    it('does not log when debug mode disabled', () => {
      ...
      const silentBus = new MessageBus(policyEngine, false, logger);
      const message: ToolConfirmationRequest = {
        type: MessageBusType.TOOL_CONFIRMATION_REQUEST,
        toolCall: { name: 'edit', args: {} },
        correlationId: 'test-id',
