...
describe('bounded HTTP response acquisition', () => {
  ...
  function requireReadableBody(body: NodeJS.ReadableStream | null): Readable {
    if (!(body instanceof Readable)) {
      throw new Error('Expected a Node.js Readable response body');
    }
    return body;
  }
  ...
  describe('acquireBoundedHttpBody — overflow detection', () => {
    it('fails when a missing-Content-Length response exceeds the budget and closes the body', async () => {
      ...
      const response = await fetch(serverUrl(server));
      const body = requireReadableBody(response.body);
    ...
    const observeFailsOnObservedBytesWhenContentLengthAdvertisesLessThanTheStreamSendsAt179 =
      async () => {
    ...
    it('fails on observed bytes when Content-Length advertises less than the stream sends', async () => {
      const { fakeResponse } =
        await observeFailsOnObservedBytesWhenContentLengthAdvertisesLessThanTheStreamSendsAt179();
      await expect(
        acquireBoundedHttpBody(
          fakeResponse,
          createByteBudget(1024),
  ...
  describe('acquireBoundedHttpBody — strict Content-Length parsing', () => {
    const observeTreatsAMalformedOverLimitLookingContentLengthAsAbsentAndSucceedsWithABoundedBodyAt224 =
      async () => {
        const payload = 'x'.repeat(512);
        const fakeResponse: BoundedFetchResponse = {
    ...
    it('treats a malformed over-limit-looking Content-Length as absent and succeeds with a bounded body', async () => {
      const { payload, body } =
        await observeTreatsAMalformedOverLimitLookingContentLengthAsAbsentAndSucceedsWithABoundedBodyAt224();
      expect(body.text).toBe(payload);
      expect(body.metadata.observedBytes).toBe(512);
    });

    const observeTreatsAMalformedUnderstatedLookingContentLengthAsAbsentButFailsWhenObservedBytesExceedBudgAt245 =
      async () => {
        const payload = 'x'.repeat(2048);
        const fakeResponse: BoundedFetchResponse = {
          body: Readable.from([Buffer.from(payload)]),
    ...
    it('treats a malformed understated-looking Content-Length as absent but fails when observed bytes exceed budget', async () => {
      const { fakeResponse } =
        await observeTreatsAMalformedUnderstatedLookingContentLengthAsAbsentButFailsWhenObservedBytesExceedBudgAt245();
      await expect(
        acquireBoundedHttpBody(
          fakeResponse,
          createByteBudget(1024),
    ...
    const observeRejectsAnUnsafeNonSafeIntegerContentLengthAsAbsentAndStreamsObservedBytesAt264 =
      async () => {
    ...
    it('rejects an unsafe (non-safe-integer) Content-Length as absent and streams observed bytes', async () => {
      const { payload, body } =
        await observeRejectsAnUnsafeNonSafeIntegerContentLengthAsAbsentAndStreamsObservedBytesAt264();
      expect(body.text).toBe(payload);
