...
type DiscoveredToolExecution = Awaited<ReturnType<typeof executeTool>>;

function errorMessageOrEmpty(result: DiscoveredToolExecution): string {
  return result.error?.message ?? '';
}

function reportsExitCodeWhenFailed(
  result: DiscoveredToolExecution,
  content: string,
): boolean {
  return result.error === undefined || content.includes('Exit Code:');
}

describe('DiscoveredTool process and acquisition behavior', () => {
  describe.skipIf(process.platform === 'win32')(
    'DiscoveredTool bounded acquisition',
    () => {
      let tempDir: string;
      let cleanup: () => void;
      ...
      it(
        ...
        async () => {
          ...
          const content = stringContent(result.llmContent);
      ...
      it(
        ...
        async () => {
          ...
          const truncation = meta!['outputTruncation'] as Record<
            string,
            unknown
          >;
      ...
      it(
        ...
        async () => {
          ...
          const llm = stringContent(result.llmContent);
          const display = stringContent(result.returnDisplay);
          const errMsg = textOrEmpty(result.error?.message);
      ...
      const observeHandlesAMultiMegabyteProducerWithinBoundedMemoryAt190 =
        async () => {
          const script = createScript(
            tempDir,
            'huge-chunk.sh',
      ...
      it(
        ...
        async () => {
          const { content } =
            await observeHandlesAMultiMegabyteProducerWithinBoundedMemoryAt190();
      ...
      const observeHandlesManySmallInterleavedStdoutStderrChunksAt215 =
        async () => {
          const script = createScript(
            tempDir,
            'interleaved.sh',
      ...
      it(
        ...
        async () => {
          const { content } =
            await observeHandlesManySmallInterleavedStdoutStderrChunksAt215();
  ...
  describe.skipIf(process.platform === 'win32')(
    'DiscoveredTool already-aborted signal',
    () => {
      let tempDir: string;
      let cleanup: () => void;
      ...
      const observeDoesNotExecuteTheCommandWhenTheSignalIsAlreadyAbortedAt292 =
        async () => {
          ...
          const content =
            typeof result.llmContent === 'string' ? result.llmContent : '';
          return { markerPath, content };
        };

      it(
        'does not execute the command when the signal is already aborted',
        async () => {
          const { markerPath, content } =
            await observeDoesNotExecuteTheCommandWhenTheSignalIsAlreadyAbortedAt292();
  ...
  describe.skipIf(process.platform === 'win32')(
    'DiscoveredTool bounded cancellation',
    () => {
      let tempDir: string;
      let cleanup: () => void;
      ...
      const observeTerminatesTheSubprocessWhenTheAbortSignalFiresAt336 =
        async () => {
          const script = createScript(
            tempDir,
            'long-running.sh',
      ...
      it(
        ...
        async () => {
          const { result, content } =
            await observeTerminatesTheSubprocessWhenTheAbortSignalFiresAt336();
          expect(result.error).toBeDefined();
  ...
  describe.skipIf(process.platform === 'win32')(
    'DiscoveredTool unexpected signal is treated as failure',
    () => {
      let tempDir: string;
      let cleanup: () => void;
  ...
  describe.skipIf(process.platform === 'win32')(
    'DiscoveredTool stdin EPIPE during early child exit',
    () => {
      let tempDir: string;
      let cleanup: () => void;
      ...
      const observeChildClosingStdinImmediatelyWithLargePayloadReportsTruthfulResultAt497 =
        async () => {
          const script = createScript(
            tempDir,
            'close-stdin.sh',
      ...
      it(
        ...
        async () => {
          const { result, content } =
            await observeChildClosingStdinImmediatelyWithLargePayloadReportsTruthfulResultAt497();
          expect(typeof result.llmContent).toBe('string');
          expect(result.returnDisplay).toBe(content);
          expect(errorMessageOrEmpty(result)).toBe(content);
          expect(reportsExitCodeWhenFailed(result, content)).toBe(true);
        },
        { timeout: 15000 },
      );
    },
  ...
  describe.skipIf(process.platform === 'win32')(
    'DiscoveredTool drain timeout terminates descendant group',
    () => {
      let tempDir: string;
  ...
  describe.skipIf(process.platform === 'win32')(
    'DiscoveredTool abort settlement with failed termination',
    () => {
      let tempDir: string;
      let pidFile: string;
      let cleanup: () => void;
