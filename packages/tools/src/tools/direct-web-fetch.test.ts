...
describe('DirectWebFetchTool', () => {
  ...
  const observeShouldFetchAndReturnTextContentAt74 = async () => {
    const params: DirectWebFetchToolParams = {
  ...
  it('should fetch and return text content', async () => {
    const { result } = await observeShouldFetchAndReturnTextContentAt74();
  ...
  const observeShouldFetchAndReturnMarkdownContentAt109 = async () => {
    const params: DirectWebFetchToolParams = {
      url: 'https://example.com',
      format: 'markdown',
    };
  ...
  it('should fetch and return markdown content', async () => {
    const { result } = await observeShouldFetchAndReturnMarkdownContentAt109();
    expect(result.llmContent).toContain('# Hello');
    expect(result.llmContent).toContain('World');
  });
  ...
  const observeShouldHandleLargeFilesAt171 = async () => {
    const params: DirectWebFetchToolParams = {
  ...
  it('should handle large files', async () => {
    const { result } = await observeShouldHandleLargeFilesAt171();
    expect(result.error).toBeDefined();
    expect(result.error?.message).toMatch(/exceeds/i);
  });

  const observeReturnsFETCHERRORWhenTheBodyExceedsTheSizeLimitAndDoesNotRetryBodyAcquisitionAt194 =
    async () => {
      const params: DirectWebFetchToolParams = {
      ...
      const result = await invocation.execute(new AbortController().signal);
      return { overflowBody, result };
    };

  it('returns FETCH_ERROR when the body exceeds the size limit and does not retry body acquisition', async () => {
    const { overflowBody, result } =
      await observeReturnsFETCHERRORWhenTheBodyExceedsTheSizeLimitAndDoesNotRetryBodyAcquisitionAt194();
  ...
  describe('retry behavior', () => {
    const observeRetriesENOTFOUNDOnceAndSucceedsAt223 = async () => {
      const params: DirectWebFetchToolParams = {
        url: 'https://example.com',
        format: 'text',
      };
    ...
    it('retries ENOTFOUND once and succeeds', async () => {
      const { result } = await observeRetriesENOTFOUNDOnceAndSucceedsAt223();
    ...
    const observeRetriesRetryable5xxWhenStatusIsPreservedAt284 = async () => {
      const params: DirectWebFetchToolParams = {
        url: 'https://example.com',
        format: 'text',
      };
    ...
    it('retries retryable 5xx when status is preserved', async () => {
      const { result } =
        await observeRetriesRetryable5xxWhenStatusIsPreservedAt284();
      expect(mockedFetch).toHaveBeenCalledTimes(2);
      expect(result.error).toBeUndefined();
    ...
    const observeTimeoutAbortReturnsToolResultErrorAndCancelsRetriesAt344 =
      async () => {
        const params: DirectWebFetchToolParams = {
          url: 'https://example.com',
          format: 'text',
    ...
    it('timeout abort returns ToolResult.error and cancels retries', async () => {
      const { result } =
        await observeTimeoutAbortReturnsToolResultErrorAndCancelsRetriesAt344();
      expect(result.error).toBeDefined();
  ...
  describe('non-success response body disposal', () => {
    ...
    const observeDestroysThe5xxRetryableBodyOnEachRetryAttemptAt414 =
      async () => {
        const params: DirectWebFetchToolParams = {
        ...
        const result = await invocation.execute(new AbortController().signal);
        return { firstErrorBody, result };
      };

    it('destroys the 5xx retryable body on each retry attempt', async () => {
      const { firstErrorBody, result } =
        await observeDestroysThe5xxRetryableBodyOnEachRetryAttemptAt414();
      expect(mockedFetch).toHaveBeenCalledTimes(2);
      expect(result.error).toBeUndefined();
      expect(
