...
describe('Line range tools issue #3036', () => {
  /**
  ...
  function newlineSuffix(hasTrailingNewline: boolean): string {
  ...
  describe('AB1: delete_line_range preserves trailing-newline state', () => {
    ...
    it('confirm preview is byte-identical to the bytes execute writes', async () => {
      ...
      const confirmationDetails = requireConfirmation(confirmation);
      expect(confirmationDetails.type).toBe('edit');
      const preview = editConfirmation(confirmationDetails).newContent;
  ...
  describe('AB2: user-facing line counts exclude the phantom line', () => {
    ...
    it("a single '\\n' file reports length 1: delete(2,2) is out-of-bounds", async () => {
      ...
      expect(result.error?.message).toBe(
        'start_line 2 exceeds file length (1)',
      );
    });
  ...
  describe('regression: insert_at_line preserves newline state across content/file combinations', () => {
    ...
    const observeNameAt665 = async (c: {
      ...
      const fileContent =
        joinLines('aaa', 'bbb', 'ccc') + newlineSuffix(c.fileTrailingNewline);
      writeFileSync(filePath, fileContent, 'utf-8');
      const insertContent = 'xxx' + newlineSuffix(c.contentTrailingNewline);
      const result = await runInsert(tempDir(), {
        file_path: filePath,
        line_number: 2,
    ...
    it.each(cases.map((c, index) => ({ ...c, index })))('$name', async (c) => {
      const { filePath, result } = await observeNameAt665(c);
      expect(result.error).toBeUndefined();
      const expected =
        joinLines('aaa', 'xxx', 'bbb', 'ccc') +
        newlineSuffix(c.expectedTrailingNewline);
