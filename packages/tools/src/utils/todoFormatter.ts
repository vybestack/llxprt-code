...
import { type Todo, type TodoToolCall } from '../types/todo-schemas.js';
...
const formatTodoEntry = (todo: Todo): string => {
  ...
  const statusText = todo.status === 'in_progress' ? ' (in_progress)' : '';
  return `${marker} ${todo.content}${statusText}${currentSuffix}`;
};
