...
import type { ShellJob, SurvivorInfo } from './shellJobManager.js';
...
function firstFailureThenBoundedTaskkill(): (
  pid: number,
) => Promise<TaskkillResult> {
  let killCalls = 0;
  return (pid: number): Promise<TaskkillResult> => {
    killCalls++;
    if (killCalls === 1) {
...
function firstDisposalSurvivor(error: unknown): SurvivorInfo {
  if (!(error instanceof ShellJobDisposalError)) {
    throw new Error('Expected ShellJobDisposalError');
  }
  return error.survivors[0];
}

async function observeMicrotaskLaunchError(
  manager: ShellJobManager,
...
describe('ShellJobManager survivors behavior', () => {
    ...
    () => {
      ...
      it('dispose still reaps a survivor after retention evicts its job context (H2)', async () => {
        ...
        const h2Manager = new ShellJobManager({
          baseDir: h2Dir,
          maxBackgroundJobs: 1,
          taskkillImpl: firstFailureThenBoundedTaskkill(),
        });
        let survivorPid = 0;
        try {
      ...
      it('cancel and dispose complete cleanup when taskkillImpl throws synchronously (H3)', async () => {
        ...
        try {
          ...
          survivorPid = observedProcessPid(job.pid);
      ...
      it('reaps multiple survivors concurrently (bounded by one kill timeout, not N) (H5)', async () => {
        ...
        try {
          for (let i = 0; i < 3; i++) {
            ...
            survivorPids.push(observedProcessPid(job.pid));
          }
      ...
      it('dispose rejects with ShellJobDisposalError when kill always fails (I2)', async () => {
        const i2Dir = fs.mkdtempSync(
          path.join(os.tmpdir(), 'i2-fail-survivor-'),
        );
        ...
        try {
          ...
          const survivor = firstDisposalSurvivor(caughtError);
      ...
      it('launch() throws synchronously when dispose() has begun (lifecycle gate)', async () => {
        ...
        try {
          ...
          await reapAndRemoveWindowsTestDir(gateDir, gateManager, [
            survivorPid,
          ]);
        }
      }, 30000);
      ...
      it('launch() from a microtask scheduled during disposal also throws', async () => {
        const microDir = fs.mkdtempSync(
          path.join(os.tmpdir(), 'micro-launch-'),
        );
        ...
        try {
          ...
          const microError = await observeMicrotaskLaunchError(microManager);
