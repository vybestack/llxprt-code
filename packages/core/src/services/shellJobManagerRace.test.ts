...
function failureReasonText(failureReason: string | undefined): string {
  return failureReason ?? '';
}

describe('ShellJobManager race behavior', () => {
    ...
    () => {
      it('keeps cancellation as terminal owner when output crosses the cap afterward', async () => {
        const baseDir = fs.mkdtempSync(
          path.join(os.tmpdir(), 'shell-job-race-'),
        );
        ...
        try {
          ...
          expect(failureReasonText(terminal?.failureReason)).not.toContain(
            'exceeded cap',
          );
        } finally {
