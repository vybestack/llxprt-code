  ...
  implements HistoryServiceEventEmitter
{
  ...
  protected pendingOperations: Array<() => void> = [];
  protected pendingCompressionHighWaterReported: boolean = false;
  ...
  protected queueCompressionOperation(operation: () => void): void {
    ...
    if (
      ...
      this.pendingOperations.length >=
        HistoryServiceCore.COMPRESSION_QUEUE_HIGH_WATER
    ) {
  ...
  private async finalizeMutationEffects(
    effects: readonly PreparedHistoryBatchEffect[],
  ): Promise<void> {
    const failures: unknown[] = [];
    ...
    if (failures.length > 1) {
      throw new AggregateError(
        failures,
        'History mutation finalization failed',
      );
    }
  }
  ...
  private async commitHistoryMutation(input: {
    ...
    try {
      ...
      await this.finalizeMutationEffects(effects);
    } catch (error: unknown) {
      if (historyPublished) {
        this.invalidatePendingSyncs();
