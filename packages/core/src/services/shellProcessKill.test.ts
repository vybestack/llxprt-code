...
function isTerminationSignal(
  signal: NodeJS.Signals | null,
): signal is 'SIGTERM' | 'SIGKILL' {
  return signal === 'SIGTERM' || signal === 'SIGKILL';
}

describe('ShellProcessKill platform behavior', () => {
  describe.skipIf(isWindows)('escalateKillUnix pid validation (POSIX)', () => {
    it('pid 0 does NOT signal the caller process group (caller + sibling survive)', async () => {
      ...
      helper.unref();
      const helperPid = observedProcessPid(helper.pid);
      let siblingPid = 0;
      try {
    ...
    it('a valid pid of a real detached child is still killed (guard did not break the feature)', async () => {
      ...
      child.unref();
      const childPid = observedProcessPid(child.pid);
      ...
      try {
        ...
        // delivered at all.
        expect(isTerminationSignal(signal)).toBe(true);
      } finally {
        reapGroup(childPid);
      }
    }, 20000);
