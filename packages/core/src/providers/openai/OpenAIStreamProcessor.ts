 ...
 */
function applyTerminalMetadata(content: IContent, state: StreamingState): void {
...
/**
 * Emit usage-only terminal metadata chunk when no combined blocks.
 */
function* emitUsageOnlyMetadata(
  state: StreamingState,
  model: string,
  deps: StreamProcessorDeps,
): Generator<IContent, void, unknown> {
...
/**
 * Emit a terminal metadata chunk when there is no usage data (issue #1844).
 */
function* emitFinishOnlyMetadata(
  state: StreamingState,
  model: string,
  deps: StreamProcessorDeps,
): Generator<IContent, void, unknown> {
...
function* emitTerminalChunks(
  ...
  yield* emitUsageOnlyMetadata(state, model, deps);
  yield* emitFinishOnlyMetadata(state, model, deps);
}

/**
 * Log summary of empty/non-empty streaming response.
 */
function logStreamCompletionSummary(
  state: StreamingState,
  toolCallCount: number,
  model: string,
 ...
 */
function buildToolCallsForHistory(
...
async function* handleToolCallsWithoutText(
  ...
  if (!hasToolsButNoTextContent(state, toolCallCount)) {
    logStreamCompletionSummary(state, toolCallCount, model, baseURL, deps);
    return;
  }
  ...
  const toolCallsForHistory = buildToolCallsForHistory(pipelineResult);
