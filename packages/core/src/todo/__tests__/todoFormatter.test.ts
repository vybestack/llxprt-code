...
import type { Todo, TodoToolCall } from '@vybestack/llxprt-code-tools';
...
describe('formatTodoListForDisplay (core)', () => {
  ...
  describe('status and summary lines', () => {
    it('renders the header, summary, and the core in_progress line without (in_progress)', () => {
      const todo = makeTodo({
        content: 'Active task',
        status: 'in_progress',
      });
      ...
      expect(result).toContain('→ Active task ← current');
