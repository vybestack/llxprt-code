...
import type { ResolvedSearchTarget } from '../resolveTextSearchTarget.js';
...
describe('resolveTextSearchTarget', () => {
  ...
  it('should return kind directory for valid directory path', () => {
    ...
    const narrowed = result as Extract<
      ResolvedSearchTarget,
      { kind: 'directory' }
    >;
    expect(narrowed.searchDir).toBe(path.join(tempDir, 'sub'));
  });
  ...
  it('should return kind file with filePath, parentDir, basename for valid file path', () => {
    ...
    const narrowed = result as Extract<ResolvedSearchTarget, { kind: 'file' }>;
    expect(narrowed.filePath).toBe(path.join(tempDir, 'file.txt'));
    expect(narrowed.parentDir).toBe(tempDir);
    expect(narrowed.basename).toBe('file.txt');
  });
  ...
  it('should return kind file for nested file path', () => {
    ...
    expect(result.kind).toBe('file');
    const narrowed = result as Extract<ResolvedSearchTarget, { kind: 'file' }>;
    expect(narrowed.filePath).toBe(path.join(tempDir, 'sub', 'nested.txt'));
    expect(narrowed.parentDir).toBe(path.join(tempDir, 'sub'));
    expect(narrowed.basename).toBe('nested.txt');
  });
  ...
  it('should handle absolute directory path', () => {
    ...
    const narrowed = result as Extract<
      ...
      { kind: 'directory' }
    >;
    expect(narrowed.searchDir).toBe(absPath);
  });
  ...
  it('should handle absolute file path', () => {
    ...
    expect(result.kind).toBe('file');
    const narrowed = result as Extract<ResolvedSearchTarget, { kind: 'file' }>;
    expect(narrowed.filePath).toBe(absPath);
    expect(narrowed.parentDir).toBe(tempDir);
    expect(narrowed.basename).toBe('file.txt');
  });
  ...
  it('should handle dot path as directory', () => {
    ...
    const narrowed = result as Extract<
      ...
      { kind: 'directory' }
    >;
    expect(narrowed.searchDir).toBe(tempDir);
  });
});
