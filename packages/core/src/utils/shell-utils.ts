...
export function stripShellWrapper(command: string): string {
  // Match sh, bash, zsh with -c flag, cmd.exe with /c flag,
  // or powershell.exe/pwsh with optional -NoProfile and -Command flag
  const pattern =
    /^\s*(?:(?:sh|bash|zsh)\s+-c\s+|cmd\.exe\s+\/c\s+|(?:powershell\.exe|pwsh)\s+(?:-NoProfile\s+)?-Command\s+)/i;
