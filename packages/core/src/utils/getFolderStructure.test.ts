...
`.trim();
  ...
  describe('with llxprtignore', () => {
    it('should ignore llxprtignore files by default', async () => {
      await fsPromises.writeFile(
        nodePath.join(testRootDir, '.llxprtignore'),
        'ignored.txt\nnode_modules/\n.llxprt/\n!/.llxprt/config.yaml',
      );
      await createTestFile('file1.txt');
      await createTestFile('node_modules', 'some-package', 'index.js');
    ...
    it('should not ignore files if respectLlxprtIgnore is false', async () => {
      await fsPromises.writeFile(
        nodePath.join(testRootDir, '.llxprtignore'),
        'ignored.txt\nnode_modules/\n.llxprt/\n!/.llxprt/config.yaml',
      );
