...
describe('secure-browser-launcher', () => {
  ...
  describe('Browser-specific launch with profile directory', () => {
    ...
    it('accepts a Firefox absolute profile path with traversal segments', async () => {
      // Absolute Firefox profile paths are a legitimate product feature
      // (Firefox -profile /path/to/profile). Traversal sequences are safe
      // here because the path is passed via execFile (no shell) and the
      // user explicitly chooses the profile directory.
      setPlatform('darwin');
      const profilePath = '/Users/testuser/../other/Profiles/work';
      ...
      expect(mockExecFile).toHaveBeenCalledWith(
        'open',
        expect.arrayContaining(['-profile', profilePath]),
        expect.any(Object),
      );
    });
    ...
    it('throws when no Windows Chrome install location is found', async () => {
      ...
      // skipped because stat rejects for all candidate paths).
      mockExecFile.mockReset();
