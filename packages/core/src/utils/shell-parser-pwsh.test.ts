...
/**
 * Returns true when the error indicates powershell.exe is not installed, which
 * allows the Windows conformance probe to be silently skipped on machines without
 * PowerShell rather than failing a Linux/macOS-friendly test.
 */
function isMissingPowershell(error: unknown): boolean {
  return (
...
/**
 * Compares a single command source against PowerShell's ground-truth parser by
 * passing the source as DATA through stdin. ParseInput parses but never executes
 * the input. A missing powershell.exe is treated as an acceptable skip.
 */
function runParseInputConformance(source: string): void {
  const helperScript =
...
describe('PowerShell parser behavior', () => {
...
describe('PowerShell parser lifecycle behavior', () => {
 ...
 */
describe('PowerShell saved-corpus behavior', () => {
  describePwsh('saved-corpus construct families', () => {
    const corpus: Array<{ label: string; cmd: string }> = [
      ...
      {
        label: 'ForEach-Object block',
        cmd: 'ForEach-Object { Write-Host $_ }',
      },
      ...
      {
        label: 'var + method chain',
        cmd: '$text = "hello"; $text.ToUpper()',
      },
      // Family 20: hash table
      {
        label: 'hash table literal',
        cmd: '$h = @{ Key = "Value"; Num = 42 }',
      },
      // Family 21: try/catch
      {
 ...
 */
describe('PowerShell .NET root behavior', () => {
  describePwsh('exact .NET roots', () => {
    ...
    it('nested .NET in arguments produces only outer command name', () => {
      ...
      expect(tree).not.toBeNull();
      const names = extractCommandNamesForLanguage(tree!, 'powershell');
      expect(names).toStrictEqual(['Write-Host']);
    });
  });
});
 ...
 */
describe('PowerShell Parser.ParseInput conformance', () => {
