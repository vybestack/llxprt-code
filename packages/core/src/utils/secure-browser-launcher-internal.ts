...
/**
 * Internal implementation module for the secure browser launcher.
 *
 * This module is NOT exported through the package subpath map. It houses the
 * dependency-injected factory ({@link createBrowserLauncher}) used by tests
 * with fake executors, plus all platform/browser-specific launch logic. The
 * public surface ({@link openBrowserSecurely}, {@link shouldLaunchBrowser},
 * {@link validateProfileDirectory}, {@link BrowserKind},
 * {@link BrowserLaunchOptions}) is re-exported from
 * `secure-browser-launcher.ts`, which IS the package subpath.
 *
 * Every browser-execution path exposed by the factory checks the test guard
 * so that no code path — public or internal — can bypass the fail-closed
 * policy during detected test processes.
 */
...
const TEST_BROWSER_LAUNCH_OPT_IN = 'LLXPRT_ALLOW_BROWSER_LAUNCH_IN_TESTS';
...
export interface BrowserLauncher {
  readonly openBrowserSecurely: (
    url: string,
    options?: BrowserLaunchOptions,
  ) => Promise<void>;
  readonly shouldLaunchBrowser: (options?: {
    forceManual?: boolean;
  }) => boolean;
}

const defaultBrowserLauncherDependencies: BrowserLauncherDependencies = {
...
export async function openBrowserSecurely(
  ...
  await defaultBrowserLauncher.openBrowserSecurely(url, options);
}

/**
 * Factory that creates a {@link BrowserLauncher} backed by the supplied
 * dependencies. Every method on the returned launcher enforces its supplied
 * test policy before delegating to the dependency-injected implementation.
 * The production launcher supplies the environment-based fail-closed policy;
 * unit tests supply an explicit policy alongside fake execution dependencies.
 */
export function createBrowserLauncher(
  dependencies: BrowserLauncherDependencies,
): BrowserLauncher {
  return {
    openBrowserSecurely: async (url, options) => {
      if (dependencies.isBrowserLaunchDisabledDuringTests()) {
        throw new Error(
          `Browser launch is disabled during tests. Set ${TEST_BROWSER_LAUNCH_OPT_IN}=true to enable it explicitly.`,
        );
      }
      await openBrowserSecurelyWithDependencies(url, options, dependencies);
    },
    shouldLaunchBrowser: (options) => {
      if (dependencies.isBrowserLaunchDisabledDuringTests()) {
        return false;
      }
      return shouldLaunchBrowserForPlatform(options, dependencies.platform());
    },
  };
}

const defaultBrowserLauncher = createBrowserLauncher(
  defaultBrowserLauncherDependencies,
);

async function openBrowserSecurelyWithDependencies(
  url: string,
  ...
  await openDefaultBrowser(url, dependencies);
}
...
export function shouldLaunchBrowser(
  ...
  return defaultBrowserLauncher.shouldLaunchBrowser(options);
}

function shouldLaunchBrowserForPlatform(
  options: { forceManual?: boolean } | undefined,
  platformName: NodeJS.Platform,
): boolean {
...
async function tryFallbackBrowserCommands(
  ...
  url: string,
  options: ExecFileOptions,
  execute: BrowserLauncherDependencies['execute'],
): Promise<boolean> {
  for (const fallbackCommand of fallbackCommands) {
    try {
      await execute(fallbackCommand, [url], options);
      return true;
    } catch {
      // Try next command
    }
...
async function openSpecificBrowser(
  ...
  try {
    await dependencies.execute(command, args, getBrowserExecOptions());
  } catch (error) {
    const succeeded =
      (await tryLinuxFallbackBinaries(
        platformName,
        browser,
        command,
        args,
      ...
      (await tryWindowsFallbackBinaries(
        platformName,
        browser,
        command,
        args,
...
async function tryLinuxFallbackBinaries(
  ...
  for (const fallback of fallbacks) {
    ...
    tried.add(fallback);
    try {
      await execute(fallback, args, getBrowserExecOptions());
      return true;
    } catch {
...
async function tryWindowsFallbackBinaries(
  ...
  for (const candidate of candidates) {
    ...
    const isExecutableFile = await dependencies
      .stat(resolved)
    ...
    tried.add(resolved);
    try {
      await dependencies.execute(resolved, args, getBrowserExecOptions());
      return true;
    } catch {
...
export function isBrowserLaunchDisabledDuringTests(): boolean {
  const currentEnvironment = process.env;
  const isTestProcess =
    currentEnvironment.LLXPRT_RUNNING_TESTS === 'true' ||
    currentEnvironment.NODE_ENV === 'test';
  return (
    isTestProcess && currentEnvironment[TEST_BROWSER_LAUNCH_OPT_IN] !== 'true'
  );
}
