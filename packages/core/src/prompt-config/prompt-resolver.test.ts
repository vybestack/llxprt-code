...
describe('PromptResolver', () => {
  ...
  describe('resolveFile', () => {
    ...
    it('should find provider-specific file over base file', async () => {
      ...
      await fs.mkdir(path.join(tempDir, 'providers', 'openai'), {
        recursive: true,
      });
      await fs.writeFile(
        path.join(tempDir, 'providers', 'openai', 'core.md'),
        'provider content',
      );
    ...
    it('should find model-specific file over provider and base files', async () => {
      ...
      await fs.writeFile(path.join(tempDir, 'core.md'), 'base content');
      await fs.mkdir(
        path.join(tempDir, 'providers', 'openai', 'models', 'gpt-4'),
        { recursive: true },
      );
      await fs.writeFile(
        path.join(tempDir, 'providers', 'openai', 'core.md'),
      ...
      await fs.writeFile(
        path.join(tempDir, 'providers', 'openai', 'models', 'gpt-4', 'core.md'),
        'model content',
      );
    ...
    it('should handle nested relative paths correctly', async () => {
      ...
      await fs.writeFile(
        path.join(tempDir, 'tools', 'read-file.md'),
        'base tool content',
      );

      const context: Partial<PromptContext> = {};

      const result = resolver.resolveFile(
        tempDir,
        'tools/read-file.md',
        context,
      );
    ...
    it('should handle provider names with special characters', async () => {
      ...
      await fs.mkdir(path.join(tempDir, 'providers', 'anthropic-claude'), {
        recursive: true,
      });
      await fs.writeFile(
        path.join(tempDir, 'providers', 'anthropic-claude', 'core.md'),
        'provider content',
      );
    ...
    it('should handle very long model names by truncating', async () => {
      ...
      await fs.mkdir(
        path.join(tempDir, 'providers', 'openai', 'models', truncatedModelName),
        { recursive: true },
      );
      await fs.writeFile(
        path.join(
    ...
    it('should handle null context gracefully', async () => {
      ...
      const result = resolver.resolveFile(
        tempDir,
        'core.md',
        null as unknown as Partial<PromptContext>,
      );
  ...
  describe('resolveAllFiles', () => {
    ...
    it('should resolve environment prompts based on context', async () => {
      ...
      await fs.writeFile(
        path.join(tempDir, 'env', 'git-repository.md'),
        'git content',
      );
      await fs.writeFile(
        path.join(tempDir, 'env', 'sandbox.md'),
        'sandbox content',
      );
      await fs.writeFile(
        path.join(tempDir, 'env', 'ide-mode.md'),
        'ide content',
      );
    ...
    it('should resolve tool prompts with kebab-case conversion', async () => {
      ...
      await fs.writeFile(
        path.join(tempDir, 'tools', 'write-file.md'),
        'write file content',
      );
    ...
    it('should use hierarchical resolution for all file types', async () => {
      ...
      await fs.mkdir(path.join(tempDir, 'tools'), { recursive: true });
      await fs.mkdir(path.join(tempDir, 'providers', 'openai', 'tools'), {
        recursive: true,
      });
      ...
      await fs.writeFile(
        path.join(tempDir, 'providers', 'openai', 'tools', 'read-file.md'),
        'provider tool',
      );
    ...
    it('should handle null inputs gracefully', () => {
      const result = resolver.resolveAllFiles(
  ...
  describe('sanitizePathComponent', () => {
    ...
    it('should replace special characters with hyphens', () => {
      expect(resolver.sanitizePathComponent('anthropic/claude')).toBe(
        'anthropic-claude',
      );
    });

    it('should collapse multiple special characters to single hyphen', () => {
      expect(resolver.sanitizePathComponent('model@#$%name')).toBe(
        'model-name',
      );
    });

    it('should remove leading and trailing hyphens', () => {
      expect(resolver.sanitizePathComponent('-leading-trailing-')).toBe(
        'leading-trailing',
      );
    });
  ...
  describe('convertToKebabCase', () => {
    ...
    it('should handle numbers in names', () => {
      expect(resolver.convertToKebabCase('Base64Encode')).toBe(
        'base-64-encode',
      );
  ...
  describe('listAvailableFiles', () => {
    ...
    it('should include provider overrides', async () => {
      ...
      expect(result).toHaveLength(2);
      expect(
        result.find((f) => f.path === 'core.md' && f.source === 'base'),
      ).toBeTruthy();
      expect(
        result.find(
          (f) =>
            f.path === 'providers/openai/core.md' && f.source === 'provider',
        ),
      ).toBeTruthy();
    });
    ...
    it('should handle invalid file type by defaulting to "all"', async () => {
      ...
      const result = resolver.listAvailableFiles(
        tempDir,
        'invalid' as 'core' | 'env' | 'tool' | 'all',
      );
    ...
    it('should skip non-markdown files', async () => {
      ...
      expect(
        result.find((f) => f.path.includes('config.json')),
      ).toBeUndefined();
    });
  ...
  describe('validateFileStructure', () => {
    ...
    it('should report warnings for non-markdown files', async () => {
      ...
      expect(result.isValid).toBe(true);
      expect(
        result.warnings.some((w) => w.includes('Non-markdown file found')),
      ).toBe(true);
    ...
    it('should report warnings for large files', async () => {
      ...
      expect(
        result.warnings.some(
          (w) => w.includes('Large file found') && w.includes('large.md'),
        ),
      ).toBe(true);
    });
    ...
    it('should report warnings for invalid filenames', async () => {
      ...
      expect(
        result.warnings.some(
          (w) => w.includes('Invalid filename') && w.includes('file@#$.md'),
        ),
      ).toBe(true);
    });
    ...
    it('should check read permissions on core.md', async () => {
      ...
      expect(result.errors).toContain(
        'Cannot read core.md - check permissions',
      );
