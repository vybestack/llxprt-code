 ...
 *
 * Integration tests for ChatSession runtime context isolation.
...
function createProviderRuntimeStub(runtimeId: string) {
  return createProviderRuntimeContext({
    ...
    runtimeId,
    metadata: { source: 'chatSession-isolation.integration.test' },
  });
}
...
describe('ChatSession Isolation Integration Tests', () => {
  ...
  describe('Provider/Model Isolation', () => {
    it('should keep foreground model unchanged after subagent execution', async () => {
      ...
      // In real scenario, ChatSession would be instantiated with each context
