...
describe('TodoRead', () => {
  ...
  describe('execute', () => {
    ...
    it('should sort todos by status (in_progress > pending > completed)', async () => {
      ...
      const inProgressIndex = lines.findIndex(line => line.includes('High priority in progress task'));
      const pendingIndex = lines.findIndex(line => line.includes('Medium priority pending task'));
      const completedIndex = lines.findIndex(line => line.includes('Low priority completed task'));
    ...
    it('should sort by priority within same status', async () => {
      ...
      const highPendingIndex = lines.findIndex(line => line.includes('High priority pending task'));
      const mediumPendingIndex = lines.findIndex(line => line.includes('Medium priority pending task'));
      
      expect(highPendingIndex).toBeLessThan(mediumPendingIndex);
    });

    it('should handle read errors gracefully', async () => {
      vi.mocked(TodoStore.prototype.readTodos).mockRejectedValue(new Error('File read error'));
      
      await expect(tool.execute({}, abortSignal)).rejects.toThrow('File read error');
    });
  });
