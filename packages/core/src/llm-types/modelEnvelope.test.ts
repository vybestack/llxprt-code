...
describe('accumulateModelStreamChunk property-based', () => {
  it('for ANY sequence of text-block chunks, accumulated blocks length === sum of chunk block lengths and text preserved in order', () =>
    fc.assert(
      fc.property(
        fc.array(
          fc.array(
  ...
  it('accumulate is pure: repeated accumulation yields same result', () =>
    fc.assert(
...
describe('getToolCalls property-based', () => {
  it('n tool_call blocks in → n ToolCallRequests out with matching names', () =>
    fc.assert(
      fc.property(
        fc.array(
          fc.record({
...
describe('toModelStreamChunk property-based', () => {
  ...
  it('any raw stopReason produces a canonical finishReason and preserves raw', () =>
    fc.assert(
      fc.property(fc.string({ minLength: 0, maxLength: 30 }), (raw: string) => {
        const ic: IContent = {
          speaker: 'ai',
          blocks: [],
  ...
  it('toModelStreamChunk never mutates input IContent', () =>
    fc.assert(
  ...
  it('toModelStreamChunk preserves usage byte-identical from metadata', () =>
    fc.assert(
      fc.property(
        fc.record({
  ...
  it('toModelStreamChunk preserves id as responseId', () =>
    fc.assert(
...
describe('emptyModelOutput property-based', () => {
  it('always produces empty blocks array and correct speaker', () =>
    fc.assert(
  ...
  it('result has no optional keys regardless of speaker', () =>
    fc.assert(
      fc.property(fc.constantFrom('human', 'ai', 'tool'), (speaker) => {
        const out = emptyModelOutput(speaker);
...
describe('accumulateModelStreamChunk additional property-based', () => {
  it('providerMetadata shallow-merge always has chunk keys overwrite acc keys', () =>
    fc.assert(
      fc.property(
        fc.record({
          a: fc.nat({ max: 100 }),
  ...
  it('usage from last chunk always wins', () =>
    fc.assert(
      fc.property(
        fc.record({
  ...
  it('accumulating into emptyModelOutput preserves block count', () =>
    fc.assert(
      fc.property(
  ...
  it('getToolCalls extracts matching args when parameters is a plain object', () =>
    fc.assert(
      fc.property(
        fc.array(
          fc.record({
        ...
        (calls) => {
          const output: ModelOutput = {
            content: {
              speaker: 'ai',
              blocks: calls.map((c) =>
                toolCallBlock(c.id, c.name, c.parameters),
              ),
            },
          };
