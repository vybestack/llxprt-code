...
describe('JanitorLease — shared safety harness', () => {
  afterEach(async () => {
    if (trackedLease) {
...
stderr: ${getStderr()}`,
  ...
  describe('JanitorLease — atomic publication (Item 5)', () => {
    ...
    /**
     * True for any lease-file or temp-artifact entry name (used to assert the
     * exact set of durable artifacts after acquire).
     */
    function isLeaseOrTempArtifact(entry: string): boolean {
      return entry.includes('.tmp') || entry.includes('.lease');
    }
    ...
    it('does not leave temp artifacts after successful acquire', async () => {
      ...
      expect(entries.filter(isLeaseOrTempArtifact)).toStrictEqual([
        LEASE_FILE_NAME,
      ]);
