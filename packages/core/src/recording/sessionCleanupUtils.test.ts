...
describe('sessionCleanupUtils @plan:PLAN-20260211-SESSIONRECORDING.P16', () => {
  ...
  describe('Property-based tests @plan:PLAN-20260211-SESSIONRECORDING.P16', () => {
     ...
     * @requirement REQ-CLN-001
     */
    it('all N .jsonl session files are discovered for any N @requirement:REQ-CLN-001', () =>
      fc.assert(
     ...
     * @requirement REQ-CLN-004
     */
    it('orphaned lock cleanup is safe: only orphans removed for any counts @requirement:REQ-CLN-004', () =>
      fc.assert(
        fc.asyncProperty(
          fc.nat({ max: 5 }),
          fc.nat({ max: 5 }),
     ...
     * @requirement REQ-CLN-002
     */
    it('shouldDeleteSession never returns delete for active-locked sessions @requirement:REQ-CLN-002', () =>
      fc.assert(
     ...
     * @requirement REQ-CLN-002, REQ-CLN-003
     */
    it('shouldDeleteSession result is consistent with PID liveness @requirement:REQ-CLN-002', () =>
      fc.assert(
        fc.asyncProperty(fc.boolean(), async (useAlivePid) => {
          ...
          try {
            ...
            const expectedResult = useAlivePid ? 'skip' : 'stale-lock-only';
     ...
     * @requirement REQ-CLN-001
     */
    it('non-session files are never included in scan results @requirement:REQ-CLN-001', () =>
      fc.assert(
        fc.asyncProperty(fc.nat({ max: 8 }), async (count) => {
          const localTemp = fs.mkdtempSync(
     ...
     * @requirement REQ-CLN-004
     */
    it('cleanupStaleLocks returns correct count for any combo of orphan/stale/active @requirement:REQ-CLN-004', () =>
      fc.assert(
        fc.asyncProperty(
          fc.nat({ max: 4 }),
          fc.nat({ max: 4 }),
     ...
     * @requirement REQ-CLN-003
     */
    it('stale lock on recent sessions: lock removed, session preserved @requirement:REQ-CLN-003', () =>
      fc.assert(
     ...
     * session files).
     */
    it('stale lock cleanup removes locks but never deletes session files @requirement:REQ-CLN-003', () =>
      fc.assert(
        fc.asyncProperty(fc.nat({ max: 5 }), async (count) => {
          const localTemp = fs.mkdtempSync(
     ...
     * @requirement REQ-CLN-003
     */
    it('stale lock status never causes deletion of sessions within retention window @requirement:REQ-CLN-003', () =>
      fc.assert(
        fc.asyncProperty(
          fc.nat({ max: 3 }),
          fc.nat({ max: 3 }),
     ...
     * @requirement REQ-CLN-001
     */
    it('getAllJsonlSessionFiles always returns entries with correct structure @requirement:REQ-CLN-001', () =>
      fc.assert(
        fc.asyncProperty(fc.nat({ max: 5 }), async (count) => {
          const localTemp = fs.mkdtempSync(
     ...
     * @requirement REQ-CLN-002
     */
    it('shouldDeleteSession returns delete for all unlocked sessions @requirement:REQ-CLN-002', () =>
      fc.assert(
        fc.asyncProperty(fc.nat({ max: 6 }), async (count) => {
          const localTemp = fs.mkdtempSync(
     ...
     * @requirement REQ-CLN-004
     */
    it('cleanupStaleLocks on directory with only session files (no locks) returns 0 @requirement:REQ-CLN-004', () =>
      fc.assert(
