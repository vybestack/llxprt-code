  ...
  describe('tryCompressChat', () => {
    ...
    beforeEach(() => {
      ...
      client['chat'] = {
        ...
        getLastPromptTokenCount: mockGetTotalTokens,
    ...
    it('should return NOOP if history is too short to compress', async () => {
      ...
      mockGetTotalTokens.mockReturnValue(50);
    ...
    it('uses history token count when prompt tokens are unavailable', async () => {
      ...
      const result = await client.tryCompressChat('prompt-id-fallback', true);
    ...
    it('restores history when compression inflates token count', async () => {
      ...
      const result = await client.tryCompressChat('prompt-id-restore', true);

      expect(result.compressionStatus).toBe(
        CompressionStatus.COMPRESSION_FAILED_INFLATED_TOKEN_COUNT,
      );
      expect(historyState).toEqual(originalHistory);
    });
  ...
  describe('sendMessageStream', () => {
    ...
    it.each([
      ...
      async ({ compressionStatus }) => {
        ...
        expect(events.some((e) => e.type === GeminiEventType.Content)).toBe(
          true,
        );
        expect(events.some((e) => e.type === GeminiEventType.Finished)).toBe(
          true,
        );
        // Should NOT emit ChatCompressed
        expect(
          events.some((e) => e.type === GeminiEventType.ChatCompressed),
        ).toBe(false);
      },
    );
