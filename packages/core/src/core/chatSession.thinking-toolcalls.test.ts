 ...
 *
 * 2. ChatSession.makeApiCallAndProcessStream converts each IContent to GenerateContentResponse:
 ...
 *
 * 3. ChatSession.processStreamResponse accumulates parts into modelResponseParts[]
 *
 * 4. ChatSession.recordHistory receives modelOutput with all parts consolidated
 *
...
describe('Issue #1150: Thinking blocks must be attached to tool call messages', () => {
  ...
  it('ISSUE #1150 REAL BUG: thinking block NOT in error dump - the Turn.run contextForReport shows Gemini format not IContent', async () => {
    ...
    // The isThoughtPart check from chatSession.ts:
