    ...
    try {
      ...
      if (candidate !== undefined) {
        ...
        if (active !== undefined && active.name !== desiredProviderName) {
          this.logger.debug(
            () =>
              `[ChatSession] selected provider '${desiredProviderName}' via getProviderByName (active remains '${active.name}') [${contextLabel}]`,
          );
        }
        return candidate;
      }
      ...
      this.logger.debug(
        () =>
          `[ChatSession] provider lookup skipped (${contextLabel}): ${error instanceof Error ? error.message : String(error)}`,
      );
    }
    return undefined;
  }
  ...
  resolveProviderForRuntime(contextLabel: string): IProvider {
    ...
    if (desiredProviderName && provider.name !== desiredProviderName) {
      ...
      try {
        ...
        this.logger.debug(
          () =>
            `[ChatSession] enforced provider switch to '${desiredProviderName}' (previous '${previousProviderName}') [${contextLabel}]`,
        );
      } catch (error) {
        this.logger.debug(
          () =>
            `[ChatSession] provider switch skipped (${contextLabel}, read-only context): ${error instanceof Error ? error.message : String(error)}`,
        );
      }
    }
