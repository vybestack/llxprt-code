...
### Changed (0.12.0 breaking)

- **Hook wire format v2 (#2624):** `BeforeModel`, `AfterModel`, and `BeforeToolSelection` hooks now exchange versioned, provider-neutral v2 payloads, and the v1 decode path is removed with no fallback. On the input side, `llm_request` is `{version: 2, model, contents, tools?, settings?}` over neutral `IContent` (the version field is stamped by the runtime); `AfterModel` adds `llm_response = {version: 2, content, finishReason?, rawStopReason?, usage?}`. On the output side, a hook-returned `llm_request` is a partial v2 envelope (`contents` replace the conversation wholesale, `model` overrides, `settings` shallow-merge); a hook-returned `llm_response` is keyed on `content` (no longer `candidates[].content.parts`); and `BeforeToolSelection` returns `toolChoice = {mode: 'auto'|'required'|'none', allowedToolNames?}`, replacing the Gemini-shaped `toolConfig` (`AUTO|ANY|NONE` + `allowedFunctionNames`). The `BeforeToolSelection` input is now an object envelope (`{version: 2, model, contents: [], tools}`) instead of the bare tools array. Hook payloads with v1 shapes (for example `llm_request.messages` or `llm_response.candidates`) are ignored rather than decoded. See the migration table in `docs/hooks/api-reference.md` (#2624).
- **Telemetry `finish_reason` values are now canonical (#2624):** streaming telemetry records `finish_reason` using the fixed vocabulary `stop|max_tokens|tool_calls|safety|refusal|error|other` instead of raw provider-native values (`STOP`, `end_turn`, `length`, ...); the provider-native value is retained in `raw_stop_reason`. Log consumers matching on provider-native finish/stop strings must switch to the canonical values (or read `raw_stop_reason`).
