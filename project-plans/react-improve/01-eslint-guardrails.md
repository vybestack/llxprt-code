...
> **STOP.** Complete the tasks in *this* file, tick every checkbox in **Checklist (implementer)**, write the required progress report, then stop. Do **not** continue to later phases.
...
- **reports/react-improve/phase01-worker.md** – markdown report written by the worker *at startup*, updated on every action/error/finding, and ending with a final line `### DONE` when finished.

## Checklist (implementer)
- [x] Added **eslint-plugin-react-hooks** to devDependencies in *package.json*.
- [x] Updated *eslint.config.js* to include `plugin:react-hooks/recommended`.
