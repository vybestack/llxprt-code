...
| PR OCR finding 11 | `sendRefreshResult` can return a provider refresh token because it does not sanitize again at socket write time. | Reject | `RefreshCoordinator` parses the provider result, merges it host-side, then validates `sanitizeTokenForProxy(merged)` with `SanitizedOAuthTokenSchema`. Its success union exposes only `SanitizedOAuthToken`, and refresh tests prove the returned token omits `refresh_token`. Re-sanitizing the trusted result at serialization would duplicate the boundary parse. |
| PR OCR finding 12 | Wrong-typed request fields produce existing generic INVALID_REQUEST messages that can describe the field as missing. | Reject | Preserving the operation-specific error text is an explicit compatibility requirement. The tests assert the existing protocol message while proving wrong-typed fields are rejected before store access. |
| PR OCR finding 13 | A duplicate inline report recommends field-specific Zod error messages or dropping the message assertions. | Reject | This duplicates finding 12. Changing or weakening those assertions would stop enforcing the accepted error-message compatibility behavior. |
| PR OCR finding 14 | The scalar bucket-stats test should expect `PROXY_PAYLOAD_ERROR` instead of the malformed correlated-frame error. | Reject | Scalar `data` violates the response envelope before operation parsing, so the socket client must report `Malformed response for request <id>` and reset. `PROXY_PAYLOAD_ERROR` applies to malformed operation data inside a valid envelope. |
| PR OCR finding 15 | The newly added host refresh-token preservation test duplicates an existing stronger test. | In-scope-Fix | Removed the duplicate. The existing test proves the new access token is stored, the host refresh token remains, and the caller refresh token is rejected. |
| CodeRabbit pre-merge warning 16 | Touched-function docstring coverage is below CodeRabbit's configured threshold. | Reject | The repository does not require docstrings on these small boundary handlers, and adding comments solely to satisfy this external metric would conflict with the project's comment guidance. CodeRabbit generated no actionable code comments and its check passed. |
| PR walkthrough note 17 | `credential-store-factory.ts` was named in the issue but has no diff. | Reject | The file was audited and remains byte-identical to `origin/main` because it contains no applicable untrusted proxy-payload cast to replace. |
| PR OCR warning 18 | Changed-file preview coverage was below the OCR threshold and one file read failed. | Reject | This is review-tool execution metadata rather than a code finding. The OCR workflow completed successfully, three inline reports were triaged, and CodeRabbit independently reviewed all 20 eligible source and test files with no actionable comments. |
| PR OCR finding 19 | `handleSaveToken` should ignore an invalid existing stored token and merge the inbound token against an empty object. | Reject | `TokenStore.getToken` returns `OAuthToken | null`; `undefined` is outside that interface. A non-null malformed value is corrupted storage and must fail validation rather than silently discard the host refresh token. Falling back to the inbound sanitized token would hide the corruption and weaken the trust boundary this issue adds. |
| PR OCR finding 20 | Socket-client tests should avoid asserting the exact malformed-handshake and malformed-response messages. | Reject | Those two messages are part of the accepted compatibility behavior. Exact assertions verify both the handshake path and the correlated-frame reset path keep their established errors. |
| PR OCR finding 21 | Request validation should replace the existing `Missing provider` and `Missing name` text with field-specific Zod messages. | Reject | This duplicates findings 12 and 13. Exact operation-level error text must remain compatible while wrong-typed fields are rejected before store access. |
| PR OCR finding 22 | `OAuthExchangeRequestSchema` makes a request missing only `code` report `Missing session_id`. | Reject | The handler inspects the Zod issue paths. A valid `session_id` with a missing or invalid `code` has no session issue and returns `Missing code`; both missing fields continue to return `Missing session_id`, matching `origin/main`. Existing socket tests cover the missing-code path. |
| PR OCR finding 23 | Refresh retries should replace `currentToken` with a provider result that fails parsing or merging. | Reject | A malformed provider result is untrusted and cannot become the input to a later refresh attempt. The existing retry contract retries non-auth failures against the last stored token; parsing happens before persistence and cooldown mutation, and tests prove malformed results set no cooldown. |
| PR OCR finding 24 | `handleListProviders` and `handleListApiKeys` should return the result of their final `sendOk` calls for consistency. | Reject | Both calls are terminal statements and `sendOk` returns `void`, so adding `return` changes no control flow or behavior. |
| PR OCR finding 25 | Polling marks an OAuth session used before the provider poll succeeds. | Reject | `session.used` is assigned only after `pollForToken` resolves. The assignment before parse and persistence matches `origin/main` and prevents reuse of a one-shot provider result if later local processing fails. Changing OAuth session lifecycle is outside payload-validation scope. |
| PR OCR warning 26 | Changed-file preview coverage remained below the OCR threshold. | Reject | This repeats finding 18 and is review-tool execution metadata, not a code finding. CI completed all selected tests and review workflows successfully. |
