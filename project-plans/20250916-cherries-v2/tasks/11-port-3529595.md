# Task 11 – High-Risk PORT 3529595e

**Task Type:** High-Risk PORT\n**Area:** auth\n**Risk Level:** high\n**Results File:** `project-plans/20250916-cherries-v2/results/task-11.md`\n
## Commit Details
- Upstream hash: `3529595e`
- Description: Fix permissions for oauth_creds.json
- Rationale: OAuth file permissions security (add platform-aware chmod)

## Execution Steps
1. Cherry-pick the commit with metadata preserved:
   ```bash
   git cherry-pick -x 3529595e
   ```
...
3. Capture upstream and local diffs in `project-plans/20250916-cherries-v2/results/task-11.md` using the template.
4. Run the automated quality gate (fails stop progress):
   ```bash
   ./project-plans/20250916-cherries-v2/run-quality-gates.sh project-plans/20250916-cherries-v2/results/task-11.md
   ```
5. Address any failures, update the results file, and rerun until the script passes.
- Have another engineer review the diff before moving on.
...
7. Ask Claude to run the Codex audit (10-minute shell timeout):
   ...
   codex exec "Evaluate the cherry-pick task results in project-plans/20250916-cherries-v2/results/task-11.md. Check:
     ...
     3. Did the quality gate script pass? Check the log file at project-plans/20250916-cherries-v2/.quality-logs/task-11 for any failures.
