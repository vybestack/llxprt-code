# Task 24 – Batch Picks (5 commits)

**Task Type:** Batch PICK\n**Commit Count:** 5\n**Results File:** `project-plans/20250916-cherries-v2/results/task-24.md`\n
## Commits
- `c9bd3ecf` — Prevent race condition when diff accepted through CLI (ide, risk: medium)
- `5c2bb990` — Prevent crash when processing malformed file paths (tools, risk: low)
- `93ec574f` — Fix gemini-cli-vscode-ide-companion's package script (build, risk: low)
- `abddd2b6` — Handle nested gitignore files (tools, risk: low)
- `2782af3f` — Refactor a2a-server src directory (core, risk: low)

## Execution Steps
1. Ensure you are on the cherry-pick branch for this cycle.
2. Cherry-pick the commits in the exact order shown:
   ```bash
git cherry-pick -x c9bd3ecf
git cherry-pick -x 5c2bb990
git cherry-pick -x 93ec574f
git cherry-pick -x abddd2b6
git cherry-pick -x 2782af3f
   ```
...
4. Copy `RESULTS-TEMPLATE.md` to `project-plans/20250916-cherries-v2/results/task-24.md` (or update it if it already exists) and fill in every section before running validation.
5. Run the automated quality gate and stop immediately on failure:
   ```bash
   ./project-plans/20250916-cherries-v2/run-quality-gates.sh project-plans/20250916-cherries-v2/results/task-24.md
   ```
...
8. Ask Claude to run the Codex audit (10-minute shell timeout):
   ...
   codex exec "Evaluate the cherry-pick task results in project-plans/20250916-cherries-v2/results/task-24.md. Check:
     ...
     3. Did the quality gate script pass? Check the log file at project-plans/20250916-cherries-v2/.quality-logs/task-24 for any failures.
