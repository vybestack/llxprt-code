# Task 20 – Batch Picks (2 commits)

**Task Type:** Batch PICK\n**Commit Count:** 2\n**Results File:** `project-plans/20250916-cherries-v2/results/task-20.md`\n
## Commits
- `c29e4484` — Add highlights for input /commands and @file/paths (ui, risk: low)
- `997136ae` — Enable citations by default for certain users (ui, risk: low)

## Execution Steps
1. Ensure you are on the cherry-pick branch for this cycle.
2. Cherry-pick the commits in the exact order shown:
   ```bash
git cherry-pick -x c29e4484
git cherry-pick -x 997136ae
   ```
...
4. Copy `RESULTS-TEMPLATE.md` to `project-plans/20250916-cherries-v2/results/task-20.md` (or update it if it already exists) and fill in every section before running validation.
5. Run the automated quality gate and stop immediately on failure:
   ```bash
   ./project-plans/20250916-cherries-v2/run-quality-gates.sh project-plans/20250916-cherries-v2/results/task-20.md
   ```
...
8. Ask Claude to run the Codex audit (10-minute shell timeout):
   ...
   codex exec "Evaluate the cherry-pick task results in project-plans/20250916-cherries-v2/results/task-20.md. Check:
     1. Were ALL commits listed in the task file actually cherry-picked? Use \"git log --oneline -n 2\" to verify. List any missing commits.
     2. Review the actual code changes with \"git diff HEAD~2\" - do they match what the task intended? Check for:
     ...
     3. Did the quality gate script pass? Check the log file at project-plans/20250916-cherries-v2/.quality-logs/task-20 for any failures.
