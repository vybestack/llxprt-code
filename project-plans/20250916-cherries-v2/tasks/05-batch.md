# Task 05 – Batch Picks (5 commits)

**Task Type:** Batch PICK\n**Commit Count:** 5\n**Results File:** `project-plans/20250916-cherries-v2/results/task-05.md`\n
## Commits
- `1fc1c2b4` — Settings in Folder trust hook (trust, risk: low)
- `2fc85709` — Refuse to load extensions from untrusted workspaces (trust, risk: low)
- `fe5bb669` — Screen reader updates (ui, risk: low)
- `10c6af7e` — Disable commands from untrusted directories (trust, risk: low)
- `f00cf42f` — Update documentation for settings structure (docs, risk: low)

## Execution Steps
1. Ensure you are on the cherry-pick branch for this cycle.
2. Cherry-pick the commits in the exact order shown:
   ```bash
git cherry-pick -x 1fc1c2b4
git cherry-pick -x 2fc85709
git cherry-pick -x fe5bb669
git cherry-pick -x 10c6af7e
git cherry-pick -x f00cf42f
   ```
...
4. Copy `RESULTS-TEMPLATE.md` to `project-plans/20250916-cherries-v2/results/task-05.md` (or update it if it already exists) and fill in every section before running validation.
5. Run the automated quality gate and stop immediately on failure:
   ```bash
   ./project-plans/20250916-cherries-v2/run-quality-gates.sh project-plans/20250916-cherries-v2/results/task-05.md
   ```
...
8. Ask Claude to run the Codex audit (10-minute shell timeout):
   ...
   codex exec "Evaluate the cherry-pick task results in project-plans/20250916-cherries-v2/results/task-05.md. Check:
     ...
     3. Did the quality gate script pass? Check the log file at project-plans/20250916-cherries-v2/.quality-logs/task-05 for any failures.
