# Phase 15a: CredentialProxyServer Stub — Verification

## Phase ID
`PLAN-20250214-CREDPROXY.P15a`
...
- [ ] `packages/cli/src/auth/proxy/credential-proxy-server.ts` exists
- [ ] Plan markers `@plan:PLAN-20250214-CREDPROXY.P15` present
- [ ] `npm run typecheck` passes

## Semantic Verification
- [ ] `CredentialProxyServer` class exported
- [ ] Constructor accepts `CredentialProxyServerOptions` (tokenStore, keyStorage, providers, flowFactories, allowedProviders, allowedBuckets)
- [ ] `start()`, `stop()`, `getSocketPath()` methods exist
- [ ] Handler methods exist: `handleGetToken`, `handleSaveToken`, `handleRemoveToken`, `handleListProviders`, `handleListBuckets`, `handleGetApiKey`, `handleListApiKeys`
...
2. **Are the exports correct?** — Read actual export statements, verify match to pseudocode 005
3. **Is the constructor contract correct?** — Verify it matches the `CredentialProxyServerOptions` interface
...
- What is the type surface area? (exported types and classes)
- Verdict: PASS/FAIL
...
Create: `project-plans/issue1358_1359_1360/.completed/P15a.md`
