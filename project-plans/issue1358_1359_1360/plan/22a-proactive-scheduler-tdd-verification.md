# Phase 22a: ProactiveScheduler TDD — Verification
...
- [ ] `packages/cli/src/auth/proxy/__tests__/proactive-scheduler.test.ts` exists with 15+ tests
- [ ] Plan markers `@plan:PLAN-20250214-CREDPROXY.P22` present
- [ ] Requirement markers (`@requirement`) present for R16
...
1. **Do tests verify timer scheduling?** — Lead time, jitter, immediate for expired
2. **Do tests verify re-check logic?** — Skip refresh when token already valid
3. **Do tests verify failure handling?** — Backoff, cap, max failures
4. **Do tests verify cleanup?** — cancelAll, cancelForKey
...
Create: `project-plans/issue1358_1359_1360/.completed/P22a.md`
