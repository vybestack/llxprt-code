# Phase 16a: CredentialProxyServer TDD — Verification

## Phase ID
`PLAN-20250214-CREDPROXY.P16a`
...
- [ ] `packages/cli/src/auth/proxy/__tests__/credential-proxy-server.test.ts` exists with 20+ tests
- [ ] Plan markers `@plan:PLAN-20250214-CREDPROXY.P16` present
- [ ] Requirement markers (`@requirement`) present for R3, R4, R6, R7, R8, R10, R21, R22, R25
...
1. **Would these tests fail if the implementation was wrong?** — Each test verifies actual data transformation or error behavior
2. **Do tests cover all requirement behaviors?** — Socket lifecycle (R3, R25), handshake (R6), schema validation (R7), token operations (R8), sanitization (R10), profile scoping (R21), rate limiting (R22)
3. **Are edge cases covered?** — Stale socket cleanup, best-effort remove_token, empty arrays on error, missing fields, provider-specific passthrough fields
4. **Do tests verify the security invariant?** — refresh_token NEVER present in any response crossing the socket
...
Create: `project-plans/issue1358_1359_1360/.completed/P16a.md`
