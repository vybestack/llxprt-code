...
- `git` command failure is treated as "not dirty" (status error → no star; branch failure → hides branch). This is the issue's error contract.
- A `git` process that hangs with no callback leaves the refresh queued rather than overlapping. Out of scope: the previous hook had no command timeout either; a live refresh just waits for git. No watchdog added (periodic poll cannot recover a hung process; treat as external-input edge).
