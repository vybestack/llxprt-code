...
Write **behavioral tests FIRST** for `handleOAuthExchange` that:
1. Verify token is stored in backingStore after exchange
2. Verify refresh_token is stripped from response but preserved in store
3. Verify session is consumed (single-use)
...
- Phase 03 completed (handleOAuthInitiate implemented)
- Phase 03a verification passed

---

## Test File

**Create**: `packages/cli/src/auth/proxy/__tests__/oauth-exchange.spec.ts`
...
class TestOAuthFlow {
  ...
  constructor(flowType: 'pkce_redirect' | 'device_code') {
    ...
    this.initiateResult = {
      ...
      verification_uri_complete: 'https://example-provider.com/auth?challenge=xyz',
      expires_in: 1800,
      interval: 5,
    };
  }
...
describe('oauth_exchange handler', () => {
  ...
  describe('token storage', () => {
    ...
    it('stores token with correct provider and bucket', async () => {
      ...
      const defaultBucket = await backingStore.getToken('anthropic', 'default');
  ...
  describe('token sanitization', () => {
    ...
    it('preserves provider-specific fields in backing store', async () => {
      ...
      const stored = await backingStore.getToken('anthropic');
      expect((stored as unknown as Record<string, unknown>).account_id).toBe('acct_12345');
    });
  });
  ...
  describe('session lifecycle', () => {
    ...
    it('expired session returns SESSION_EXPIRED', async () => {
      ...
      server = new CredentialProxyServer({
        tokenStore: backingStore,
        providerKeyStorage: keyStorage,
        flowFactories: new Map([['anthropic', () => testFlow]]) as unknown as Map<string, () => unknown>,
      ...
      expect(exchange.ok).toBe(false);
      expect(exchange.code).toBe('SESSION_EXPIRED');
    });
  });
  ...
  describe('provider integration', () => {
    it('calls flow.exchangeCodeForToken with code', async () => {
      ...
      expect(testFlow.getLastExchangeParams().code).toBe('the_authorization_code');
    });
  ...
  describe('argument capture verification', () => {
    ...
    it('exchange passes correct state to provider (for PKCE flows)', async () => {
      ...
      await client.request('oauth_exchange', {
        session_id: init.data.session_id,
        code: 'auth_code',
        state: init.data.state, // Pass state from initiate (if applicable)
      });
...
# Expected: FAIL (handleOAuthExchange is NOT_IMPLEMENTED)
```
...
2. [x] Tests verify backingStore state (not just response)
3. [x] Tests verify refresh_token stripped from response
4. [x] Tests verify refresh_token preserved in backingStore
5. [x] Tests verify session single-use
6. [x] Tests FAIL against NOT_IMPLEMENTED stub
7. [x] No mock theater
...
Create: `project-plans/issue1358_1359_1360_remediation/.completed/P04.md`
