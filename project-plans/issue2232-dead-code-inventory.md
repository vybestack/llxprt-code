...
| cli | dependency-only | `gradient-string` in root `package.json` and `packages/cli/package.json`; root CLI-family declaration of `@xterm/headless` | no source import of `gradient-string`; no CLI import of `@xterm/headless`; core imports `@xterm/headless` through its own declaration | none attributable to these declarations | no source, script, bundle, metadata, main, bin, or export target reaches either `gradient-string` declaration or the root `@xterm/headless` declaration | root and CLI manifests share the CLI package name; ownership must be validated at each declaration before removal | high | executed by #3295: root and cli `gradient-string` both removed on mainline by 0cfb65fc5 (#2235, PR #3674; the #2236 slice's identical cli removal no-op'd at its later merge); root `@xterm/headless` retained after the 92cd92e1a restore (packed-layout resolution owner for the shipped `packages/core/src` import, issue #3055 rule) | #3295 |
...
| providers | dependency-only | `@ai-sdk/provider-utils` | none in production source | used by a providers test | not exported | move applies only if the test remains | high | executed by #3295: retained in devDependencies; sole importer is `src/openai-vercel/messageConversion.test.ts` (test-only), verified live | #3295 |
...
| tools | dependency-only | `html-to-text` | none in tools source | none | none | none found | high | executed by #3295: superseded by production importer `src/tools/direct-web-fetch.ts` (import added by 757b65451, #2760); retained as a tools dependency | #3295 |
...
| auth (`@vybestack/llxprt-code-auth`) | dependency-only | `fast-check` | none | no auth source or test import | already declared in devDependencies; no script, export, build, or metadata ownership | none found | high | executed by #3295: removed by 61214722a (#2234); no auth source or test reference remains | #3295 |
...
| ide-integration (`@vybestack/llxprt-code-ide-integration`) | dependency-only | `fast-check` | none | no ide-integration source or test import | already declared in devDependencies; no script, export, build, or metadata ownership | none found | high | executed by #3295: removed by 0cfb65fc5 (#2235); no ide-integration source or test reference remains | #3295 |
...
| policy (`@vybestack/llxprt-code-policy`) | dependency-only | `fast-check` | none | no policy source or test import | already declared in devDependencies; no script, export, build, or metadata ownership | none found | high | executed by #3295: removed by 61214722a (#2234); no policy source or test reference remains | #3295 |
...
| test-utils (`@vybestack/llxprt-code-test-utils`) | dependency-only | `@vybestack/llxprt-code-storage` | none in test-utils source | none | none | none found | high | executed by #3295: removed by 0cfb65fc5 (#2235); no test-utils reference remains | #3295 |
...
| vscode-ide-companion | dependency-only | `npm-run-all` | none | none | package declares `npm-run-all`, but the watch script invokes `npm-run-all2` | script and declaration mismatch requires an ownership decision | high | executed by #3295: decision settled by 0cfb65fc5 (#2235), `npm-run-all` removed and `npm-run-all2@^8.0.4` declared matching the watch script; binary resolution revalidated | #3295 |
...
Final outcomes (#3295): every row above with subissue #3295 was re-verified
against current source on branch `issue3295` and carries its executed
disposition (removed by the named commit, superseded by a production importer,
or retained with the ownership reason recorded in the cell). No still-stale
declaration was found. Fresh evidence, the boundary-checklist commands, and
the verification results are in
`project-plans/issue3295-dependency-reconciliation.md`.
