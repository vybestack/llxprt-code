...
Plan ID: PLAN-20260808-ISSUE2615.P10
Requirement: REQ-001

## Goal

Zero production files in `packages/mcp-and-a2a` importing the `Config` type.
Approximately 2 files at plan time; use the P04 guard for the live list.
...
- `cd packages/mcp-and-a2a && npm test` — zero non-timeout failures
...
- Guard reports 0 production `Config` holders in `packages/mcp-and-a2a`
