# P08 — Migrate providers

Plan ID: PLAN-20260808-ISSUE2615.P08
Requirement: REQ-001

## Goal

Zero production files in `packages/providers` importing the `Config` type.
Approximately 22 files at plan time; use the P04 guard for the live list.
...
- `cd packages/providers && npm test` — zero non-timeout failures
...
- Guard reports 0 production `Config` holders in `packages/providers`
