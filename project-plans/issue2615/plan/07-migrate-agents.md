...
Plan ID: PLAN-20260808-ISSUE2615.P07
Requirement: REQ-001

## Goal

Zero production files in `packages/agents` importing the `Config` type.
Approximately 53 files at plan time; use the P04 guard for the live list.
...
- `cd packages/agents && npm test` — zero non-timeout failures
...
- Guard reports 0 production `Config` holders in `packages/agents`
