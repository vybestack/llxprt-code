...
| 3d CodeRabbit (PR) | P3d | done | 3 findings, all verified real and fixed: (1) Major — append-time migration retry reloaded entries but kept the init-time messageId → duplicate (sessionId,messageId); extracted `_recomputeMessageId()`, called after reload and in initialize; 002b now asserts id sequence 0,1,2 + pair uniqueness. (2) Major — close() did not drain an in-flight initialize() → late commit could resurrect a closed logger; initialize now tracks `_initPromise` (also dedupes concurrent callers), close() awaits it before clearing; REQ-LOCK-009 regression. (3) Minor — test REPO_ROOT was 3 levels up instead of 4 → driver written to stray `packages/tmp/`; fixed path, removed stray dir. |
