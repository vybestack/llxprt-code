...
| F21 | CodeRabbit PR thread 1 (OpenAIProvider.ts continuation): custom `reasoning.fieldName` is not propagated into continuation classification; `classifyContinuationDelta` called `parseStreamingReasoningDelta(delta, logger)` with no field name, so custom-field reasoning-only continuation deltas classified as non-token-bearing and never fired `onRawTokenDelta`, freezing `last_token_ms` at the primary fragment and shortening the window (TPS inflated) | Verified in source: OpenAIProvider.ts `classifyContinuationDelta` vs `parseStreamingReasoningDelta(delta, logger, fieldName?)` (OpenAIResponseParser.ts L427-463, which honors fieldName via `resolveReasoningField`); primary path passes `deps.reasoningFieldName` (from `options.settings.get('reasoning.fieldName')`, OpenAIProvider.ts L801/L817/L597) but the continuation did not. RED: `tmp/verify3473/pr-remediation/red-run.log` CT-5 got 1 raw-delta callback (expected 3) with `reasoning.fieldName = custom_thoughts` | In-scope-Fix | `requestContinuationAfterToolCalls` and the `requestContinuation` callback type (both inline declarations in OpenAIStreamProcessor.ts) gained `reasoningFieldName?: string`, threaded from `deps.reasoningFieldName` in `handleToolCallsWithoutText`; `classifyContinuationDelta` passes it to `parseStreamingReasoningDelta`. CT-5 GREEN in `tmp/verify3473/pr-remediation/green-run.log` (3 callbacks, visible output unchanged: no text, 1 tool call) |
| F22 | CodeRabbit PR thread 2 (OpenAIStreamProcessor.ts L267/L360): a raw OpenAI choice carrying several token-bearing fields emitted several raw timing signals; `processStreamingChunk` fired once per payload kind (`processReasoningDelta` for the reasoning block and again for reasoning-embedded tool calls, `handleTextDelta` for content, `processDeltaToolCalls` for fragments), so a choice with `reasoning_content` + `content` fired twice and a Kimi reasoning delta with an embedded tool-call section fired twice within `processReasoningDelta` | Verified in source: three independent `deps.onRawTokenDelta?.()` sites all reachable for one choice in `processStreamingChunk`. The thread's CT-4 reference is inaccurate (the continuation path already fired once per chunk via `classifyContinuationDelta`, and CT-4 passed at 4), but the primary-path defect is real and violates the per-delta exactly-once rule stated in AC-4 and the PR body. RED: `tmp/verify3473/pr-remediation/red-run.log` DT-3 got 6 signals for 3 mixed choices (expected 3) | In-scope-Fix | `createPerChoiceNotifier` latches the hook per choice: `processStreamingChunk` creates it after choice extraction and threads it into `processReasoningDelta`, `handleTextDelta`, and `processDeltaToolCalls`, so each token-bearing choice stamps exactly once; the first fire still precedes the first visible yield (ordering unchanged). DT-3 GREEN in `tmp/verify3473/pr-remediation/green-run.log` (3 signals; text output still `Answer.Running`) |
| F23 | PR OCR thread 3 (OpenAIProvider.ts classifyContinuationDelta): `reasoningToolCalls` may be undefined and `.length` may throw | Refuted by parser source and type: `parseStreamingReasoningDelta` returns `{ thinking: ThinkingBlock \| null; toolCalls: ToolCallBlock[] }` with `toolCalls` an array on every return path (null delta → `[]`, uncaptured field → `[]`, otherwise `extractKimiToolCallsFromText`, which itself always returns `toolCalls: ToolCallBlock[]` including its empty-input early return); `processReasoningDelta` performs the identical destructure + `.length > 0` check on the same parser since the original implementation | Reject | No guard added: a `?? []` fallback would be a speculative guard for an impossible parser shape; the parser's return type is the source of truth and any shape change would fail typecheck at both call sites |
| F24 | PR OCR thread 4 (attemptRecorder.ts recordTimingOnly): `firstTokenMs ??=` can retain an earlier visible-chunk stamp instead of the raw-delta stamp when a visible chunk arrives before the first raw-delta hook | Unreachable ordering, verified in source: every visible token-bearing yield in the classic openai streaming path is emitted strictly after the choice's raw fire (`handleTextDelta` fires before its yield in the immediate branch and before buffering in the buffered branch; terminal and flush emissions are downstream of earlier raw fires; continuation classification fires before its yield); the non-streaming branch receives no `onRawTokenDelta` and never triggers continuations (continuation is wired only inside `processStreamingResponse`), so no mixed lifecycle exists; providers that never resolve a notifier never call `recordTimingOnly`. Therefore the only pre-set `firstTokenMs` observable by `recordTimingOnly` is the FIRST raw delta's own stamp, and `??=` preserves it, which is exactly AC-4; unconditional assignment would overwrite the first raw stamp with a later one, shortening the window and inflating TPS (the bug class this issue fixes). DT-1/DT-2 and CT-1..CT-5 already prove raw-authoritative stamps end to end | Reject | No change; the ordering proof above is the evidence. F22's latch preserves fire-before-yield ordering, so the proof remains valid post-remediation |
| F25 | PR OCR thread 5 (OpenAIProvider.continuationTiming.test.ts): the file-scope `vi.mock('openai')` module mock can leak to other test files | Mechanism real in a bare bun process, but neutralized by the repo's canonical runner: `scripts/run_bun_tests.ts` (the `test` script for every workspace) documents "Bun's module mocks are process-wide, unlike Vitest's per-file module graph. A fresh process per file preserves the isolation expected by the existing workspace suites", so each test file executes in its own process and the mock cannot reach another file. The file-scope `vi.mock` is also the uniform repo convention (298 `vi.mock`/`mock.module` sites; ≥15 files mock `openai` itself, including the cited `OpenAIProvider.e2e.test.ts` precedent). Empirical cross-check: running the openai suites in one shared process (`bun test packages/providers/src/openai/ ...`) produced 361 failures identically on the unmodified tree and the remediated tree (`tmp/verify3473/pr-remediation/baseline-openai-suite-stashed.log` vs `green-openai-suite.log`), i.e. a runner-isolation artifact predating this PR, while the canonical per-file runner is green | Reject | No change to the test; the suggested `vi.unmock`/`vi.resetModules` teardown is Vitest-scoped semantics, unnecessary under the canonical per-file-process runner, and changing one file would deviate from the repo-wide convention. Any further hardening belongs to test-infrastructure work, not this issue |

## PR #3494 review-thread remediation (post-PR follow-up)

Five actionable review threads on PR #3494 were triaged against source (F21-F25
above): two In-scope-Fix (F21 custom `reasoning.fieldName` continuation
propagation, F22 per-choice raw-timing deduplication), three Reject (F23
impossible parser shape, F24 unreachable ordering where the proposed fix would
regress AC-4, F25 module-mock leak neutralized by the canonical per-file test
runner). No Blocker-Fix and no Defer came out of the PR threads.

New behavioral tests (RED first, logs under `tmp/verify3473/pr-remediation/`):

- CT-5 in `OpenAIProvider.continuationTiming.test.ts`: custom-field
  reasoning-only continuation deltas fire the notifier once per delta;
  visible output unchanged (F21).
- DT-3 in `OpenAIStreamProcessor.timing.test.ts`: a raw choice carrying
  several token-bearing fields (reasoning + content, content + tool-call
  fragment, reasoning with an embedded Kimi tool-call section) fires the
  notifier exactly once; per-delta text yields unchanged (F22).

RED: `red-run.log` (CT-5 got 1 of 3 callbacks; DT-3 got 6 signals for 3
choices). GREEN: `green-run.log` (13/13 across both files). The canonical
workspace suites, typecheck, format, build, lint, test-audit diff, and the
stepfun-37 smoke cover the rest of this remediation cycle.

Scoped lint then flagged `OpenAIStreamProcessor.ts` past the 800-line
`max-lines` budget (808 counted), so the F22 latch factory moved to
`OpenAIStreamProcessorState.ts` (`createPerChoiceNotifier`) and the twice
inlined `requestContinuation` callback type was de-duplicated into one named
`RequestContinuationFn`; `green-post-refactor.log` (23/23 across the two
remediation files plus both guard files) and the rerun suites confirm no
behavior change.
