...
- Evict oldest-first until BOTH caps satisfied; skip live-writer files; decrement
  ...
  + coarse interval. Claims counted in accounting but not parsed.

## Verify
- [ ] Constants derived from the P04 benchmark (D5); no placeholders.
- [ ] AC-7 evidenced; no instantaneous-cap assertion; live-writer never deleted.
- [ ] Claim files counted but not parsed as JSONL (D3).
- [ ] No `vi.mock('fs')`; real files + package-private port for fault injection (D6).
