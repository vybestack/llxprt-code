...
if (process.argv[2] === 'child') {
  ...
  let bytes = 0;
  const name = (): string =>
    `${DIR}/perf-2026080${day}-${pid}-${String(seq).padStart(3, '0')}.jsonl`;
  let target = name();
  for (let i = 0; i < N_LINES; i++) {
    ...
    if (bytes + Buffer.byteLength(line) > PER_FILE_CAP) {
      seq++;
      bytes = 0;
      target = name();
    }
...
await Promise.all(
  Array.from(
    ...
    () =>
      new Promise<void>((r) => {
        spawn(process.execPath, [self, 'child'], { stdio: 'ignore' }).on(
          'exit',
          () => r(),
        );
      }),
...
let records = 0;
let over = 0;
const writersPerFile = new Map<string, number>();
for (const f of files) {
  ...
  for (const l of fs.readFileSync(p, 'utf8').split('\n')) {
    ...
    records++;
    try {
      ws.add((JSON.parse(l) as { p: number }).p);
    } catch {
      records--;
    }
  }
...
console.log(
  `records readable: ${records}   LOST: ${N_PROC * N_LINES - records}`,
);
