...
const DIR = '/tmp/rot_race';
const ACTIVE = DIR + '/perf.jsonl';
const PREV = DIR + '/perf.1.jsonl';
...
if (process.argv[2] === 'child') {
  ...
  try {
    myBytes = fs.statSync(ACTIVE).size;
  } catch {}
...
await Promise.all(
  Array.from(
    ...
    (_, k) =>
      new Promise<void>((r) => {
        spawn(process.execPath, [self, 'child', String(k)], {
          stdio: 'ignore',
        }).on('exit', () => r());
      }),
  ),
...
const totalBytes = fs
  .readdirSync(DIR)
  .reduce((a, f) => a + fs.statSync(DIR + '/' + f).size, 0);
console.log(`cap per file: 1.00 MiB   intended ceiling: 2.00 MiB (2 files)`);
console.log(`files on disk: ${files.join('  ')}`);
console.log(
  `ACTUAL total bytes: ${(totalBytes / 1024 / 1024).toFixed(2)} MiB`,
);
console.log(
  `records written: ${written}   records still readable: ${surviving}   LOST: ${written - surviving}`,
);
console.log(
