  ...
  Array.from(
    ...
    (_, k) =>
      new Promise<void>((res) => {
        const c = spawn(process.execPath, [self, 'child', String(k)], {
          stdio: 'ignore',
        });
        c.on('exit', () => res());
      }),
...
for (const l of lines) {
  try {
    const o = JSON.parse(l) as { w: number };
    parsed++;
...
console.log(
  `processes: ${N_PROC}   lines each: ${N_LINES}   record size: ~${Buffer.byteLength(JSON.stringify({ w: 0, i: 0, pad: 'x'.repeat(600), ts: Date.now() })) + 1} B`,
);
console.log(
  `elapsed: ${dt} ms   file size: ${(fs.statSync(FILE).size / 1024 / 1024).toFixed(2)} MiB`,
);
...
console.log(
  `per-writer counts complete: ${[...perWriter.entries()].sort((a, b) => a[0] - b[0]).map(([w, n]) => `${w}:${n}`).join(' ')}`,
);
console.log(
