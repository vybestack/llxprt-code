...
const SRC = DIR + '/perf-20260807-4102-000.jsonl';
const GZ = SRC + '.gz';
...
function reset(): number {
  ...
  let s = '';
  for (let i = 0; i < 2000; i++) s += JSON.stringify({ turn: i }) + '\n';
  fs.writeFileSync(SRC, s);
  return 2000;
}
// clamp: mtime can lead Date.now() (APFS / NTP step)
const ageOf = (p: string): number =>
  Math.max(0, Date.now() - fs.statSync(p).mtimeMs);
function makeOld(p: string, hours: number): void {
  const t = (Date.now() - hours * 3600e3) / 1000;
  fs.utimesSync(p, t, t);
}
...
function compress(): string {
  ...
  const age = ageOf(SRC);
  if (age < GRACE)
    return `SKIP: only ${(age / 1000).toFixed(1)}s since last write (< 1h grace)`;
...
function readAll(): number {
  ...
  for (const f of fs.readdirSync(DIR)) {
    ...
    const p = DIR + '/' + f;
    const b = f.endsWith('.gz')
      ? zlib.gunzipSync(fs.readFileSync(p))
      : fs.readFileSync(p);
...
console.log(
  `     readable: ${readAll()} of ${total}   files: ${fs.readdirSync(DIR).join(', ')}`,
);
...
console.log(
  `  late record appended          -> readable ${readAll()} of ${total + 1}, files: ${fs.readdirSync(DIR).length}`,
);
console.log(`  pass 3 (archive exists)       -> ${compress()}`);
makeOld(SRC, 3);
console.log(`  pass 4 (stray now 3h idle)    -> ${compress()}`);
const final = readAll();
console.log(`     FINAL readable: ${final} of ${total + 1}   lost: ${total + 1 - final}`);
console.log(`  files: ${fs.readdirSync(DIR).join(', ')}`);
console.log(
  `\n  VERDICT: ${final === total + 1 ? 'no loss — archive never overwritten' : 'LOSS'}`,
);
fs.rmSync(DIR, { recursive: true, force: true });
