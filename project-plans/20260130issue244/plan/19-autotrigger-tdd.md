# Phase 19: Auto-Trigger Mechanism TDD
...
Tests for REQ-ASYNC-010 and REQ-ASYNC-011.
...
- `packages/core/src/services/asyncTaskAutoTrigger.test.ts`
  - MUST include: `@plan PLAN-20260130-ASYNCTASK.P19`
...
describe('AsyncTaskAutoTrigger', () => {
  describe('trigger conditions', () => {
    ...
    it('should auto-trigger when agent is idle', async () => {
      ...
      manager.completeTask('task-1', { emitted_vars: { key: 'value' }, terminate_reason: 'GOAL' });
  ...
  describe('serialization', () => {
    ...
    it('should serialize triggers - only one in flight', async () => {
      ...
      manager.registerTask({ id: 'task-1', subagentName: 'r1', goalPrompt: 'R1', abortController: new AbortController() });
      manager.registerTask({ id: 'task-2', subagentName: 'r2', goalPrompt: 'R2', abortController: new AbortController() });
      ...
      // Should only have triggered once (or twice if second queued after first completed)
  ...
  describe('notifiedAt timing', () => {
    ...
    it('should NOT mark notifiedAt if delivery fails', async () => {
      const triggerAgentTurn = vi.fn().mockRejectedValue(new Error('Delivery failed'));
      const manager = new AsyncTaskManager(5);
      const reminderService = new AsyncTaskReminderService(manager);
...
- [ ] >= 8 tests
- [ ] Tests cover trigger conditions
- [ ] Tests cover serialization
- [ ] Tests cover notifiedAt timing
- [ ] Tests FAIL with stub

## Phase Completion Marker

Create: `project-plans/20260130issue244/.completed/P19.md`
