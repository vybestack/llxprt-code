...
PLAN_DIR = ROOT / "project-plans/20251219gemerge"
...
def write_plan(batches: list[dict], missing_files: dict[str, list[str]]) -> None:
    ...
    lines.append(f"- {code('project-plans/20251219gemerge/CHERRIES.md')}")
    lines.append(f"- {code('project-plans/20251219gemerge/SUMMARY.md')}")
    ...
    for idx, batch in enumerate(batches, 1):
        ...
        if batch_type == "PICK":
            ...
            cmd = code(f"project-plans/20251219gemerge/{shas[0]}-plan.md")
...
def write_progress(batches: list[dict]) -> None:
    ...
    lines.append(f"- [ ] Branch exists: {code('git checkout -b 20251219gemerge')}")
...
def write_audit(commits: list[Commit], picks: set[str], reimpl: set[str]) -> None:
    ...
    lines.append("- Branch: `20251219gemerge`")
