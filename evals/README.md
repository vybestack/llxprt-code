...
# matches the nightly workflow). Sets RUN_EVALS=1 and runs the Bun
# orchestrator rooted at ./evals so report.json lands at
# evals/logs/report.json.
npm run test:all_evals

# Equivalent manual invocation (must keep --root evals and the
# --json-report path so the report resolves to evals/logs/report.json,
# which the nightly aggregation and artifact upload expect)
RUN_EVALS=1 bun scripts/run_bun_tests.ts --root evals \
  --json-report evals/logs/report.json
```
...
The nightly workflow (`.github/workflows/evals-nightly.yml`) runs evals multiple times and aggregates the results using `scripts/aggregate_evals.ts`. The aggregation script:

1. **Collects current run data**: Parses all `report.json` files from the current workflow run (generated by the Bun test orchestrator's `--json-report` output)
