/**
 * Unit tests for pure functions in scripts/tmux-harness.ts.
 *
...
describe('buildStartArgs', () => {
  it('returns the default command when no script command is provided', async () => {
    ...
    expect(buildStartArgs(undefined, false)).toEqual([
      'bun',
      'scripts/start.ts',
    ]);
  });
...
describe('matchText', () => {
  ...
  it('resets stateful regex matchers before each test', async () => {
    const { matchText } = await importHarness();
    const matcher: { kind: 'regex'; value: RegExp } = {
      kind: 'regex',
      value: /hello/g,
    };
