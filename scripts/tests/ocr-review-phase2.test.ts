...
describe('.github/workflows/ocr-review.yml — Phase 2 inline comment sorting, capping, and fingerprints (#2649)', () => {
   ...
   */
  let cachedSeverityRank: ((severity: string | undefined) => number) | null =
    null;
  let cachedSortInlineComments:
    | ((
        pairs: Array<{
          comment: {
            path: string;
  ...
  function loadFunctions() {
    ...
    const severityFn = asVmFunction(exposed['severityRank']);
    cachedSeverityRank = (severity: string | undefined): number =>
      asNumber(severityFn(severity));
    const sortFn = asVmFunction(exposed['sortInlineComments']);
    cachedSortInlineComments = (
      pairs: Parameters<NonNullable<typeof cachedSortInlineComments>>[0],
    ): ReturnType<NonNullable<typeof cachedSortInlineComments>> => {
      const result = sortFn(pairs);
      if (!Array.isArray(result))
        throw new Error('sortInlineComments should return array');
      const arr: Array<{
        comment: {
          path: string;
          line: number;
      ...
      for (const item of result) {
        if (item === null || typeof item !== 'object' || Array.isArray(item)) {
          throw new Error('sortInlineComments returned a non-object item');
        }
        const obj = asRecord(item);
        const comment = asRecord(obj['comment']);
        arr.push({
          comment: {
            ...
            start_line:
              comment['start_line'] !== undefined
                ? asNumber(comment['start_line'])
                : undefined,
            end_line:
              comment['end_line'] !== undefined
                ? asNumber(comment['end_line'])
                : undefined,
            _severity:
              comment['_severity'] !== undefined
                ? String(comment['_severity'])
                : undefined,
          },
          finding: asRecord(obj['finding']),
        });
      }
      return arr;
    };
    const fingerprintFn = asVmFunction(exposed['findingFingerprint']);
    cachedFindingFingerprint = (
      finding: Record<string, unknown> | null,
      runId?: string,
      index?: number,
    ): string => asString(fingerprintFn(finding, runId, index));
    if (
      !cachedSeverityRank ||
      !cachedSortInlineComments ||
      !cachedFindingFingerprint
  ...
  describe('YAML wiring for Phase 2', () => {
    it('defines INLINE_COMMENT_CAP env var for the post step', () => {
      const env = asOptionalRecord(asOptionalRecord(postStep)?.env);
      expect(env?.OCR_INLINE_COMMENT_CAP).toBe(
