...
type FindingInput = Record<string, unknown> | string | unknown[] | null;
type Sandbox = Record<string, unknown> & {
  warnings: string[];
  stderrChunks: string[];
};

function asObjectResult(result: FindingInput): Record<string, unknown> {
  if (result === null || typeof result !== 'object' || Array.isArray(result)) {
    throw new Error('Expected validation result to be an object');
  }
  return result;
}

function narrowFindingResult(rawResult: unknown): FindingInput {
...
describe('.github/workflows/ocr-review.yml — metadata validation (issue #2671)', () => {
  ...
  function runValidation(
    ...
    overrides: { redactSecretDiagnostics?: (value: unknown) => string } = {},
  ) {
    ...
    const rawResult: unknown = sandbox['__RESULT__'];
    const result = narrowFindingResult(rawResult);
    return {
      result,
  ...
  /**
   * Variant of runValidation that narrows the result to an object for
   * tests that always pass object findings and access result fields.
   */
  function runObjectValidation(
    finding: Record<string, unknown>,
    overrides: { redactSecretDiagnostics?: (value: unknown) => string } = {},
  ): { result: Record<string, unknown>; warnings: string[]; stderr: string } {
    const { result, warnings, stderr } = runValidation(finding, overrides);
    return {
      result: asObjectResult(result),
      warnings,
      stderr,
    };
  }
  ...
  describe('validateFindingMetadata behavior', () => {
    it('passes valid category and severity through unchanged with no warnings (AC 1)', () => {
      ...
      const { result, warnings } = runObjectValidation(input);
    ...
    it('preserves originalCategory/originalSeverity unchanged when metadata is valid', () => {
      ...
      const { result } = runObjectValidation(input);
    ...
    it('does not suppress findings when core.warning throws (fail-open contract)', () => {
      ...
      const rawResultArr: unknown = sandbox['__RESULT__'];
      const resultArr: Array<Record<string, unknown>> = [];
      if (Array.isArray(rawResultArr)) {
        for (const item of rawResultArr) resultArr.push(asRecord(item));
      }
      expect(resultArr).toHaveLength(1);
      expect(resultArr[0]['category']).toBe('unknown');
    });
    ...
    it('routes the undocumented value through redactSecretDiagnostics in the warning (AC 5)', () => {
      const { warnings } = runValidation(
        ...
        {
          redactSecretDiagnostics: (value: unknown) =>
    ...
    it('redacts the entire composed warning (path and metadata) with the REAL redactor (AC 5 security)', () => {
      ...
      const realRedactorRaw = makePostSanitizer(postScript, secret);
      const redactorFn =
        typeof realRedactorRaw === 'object' &&
        realRedactorRaw !== null &&
        'redactSecretDiagnostics' in realRedactorRaw
          ? asRecord(realRedactorRaw)['redactSecretDiagnostics']
          : realRedactorRaw;
      if (typeof redactorFn !== 'function') {
        throw new Error('redactSecretDiagnostics should be a function');
      }
      const realRedactor = (value: unknown): string =>
        String(redactorFn(value));
      const input = {
  ...
  describe('validation-before-dedup pipeline ordering', () => {
    ...
    function runValidateThenDedup(
      ...
      const dedupedResult = asRecord(sandbox['__DEDUPED__']);
      const validatedRaw: unknown = sandbox['__VALIDATED__'];
      const validatedArr: Array<Record<string, unknown>> = [];
      if (Array.isArray(validatedRaw)) {
        for (const item of validatedRaw) {
          if (
            ...
            validatedArr.push(asRecord(item));
          }
        }
      }
      const dedupedRaw = dedupedResult['deduped'];
      const dedupedList: Array<Record<string, unknown>> = [];
      if (Array.isArray(dedupedRaw)) {
        for (const item of dedupedRaw) {
          if (
            ...
            dedupedList.push(asRecord(item));
          }
        }
      }
      return {
        validated: validatedArr,
        deduped: dedupedList,
        suppressed: asNumber(dedupedResult['suppressed']),
        warnings: sandbox.warnings,
      };
    }
