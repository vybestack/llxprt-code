...
} from '../aggregate-ocr-telemetry.ts';
...
describe('aggregate-ocr-telemetry.ts CLI — contract', () => {
  const REPO_ROOT = path.resolve(import.meta.dirname, '../..');
  const AGG_SCRIPT = path.join(
    REPO_ROOT,
    'scripts',
    'aggregate-ocr-telemetry.ts',
  );
  ...
  describe('aggregateTelemetry — mixed evidence and epoch ordering', () => {
    ...
    it('omits concurrency groups that have no wall-clock samples', () => {
      const result = aggregateTelemetry([record({ wall_clock_seconds: null })]);
      expect(result.average_wall_clock_by_concurrency['2']).toBeUndefined();
      expect(result.markdown).not.toContain('c=2:');
    });
  });
});
