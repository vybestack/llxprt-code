...
describe('H3: unassign-stale-issues.sh guard order', () => {
  it('empty-string GITHUB_REPOSITORY exits nonzero with clean message (not unbound-variable)', () => {
    ...
    try {
      ...
      const errRecord = asExecError(err);
      exitCode = errRecord.status ?? 1;
      stderr = errRecord.stderr?.toString() ?? '';
    }
 ...
 */
function setupConcurrencyRepo(initialState: Record<string, unknown>): {
  dir: string;
  stateFile: string;
  binDir: string;
  pathWithFakeGh: string;
} {
...
function spawnAssignProcess({
  ...
  return new Promise((resolve) => {
    ...
    // spawn() streams emit Buffer objects (encoding is only honored by
    // execFile/execFileSync, not spawn). Coerce to string explicitly.
...
async function runConcurrentAssign({
  ...
  results: Array<{ stdout: string; stderr: string; status: number }>;
