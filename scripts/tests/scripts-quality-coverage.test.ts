...
/**
 * Spawns `eslint --print-config` to read the effective rule set for a file.
 * Each invocation loads the full flat config, which is expensive; results are
 * memoized so each unique path is resolved at most once per test run.
 */
const rulesCache = new Map<string, Record<string, unknown>>();

function effectiveRulesFor(relativePath: string): Record<string, unknown> {
  const cached = rulesCache.get(relativePath);
  if (cached !== undefined) {
    return cached;
  }
  ...
  rulesCache.set(relativePath, rules);
  return rules;
}

// These tests spawn the ESLint CLI to inspect effective config. ESLint startup
// + flat-config loading can take several seconds on CI runners, so a generous
// per-test timeout is necessary.
...
describe('Issue #2282: scripts strict code-quality coverage', () => {
  ...
  it(
    'applies the same quality rules to a .cjs script',
    () => {
  ...
  it(
    'applies the same quality rules to a .ts script',
    () => {
  ...
  it(
    'turns off max-lines-per-function for scripts tests (parity with packages)',
    () => {
      const rules = effectiveRulesFor(
      ...
      expect(
        severity(rules['sonarjs/too-many-break-or-continue-in-loop']),
      ).toBe(2);
      expect(severity(rules['sonarjs/slow-regex'])).toBe(2);
      expect(severity(rules['sonarjs/no-collapsible-if'])).toBe(2);
    },
  ...
  it(
    ...
    () => {
      ...
      expect(
        severity(rules['@typescript-eslint/no-floating-promises']),
      ).not.toBe(2);
      expect(
