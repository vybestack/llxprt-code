...
describe('buildMapPrompt', () => {
  ...
  it('fails fast when required input is invalid', () => {
    ...
    expect(() => buildMapPrompt('a.ts', SAMPLE_DIFF, undefined)).toThrow(
      /prContext/,
    );
  });
});
...
describe('buildPreMergeChecksPrompt', () => {
  ...
  const build = (
    changeEvidence:
      | Array<{ filePath: string; summary: string; triage: string }>
      | undefined,
  ) =>
    buildPreMergeChecksPrompt(
      SAMPLE_PR_CONTEXT,
  ...
  it('includes the FULL issue body (no truncation)', () => {
    expect(build(undefined)).toContain(longBody);
  });
  ...
  it('asks to judge linked issues against acceptance criteria', () => {
    expect(build(undefined).toLowerCase()).toContain('acceptance criteria');
  });
  ...
  it('defaults change evidence to empty without crashing', () => {
    expect(build(undefined)).toContain('no per-file summaries available');
  });
