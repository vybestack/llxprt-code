...
describe('runLlxprtPromptWithParse', () => {
  ...
  it('preserves the immediate default retry behavior', async () => {
    ...
    try {
      let calls = 0;
      const resultPromise = runLlxprtPromptWithParse(
        async () => {
          calls += 1;
          return calls === 1
            ? 'not json'
...
describe('graceful degradation (issue #2742)', () => {
  it('mapWithConcurrency returns error objects when the mapper throws', async () => {
    ...
    const summaries = results.map((r: unknown) => {
      if (
        typeof r === 'object' &&
        r !== null &&
        'error' in r &&
        ...
        return {
          ...
          summary: `(failed: ${String(obj['error'])})`,
          signature: '',
          triage: 'chore',
        };
      }
    ...
    expect(first['triage']).toBe('chore');
    expect(String(first['summary'])).toContain('Cannot parse JSON');
  });
});
