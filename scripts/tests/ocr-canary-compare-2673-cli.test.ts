...
import { withTempDirectory } from './ocr-concurrency-canary-2673-helpers.ts';
...
describe('ocr-canary-compare-2673 CLI', () => {
  ...
  it('loads comparison logic from the production scripts boundary', () => {
    ...
    expect(source).toContain(
      "path.join(__dirname, 'lib', 'ocr-concurrency-canary-2673-comparator.ts')",
    );
  ...
  it('emits structured invalid JSON when an artifact cannot be read', () =>
    withTempDirectory(
      'ocr-compare-cli-read-error-2673-',
      (directory: string) => {
        const artifactPaths = writeArtifacts(directory);
