...
function getOn(workflow: WorkflowDocument): Record<string, unknown> {
...
describe('assign.yml workflow configuration', () => {
  ...
  it('workflow has jobs defined', () => {
    ...
    expect(getJob(workflow, 'assign'), 'assign job should exist').toBeDefined();
  });

  const job = getJob(workflow, 'assign');

  it('triggers on issue_comment created and issues assigned', () => {
    const on = getOn(workflow);
    const issueComment = asRecord(on['issue_comment']);
    const issues = asRecord(on['issues']);
    expect(issueComment?.['types']).toEqual(['created']);
    expect(issues?.['types']).toEqual(['assigned']);
  ...
  it('passes env vars and runs the script', () => {
    ...
    expect(env?.['ISSUE_NUMBER']).toBe('${{ github.event.issue.number }}');
    expect(env?.['COMMENTER_LOGIN']).toBe(
...
describe('assign.yml record-history job', () => {
  const workflow = parseWorkflowYaml(
    readRootFile('.github/workflows/assign.yml'),
  );

  it('workflow has a record-history job', () => {
    expect(workflow.jobs, 'workflow should have jobs').toBeDefined();
    expect(
      getJob(workflow, 'record-history'),
      'record-history job should exist',
    ).toBeDefined();
  });

  const job = getJob(workflow, 'record-history');
  ...
  it('validates login from event payload and delegates to record-history script', () => {
    ...
    const env = runStep?.env;
    expect(env?.['ASSIGNEE_LOGIN']).toBe('${{ github.event.assignee.login }}');
    const runText = normalize(String(runStep?.run ?? ''));
    ...
    expect(String(runStep?.run ?? '')).not.toMatch(/\/issues\/\d+\/labels/);
  });
...
describe('assign-stale-cleanup.yml workflow configuration', () => {
  ...
  it('workflow has a cleanup job', () => {
    expect(workflow.jobs, 'workflow should have jobs').toBeDefined();
    expect(
      getJob(workflow, 'cleanup'),
      'cleanup job should exist',
    ).toBeDefined();
  });

  const job = getJob(workflow, 'cleanup');
  ...
  it('runs on a daily schedule and workflow_dispatch', () => {
    ...
    if (Array.isArray(schedule)) {
      ...
      expect(first, 'schedule should have at least one entry').toBeDefined();
      const sched = asRecord(first);
      expect(sched['cron']).toBe('0 7 * * *');
    }
...
describe('assign-issue.sh behavioral', () => {
  it('assigns a user with a merged PR', () => {
    ...
    const feedbackComments = stateComments(result.state).filter(
      (c) =>
        c.issue_number === 42 &&
        c.user.login === 'github-actions[bot]' &&
        String(c.body).includes('<!-- llxprt-assign-feedback -->'),
    );
    expect(feedbackComments.length).toBeGreaterThanOrEqual(1);
    expect(String(feedbackComments[0].body)).toContain('Assigned');
  ...
  it('assigns a user in the assignment-history backfill file', () => {
    ...
    expect(result.status).toBe(0);
    const issue = stateIssue(result.state, 42);
    expect(issue._assignees).toContain('bob');
    expect(issue._label_names).toContain('auto-assigned');
  });
  ...
  it('refuses to assign when issue is already assigned', () => {
    ...
    expect(issue._assignees).toContain('someone-else');
    // Feedback posted
    const comments = stateComments(result.state).filter(
      (c) => c.issue_number === 42,
    );
    expect(
      comments.some((c) => String(c.body).includes('already assigned')),
    ).toBe(true);
  });
  ...
  it('refuses to assign when user is at the 3-issue cap', () => {
    ...
    expect(
      comments.some((c) => String(c.body).includes('open assigned issues')),
    ).toBe(true);
    expect(comments.some((c) => String(c.body).includes('maximum is'))).toBe(
      true,
    );
  });
  ...
  it('refuses ineligible user with no merged PR and no prior assignment', () => {
    ...
    expect(result.status).toBe(0);
    const issue = stateIssue(result.state, 42);
    expect(issue._assignees).not.toContain('newbie');
    expect(issue._label_names).not.toContain('auto-assigned');
    const comments = stateComments(result.state).filter(
      (c) => c.issue_number === 42,
    );
    expect(
      comments.some((c) => String(c.body).includes('eligibility requires')),
    ).toBe(true);
  });
  ...
  it('fails closed (nonzero, no mutation) when guard API fails', () => {
    ...
    const issue = stateIssue(result.state, 42);
    // No mutation
    expect(issue._assignees).toEqual([]);
    expect(issue._label_names).not.toContain('auto-assigned');
  });
  ...
  it('sticky feedback lookup only selects bot-authored marker comments', () => {
    ...
    const userComment = stateComments(result.state).find((c) => c.id === 999);
    expect(userComment).toBeDefined();
    expect(String(userComment?.body)).toBe(
      '<!-- llxprt-assign-feedback -->\nUser-injected marker',
    );
    // A NEW bot comment should be posted (not updating the user's)
    const botComments = stateComments(result.state).filter(
...
describe('unassign-stale-issues.sh behavioral', () => {
  ...
  it('never unassigns acoliver', () => {
    ...
    expect(result.status).toBe(0);
    const issue = stateIssue(result.state, 42);
    expect(issue._assignees).toContain('acoliver');
  });
  ...
  it('retains recent assignment (younger than 14 days)', () => {
    ...
    expect(result.status).toBe(0);
    const issue = stateIssue(result.state, 42);
    expect(issue._assignees).toContain('recent-user');
    expect(issue._label_names).toContain('auto-assigned');
  });
  ...
  it('retains assignment when a qualifying linked PR exists after assignment', () => {
    ...
    expect(result.status).toBe(0);
    const issue = stateIssue(result.state, 42);
    expect(issue._assignees).toContain('active-user');
    expect(issue._label_names).toContain('auto-assigned');
  });
  ...
  it('preserves state and reports nonzero when timeline query fails', () => {
    ...
    expect(issue._assignees).toContain('stale-user');
  });
