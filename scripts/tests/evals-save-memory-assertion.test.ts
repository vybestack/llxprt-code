...
describe('saveMemoryFactEquals predicate (deterministic exact-value)', () => {
  let saveMemoryFactEquals:
    | ((token: string) => (args: unknown) => boolean)
    | undefined;
  ...
  function getSaveMemoryFactEquals(): (
    token: string,
  ) => (args: unknown) => boolean {
    if (!saveMemoryFactEquals) {
      throw new Error('saveMemoryFactEquals should be defined');
    }
    return saveMemoryFactEquals;
  }

  it('accepts the canonical fact exactly', () => {
    const match = getSaveMemoryFactEquals()('blue');
...
describe('assertFavoriteColorBlueOutput predicate (deterministic exact-value)', () => {
  let assertFavoriteColorBlueOutput: (output: unknown) => void;
  ...
  it('rejects non-string input without throwing a TypeError', () => {
    expect(() => assertFavoriteColorBlueOutput(null)).toThrow(/some output/i);
    expect(() => assertFavoriteColorBlueOutput(undefined)).toThrow(
      /some output/i,
    );
