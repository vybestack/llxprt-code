...
describe('.github/workflows/ocr-review.yml — OCR telemetry (issue #2676)', () => {
  ...
  let telemetryStep: Record<string, unknown> | undefined;
  let telemetryRun: string;
  let placeholderStep: Record<string, unknown> | undefined;
  let placeholderRun: string;
  let hashStep: Record<string, unknown> | undefined;
  let _hashRun: string | string[];
  let uploadStep: Record<string, unknown> | undefined;
  let telemetryUploadStep: Record<string, unknown> | undefined;
  let wallClockStep: Record<string, unknown> | undefined;
  let _wallClockRun: string;
  let filesReviewedStep: Record<string, unknown> | undefined;
  let _filesReviewedRun: string;
  let finalClassificationStep: Record<string, unknown> | undefined;
  let _finalClassificationRun: string;

  beforeAll(() => {
    workflow = asRecord(yaml.load(readRootFile(WORKFLOW_PATH)));
  ...
  function stepNames(): string[] {
    return jobSteps().map((step) => asString(step.name));
  }
  ...
  describe('telemetry step runs always after classification', () => {
    ...
    it('uses the trusted scripts/ocr-telemetry.ts module', () => {
      expect(telemetryStep?.shell).toBe('bash');
      expect(telemetryRun).toContain('scripts/ocr-telemetry.ts');
    });
    ...
    it('does NOT use github.event.head_commit.timestamp (absent for non-push triggers)', () => {
      const env = getEnv(telemetryStep);
      expect(env?.OCR_GENERATED_AT).not.toBe(
    ...
    it('passes an authoritative workflow-measured wall-clock duration', () => {
      const env = getEnv(telemetryStep);
      expect(env?.OCR_WALL_CLOCK_SECONDS).toBe(
    ...
    it('passes the validated OCR result summary files_reviewed count', () => {
      const env = getEnv(telemetryStep);
      expect(env?.OCR_FILES_REVIEWED).toBe(
    ...
    it('passes the post-ocr-results lifecycle state', () => {
      const env = getEnv(telemetryStep);
      expect(env?.OCR_POST_STATE).toBe(
    ...
    it('passes the explicit prior-head dedup count as already_resolved', () => {
      const env = getEnv(telemetryStep);
      expect(env?.OCR_ALREADY_RESOLVED).toBe(
  ...
  describe('telemetry artifact lifecycle', () => {
    ...
    it('uploads telemetry independently from source diagnostic gating', () => {
      ...
      const withMap = getWith(telemetryUploadStep);
      expect(String(withMap?.name)).toContain('ocr-telemetry');
      expect(withMap?.path).toBe('ocr-telemetry.json');
  ...
  it('validates with the shared validator after telemetry redaction', () => {
    ...
    const validationName = asString(validationStep.name);
    const validationIndex = names.indexOf(validationName);
  ...
  it('publishes final job outputs from post-upload classification', () => {
    ...
    expect(String(outputs?.infrastructure_failure)).toContain(
      'ocr-final-classification',
    );
    expect(String(outputs?.policy_failure)).toContain(
      'ocr-final-classification',
    );
    expect(String(outputs?.completeness)).toContain('ocr-final-classification');
  });
  ...
  describe('telemetry validation before upload', () => {
    ...
    it('uses an atomic write (temp file + rename) for ocr-telemetry.json', () => {
      ...
      // the workflow step invokes bun scripts/ocr-telemetry.ts which writes
  ...
  it('preserves infrastructure diagnostics after initialization', () => {
    ...
    const laterWriters = jobSteps()
      .filter((step) => step.name !== 'Initialize OCR artifact files')
      .map((step) => commandText(asRecord(step)))
