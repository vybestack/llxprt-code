...
import { createFakeRepo, defaultState, makeIssue } from './assign-helpers.ts';
...
describe('F10: fake-gh fidelity', () => {
  it('issue PATCH drops configured unassignable assignees consistently with POST', () => {
    ...
    expect(stateIssue(state, '42')._assignees).not.toContain('alice');
    expect(stateIssue(state, '42')._assignees).toContain('bob');
  });
