...
describe('F1: Contention marker preservation', () => {
  it('rollback preserves marker when competing winner remains (interleaving)', () => {
    ...
    // winner-user remains
    expect(stateIssue(result.state, '42')._assignees).toContain('winner-user');
...
describe('F4: REST-based cap count', () => {
  it('cap check uses REST /repos/{repo}/issues, not search/issues', () => {
    ...
    const restGets = stateOpLog(result.state).filter(
      (op) =>
        op['method'] === 'GET' && op['endpoint'] === 'repos/test/repo/issues',
    );
  ...
  it('REST count excludes PRs (only counts non-PR issues)', () => {
    ...
    repo.updateState((s) => {
      const prs = asOptionalRecord(s['prs']);
      const pr = prs?.['99'];
      if (pr !== undefined && pr !== null && typeof pr === 'object') {
        const prRec = pr;
        asRecord(prRec)['_assignees'] = ['alice'];
      }
    });
...
describe('F5: Extracted record-history script', () => {
  ...
  it('creates label when absent', () => {
    ...
    expect(result.status).toBe(0);
    expect(stateLabels(result.state)['asnhist--newuser']).toBeDefined();
    expect(stateLabels(result.state)['asnhist--newuser'].color).toBe(
      HISTORY_COLOR,
    );
    expect(stateLabels(result.state)['asnhist--newuser'].description).toBe(
      HISTORY_DESC,
    );
  });
  ...
  it('idempotent when label already exists with correct definition', () => {
    ...
    expect(stateLabels(result.state)['asnhist--bob'].color).toBe(HISTORY_COLOR);
  });
  ...
  it('fails on collision (wrong color/description)', () => {
    ...
    expect(stateLabels(result.state)['asnhist--charlie'].color).toBe('FF0000');
  });
