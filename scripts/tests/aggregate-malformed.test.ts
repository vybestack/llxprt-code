...
describe('aggregate_evals: malformed current reports fail closed', () => {
  ...
  it('exits nonzero when a testResults entry is not an object', () => {
    useTempArtifactsDir((dir) => {
      const reportDir = path.join(dir, 'eval-logs-1', 'logs');
      fs.mkdirSync(reportDir, { recursive: true });
      fs.writeFileSync(
        path.join(reportDir, 'report.json'),
        JSON.stringify({ testResults: [42] }),
      );
...
describe('aggregate_evals: per-report strictness (valid + unusable report)', () => {
  function writeReports(
    ...
    unusableReport: {
      testResults:
        | never[]
        | Array<{ assertionResults: never[] }>
        | Array<{ assertionResults: Array<{ title: string; status: string }> }>
        | Array<{
            name: string;
            status: string;
            message: string;
...
describe('aggregateReports: unexpected parser exceptions fail closed', () => {
  it('records a fatal error and returns valid=false when the parser throws, instead of crashing', async () => {
    ...
    const aggregateReports =
      /** @type {(reports: string[], parseReport?: (p: string) => object) => {valid: boolean, errors: string[], usableAssertions: number}} */ mod.aggregateReports;
    expect(
      typeof aggregateReports,
      'aggregate_evals.ts must export aggregateReports',
    ).toBe('function');
