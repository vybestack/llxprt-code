...
const TELEMETRY_SCRIPT = path.join(REPO_ROOT, 'scripts', 'ocr-telemetry.ts');
...
function writeText(
  dir: string,
  name: string,
  text: string | NodeJS.ArrayBufferView<ArrayBufferLike>,
) {
  fs.writeFileSync(path.join(dir, name), text);
}

function runCli(cwd: string, env = {}) {
...
function readEmittedTelemetry(dir: string) {
...
function successArtifacts(dir: string) {
...
describe('ocr-telemetry.ts CLI — real behavioral runs', () => {
