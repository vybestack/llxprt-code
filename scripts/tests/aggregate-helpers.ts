...
const SCRIPT = path.join(ROOT, 'scripts/aggregate_evals.ts');
const HISTORICAL_SCRIPT = path.join(
  ROOT,
  'scripts/aggregate-evals-historical.ts',
);
 ...
 * @returns {void}
 */
export function useTempArtifactsDir(build: (dir: string) => void): void {
...
/**
 * Spawn scripts/aggregate_evals.ts as a child process against `artifactsDir`
...
/**
 * Dynamically import scripts/aggregate_evals.ts as an ESM module so unit tests
 * can exercise its exported helpers (e.g. isWithinRetentionWindow,
 * selectRunsInWindow) directly.
 *
 * @returns {Promise<Record<string, unknown>>}
 */
export async function loadAggregateModule(): Promise<
  typeof import('../aggregate_evals.ts')
> {
...
export function writeAttempt(
  dir: string,
  attempt: number,
  report: unknown = fixtureReport({ pass: 1 }),
): void {
 ...
 */
export function writeReportRaw(dir: string, raw: string): void {
 ...
 */
export function writeReport(dir: string, report: unknown): void {
 ...
 * @returns {{stdout: string, stderr: string, exitCode: number}}
 */
export function runWithOneExpected(dir: string): ScriptResult {
 ...
 */
export function envelope(runs: readonly unknown[]): string {
...
/**
 * Dynamically import scripts/aggregate-evals-historical.ts as an ESM module so
 ...
 * @returns {Promise<Record<string, unknown>>}
 */
export async function loadHistoricalModule(): Promise<
  typeof import('../aggregate-evals-historical.ts')
> {
 ...
 */
export function runEntry(id: number, daysAgo: number): RawRun {
  return {
    id,
    conclusion: 'success',
    head_sha: `sha-${id}`,
 ...
 */
export function normalizedRun(raw: RawRun) {
  return {
    databaseId: raw.id,
 ...
 */
export function fakeEnvelopeRunner(pages: readonly string[]) {
  let pageCalls = 0;
  const runSync = () => ({
    status: 0,
