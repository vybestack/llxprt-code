...
import { stateComments, stateIssue } from './typed-test-helpers.ts';
...
describe('F2: array mutations via --input -', () => {
  it('assign-issue.sh sends valid JSON arrays (not strings)', () => {
    ...
    expect(stateIssue(result.state, '42')._label_names).toContain('bug');
  });
...
describe('F3: fail-closed label reads/removals', () => {
  it('cleanup fails nonzero and preserves state when label read fails', () => {
    ...
    expect(stateIssue(result.state, '42')._label_names).toContain('bug');
    expect(stateIssue(result.state, '42')._label_names).toContain('important');
  });
  ...
  it('comment lookup failure does not create a duplicate comment and exits nonzero', () => {
    ...
    const markerComments = stateComments(result.state).filter(
      (c) =>
        ...
        String(c.body).startsWith('<!-- llxprt-assign-feedback -->'),
    );
...
describe('F5: cap re-read before assignment', () => {
  it('refuses assignment when issue gets assigned by concurrent run before POST', () => {
    ...
    expect(stateIssue(result.state, '42')._assignees).not.toContain('alice');
    expect(stateIssue(result.state, '42')._assignees).toContain('someone-else');
  });
});
