...
function diffFor(file: string, addedLine: string) {
  return [
...
function configDiff(file: string, removedLine: string, addedLine: string) {
  return [
    'diff --git a/' + file + ' b/' + file,
    'index 0000000..1111111 100644',
    '--- a/' + file,
...
function rulesBlockOpenerDiff(addedLine: string) {
  return [
...
describe('check-eslint-guard', () => {
  ...
  describe('TypeScript suppression directives (#2189)', () => {
    ...
    it('rejects a real @ts-nocheck added to the guard test fixture file', () => {
      const violations = checkDiff(
        diffFor('scripts/tests/eslint-guard.test.ts', '// @ts-nocheck'),
      );

      expect(violations).toHaveLength(1);
      expect(violations[0].file).toBe('scripts/tests/eslint-guard.test.ts');
      expect(violations[0].message).toContain('TypeScript suppression');
    });
    ...
    it('does not flag TS suppression text inside strings in the guard test fixture', () => {
      ...
      const violations = checkDiff(
        diffFor(
          'scripts/tests/eslint-guard.test.ts',
  ...
  describe('ESLint config loosening (#2189)', () => {
    ...
    describe('end-to-end checkDiff behavior', () => {
      ...
      describe('zero-context diffs (edge case for minimal-context hunks)', () => {
        ...
        function zeroContextDiff(removedLine: string, addedLine: string) {
          return [
            'diff --git a/eslint.config.js b/eslint.config.js',
            'index 0000000..1111111 100644',
            '--- a/eslint.config.js',
  ...
  describe('scoped ESLint rule IDs (#2189 review finding)', () => {
    ...
    describe('scoped rules in zero-context diffs', () => {
      function scopedZeroContextDiff(removedLine: string, addedLine: string) {
        return [
          'diff --git a/eslint.config.js b/eslint.config.js',
          'index 0000000..1111111 644',
          '--- a/eslint.config.js',
...
describe('scanPackageTypeScriptSuppressions (#2189)', () => {
  ...
  it.each(['mts', 'cts'])(
    'reports violations in .%s source files',
    (extension) => {
      const tmpDir = mkdtempSync(
        join(tmpdir(), `eslint-guard-ts-suppress-${extension}-`),
      );
      mkdirSync(tmpDir, { recursive: true });
      writeFileSync(
        join(tmpDir, `example.${extension}`),
        [
          'export const x = 1;',
          '// @ts-expect-error typed module suppression',
          'export const y = x.bad;',
        ].join(String.fromCharCode(10)),
      );
...
describe('scanRootTypeScriptSuppressions (#2189 review finding)', () => {
  it.each(['.test.cts', '.test.mts', '.spec.cts', '.spec.mts'])(
    'excludes %s files from production suppression scans',
    (suffix) => {
      const tmpDir = mkdtempSync(join(tmpdir(), 'eslint-guard-module-test-'));
      mkdirSync(join(tmpDir, 'scripts'), { recursive: true });
      writeFileSync(
        join(tmpDir, 'scripts', `fixture${suffix}`),
        '// @ts-expect-error intentional test fixture\nexport const value = 1;\n',
      );

      expect(scanRootTypeScriptSuppressions(tmpDir, '2746')).toEqual([]);
    },
  );
