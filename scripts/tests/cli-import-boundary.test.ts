...
function runScript(
  ...
  if (useRealRepo) {
    ...
    env.CLI_BOUNDARY_ROOT = dir ?? undefined;
  }
  let stdout = '';
  let stderr = '';
  let exitCode = 0;
  try {
    ...
    // 'SIGTERM' or code 'ETIMEDOUT'. Without this detection, e.status ?? 1
    ...
    // detection, e.status ?? 1 would coerce it to exit 1 and mask it as a
    ...
    // status is null when the process was killed by a signal (e.g. SIGKILL,
    // OOM). Narrow manually to preserve the null-coalescing semantics without
    // asNumber (which throws on null).
 ...
 */
function thinIndex(): [string, string] {
 ...
 */
function cleanProductionFile(): [string, string] {
...
describe.skipIf(process.env.CI !== 'true' && !bunAvailable())(
  ...
  () => {
    ...
    it('does not symbol-check the bare agents root (delegated to API-surface guard)', () => {
      ...
      // API-surface guard (scripts/check-agents-api-surface.ts) plus typecheck
