...
describe('J2: assignment workflow concurrency', () => {
  const parsedWorkflow = asRecord(
    yaml.load(readRootFile('.github/workflows/assign.yml')),
  );
  const workflowJobs = asRecordMap(parsedWorkflow['jobs'] ?? {});
  const assignJob = workflowJobs['assign'];
  const recordHistoryJob = workflowJobs['record-history'];
...
describe('J1: cleanup workflow concurrency', () => {
  ...
  const parsedWorkflow = parseWorkflowYaml(source);
  const cleanupJobs = asRecordMap(parsedWorkflow['jobs'] ?? {});
  const cleanupJob = cleanupJobs['cleanup'];
  ...
  it('has a repository-scoped concurrency group with cancel-in-progress false', () => {
    ...
    const concurrency = asOptionalRecord(cleanupJob?.['concurrency']);
