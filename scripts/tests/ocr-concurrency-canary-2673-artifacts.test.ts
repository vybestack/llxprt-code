...
import { asOptionalRecord, jobSteps } from './typed-test-helpers.ts';
...
describe('.github/workflows/ocr-review.yml — issue #2673 canary artifacts', () => {
  ...
  beforeAll(() => {
    ...
    const jobs = asOptionalRecord(loaded.parsed['jobs']);
  ...
  describe('dispatch-only artifact lifecycle', () => {
    ...
    it('keeps invalid canary JSON parseable and sanitized through the real redaction step', () =>
      withTempDirectory('ocr-redact-2673-', (directory: string) => {
    ...
    it('executes final validation against valid, invalid, redacted, malformed, and missing artifacts', () => {
      ...
      for (const testCase of cases) {
        withTempDirectory('ocr-validate-2673-', (directory: string) => {
    ...
    it('validates final JSON and then always uploads a separate dispatch artifact', () => {
      ...
      const steps = jobSteps(codeReviewJob);
      expect(steps.indexOf(validate)).toBeLessThan(steps.indexOf(upload));
    });

    it('never reads or uploads raw OCR session JSONL', () => {
      const allSteps = jobSteps(codeReviewJob);
      const uploadPaths = allSteps
        ...
        .map((step: Record<string, unknown>) =>
          String(asOptionalRecord(step['with'])?.['path'] ?? ''),
        )
        .join('\n');
