...
describe('.github/workflows/ocr-review.yml — severity-based routing (#2672)', () => {
  ...
  function assertFunctionsPresent(script: string | string[]) {
   ...
   * test run.
   */
  let cachedRouteFinding: ((arg: unknown) => unknown) | null = null;
  let cachedCategorySeverityLabel: ((arg: unknown) => unknown) | null = null;

  function loadWorkflowFunctions(): {
    routeFinding: (arg: unknown) => unknown;
    categorySeverityLabel: (arg: unknown) => unknown;
  } {
    ...
    const routeSandbox: Record<string, unknown> = {};
    vm.createContext(routeSandbox);
    vm.runInContext([...helpers, routeSrc].join('\n'), routeSandbox);
    expect(typeof routeSandbox.routeFinding).toBe('function');
    cachedRouteFinding = asVmFunction(routeSandbox.routeFinding);
    const labelSandbox: Record<string, unknown> = {};
    ...
    cachedCategorySeverityLabel = asVmFunction(
      labelSandbox.categorySeverityLabel,
    );
    return {
      routeFinding: cachedRouteFinding,
      categorySeverityLabel: cachedCategorySeverityLabel,
  ...
  function loadRouteFinding(): (arg: unknown) => {
    destination: string;
    reason: string;
  } {
    const fn = loadWorkflowFunctions().routeFinding;
    return (arg: unknown): { destination: string; reason: string } => {
      const result = fn(arg);
      if (
        result !== null &&
      ...
      throw new Error('routeFinding should return an object');
    };
  }

  function loadCategorySeverityLabel(): (arg: unknown) => string {
    const fn = loadWorkflowFunctions().categorySeverityLabel;
    return (arg: unknown): string => asString(fn(arg));
  }
  ...
  describe('YAML wiring', () => {
    ...
    it('wires OCR_ROUTING_SHADOW_MODE env variable', () => {
      const env = asOptionalRecord(postStep?.env);
      expect(env?.OCR_ROUTING_SHADOW_MODE).toBe(
    ...
    it('exposes comments_routed_summary in job outputs', () => {
      ...
      expect(outputs?.comments_routed_summary).toBe(
    ...
    it('includes ocr-routing-decisions.json in Upload artifacts step', () => {
      ...
      const withPath = asOptionalRecord(uploadStep?.with);
      expect(withPath?.path).toContain('ocr-routing-decisions.json');
    });
  });
