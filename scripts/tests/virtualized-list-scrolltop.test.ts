...
import type { VirtualizedListRef } from '../../packages/cli/src/ui/components/shared/VirtualizedList.types.js';

describe('VirtualizedList scrollTop', () => {
  it('clamps negative scrollTop when content fits', () => {
    const listRef = createRef<VirtualizedListRef<string>>();
    ...
    act(() => {
      render(
        React.createElement(
          Box,
          { height: 20, width: 40 },
          React.createElement(VirtualizedList<string>, {
            ref: listRef,
            data: ['one'],
