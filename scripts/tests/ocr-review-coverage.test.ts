...
describe('.github/workflows/ocr-review.yml — changed-file coverage verification (issue #2675)', () => {
  ...
  describe('evidencedPathsFromResult behavior (parsed input parity)', () => {
    ...
    it('uses a file field as a fallback when path is absent on a finding', () => {
      const fn = loadFn();
      const result = asStringArray(
        fn([{ file: 'src/file-field.ts', content: 'c' }]),
      );
    ...
    it('deduplicates paths from findings', () => {
      const fn = loadFn();
      const result = asStringArray(
        fn([{ path: 'shared.ts' }, { path: 'shared.ts' }]),
      );
  ...
  describe('parse-salvage integration (parseFindings → evidencedPathsFromResult)', () => {
    interface ParseAndEvidence {
      parseFindings: (input: string) => unknown[];
      evidencedPathsFromResult: (input: unknown) => string[];
    }

    function loadParseAndEvidence(): ParseAndEvidence {
      ...
      const parseFindings = (input: string): unknown[] => {
        const result = asVmFunction(sandbox['parseFindings'])(input);
        expect(Array.isArray(result), 'parseFindings should return array').toBe(
          true,
        );
        const arr: unknown[] = [];
        if (Array.isArray(result)) {
          for (const item of result) arr.push(item);
        }
        return arr;
      };
      const evidencedPathsFromResult = (input: unknown): string[] => {
        const result = asVmFunction(sandbox['evidencedPathsFromResult'])(input);
        expect(
          Array.isArray(result),
          'evidencedPathsFromResult should return array',
        ).toBe(true);
        const arr: string[] = [];
        if (Array.isArray(result)) {
          for (const item of result) arr.push(String(item));
        }
        return arr;
      };
      return { parseFindings, evidencedPathsFromResult };
    }
  ...
  describe('computeFileCoverage behavior', () => {
    function loadFn() {
      return asVmFunction(
        loadFunctionsTogether(['normalizeFilePaths', 'computeFileCoverage'])
          .computeFileCoverage,
      );
    }
    ...
    it('uses the intersection of preview and evidenced as the covered numerator', () => {
      ...
      expect(asStringArray(result.coveredFiles)).toEqual(['a.ts']);
      expect(numberRecordField(result, 'counts').covered).toBe(1);
      expect(numberRecordField(result, 'counts').preview).toBe(2);
      expect(numberRecordField(result, 'counts').evidenced).toBe(2);
      expect(asStringArray(result.unexpectedFiles)).toEqual(['surprise.ts']);
      expect(numberRecordField(result, 'counts').unexpected).toBe(1);
    });
    ...
    it('unexpected evidenced paths cannot inflate coverage above 100', () => {
      ...
      expect(numberRecordField(result, 'coverage').ratio).toBe(1);
      expect(numberRecordField(result, 'coverage').percentage).toBe(100);
      expect(numberRecordField(result, 'counts').covered).toBe(1);
      expect(numberRecordField(result, 'counts').unexpected).toBe(2);
    });
    ...
    it('identifies preview-only files (no evidence, no failure) as informational', () => {
      ...
      expect(numberRecordField(result, 'counts').preview_only).toBe(2);
      expect(numberRecordField(result, 'counts').covered).toBe(1);
    });
    ...
    it('removes failed preview paths from covered evidence', () => {
      ...
      expect(numberRecordField(result, 'counts').covered).toBe(1);
      expect(numberRecordField(result, 'counts').failed_preview).toBe(1);
      expect(numberRecordField(result, 'counts').read_failures).toBe(1);
    });
    ...
    it('defines empty preview semantics as 100% coverage with no gaps', () => {
      ...
      expect(numberRecordField(result, 'counts').preview).toBe(0);
      expect(numberRecordField(result, 'coverage').ratio).toBe(1);
      expect(numberRecordField(result, 'coverage').percentage).toBe(100);
    ...
    it('does not count zero-evidence preview files as covered', () => {
      ...
      expect(numberRecordField(result, 'counts').covered).toBe(0);
      expect(numberRecordField(result, 'coverage').percentage).toBe(0);
    });
  });
  ...
  describe('buildCoverageReport behavior', () => {
    ...
    it('produces a deterministic report with separated counts and lists', () => {
      ...
      expect(numberRecordField(result, 'counts').preview).toBe(2);
      expect(numberRecordField(result, 'counts').evidenced).toBe(1);
      expect(numberRecordField(result, 'counts').covered).toBe(1);
      expect(numberRecordField(result, 'counts').preview_only).toBe(1);
      expect(numberRecordField(result, 'counts').unexpected).toBe(0);
      expect(numberRecordField(result, 'counts').read_failures).toBe(0);
      expect(numberRecordField(result, 'coverage').ratio).toBe(0.5);
      expect(numberRecordField(result, 'coverage').percentage).toBe(50);
    ...
    it('compares the unrounded ratio so 89.5% is below a 90 threshold', () => {
      ...
      expect(numberRecordField(result, 'coverage').percentage).toBe(90);
      expect(result.below_threshold).toBe(true);
    });
    ...
    it('marks below_threshold true when read failures exist even if coverage is 100%', () => {
      ...
      expect(result.below_threshold).toBe(true);
      expect(numberRecordField(result, 'counts').read_failures).toBe(1);
      expect(numberRecordField(result, 'counts').failed_preview).toBe(0);
    });
    ...
    it('unions structured subtask_error failures with stderr read failures', () => {
      ...
      expect(numberRecordField(result, 'counts').read_failures).toBe(2);
      expect(numberRecordField(result, 'counts').failed_preview).toBe(2);
      expect(numberRecordField(result, 'counts').covered).toBe(1);
  ...
  describe('serializeCoverageReport behavior (redaction)', () => {
    ...
    it('produces valid JSON that round-trips', () => {
      ...
      expect(JSON.parse(asString(serialize(report)))).toEqual(report);
    });
    ...
    it('redacts a secret token that appears in the report', () => {
      ...
      expect(() => JSON.parse(asString(result))).not.toThrow();
    });
    ...
    it('emits only a safe error when redaction throws', () => {
      ...
      const result = asVmFunction(sandbox.serializeCoverageReport)({
        a: 'secret',
      });
      expect(JSON.parse(asString(result))).toEqual({
