...
import { asRecordArray } from './typed-test-helpers.ts';
import {
  useTempArtifactsDir,
  fixtureReport,
  writeReport,
...
describe('aggregate_evals: Vitest fullName is space-joined (not " > ")', () => {
  it('fixtureReport produces a space-joined fullName matching the real reporter', () => {
    ...
    const testResults = asRecordArray(report['testResults']);
    const firstResult = testResults[0];
    const assertion = asRecordArray(firstResult['assertionResults'])[0];
