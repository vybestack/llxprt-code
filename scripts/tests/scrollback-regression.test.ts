 ...
 * This should FAIL pre-fix when `LLXPRT_E2E_TMUX=1` is set.
 */
...
const extractArtifactsPath = (text: string) => {
...
runTest('scrollback regression (tmux baseline)', () => {
  ...
  const result = spawnSync(
    ...
    [
      ...
      '--script',
      'scripts/tmux-script.llm-tool-scrollback-realistic.llxprt.json',
    ],
    {
      cwd: repoRoot,
      env: { ...process.env },
  ...
  if (result.status !== 0) {
    ...
    throw new Error(
      `tmux baseline failed (exit ${result.status})\nartifacts: ${artifacts}\n--- stdout ---\n${stdout}\n--- stderr ---\n${stderr}`,
    );
  }
});
...
runGeminiTest('gemini CLI scrollback control (tmux baseline)', () => {
  ...
  const result = spawnSync(
    ...
    [
      'scripts/tmux-harness.ts',
      '--script',
      'scripts/tmux-script.llm-tool-scrollback-realistic.gemini.json',
    ],
    {
      cwd: repoRoot,
      env: { ...process.env },
