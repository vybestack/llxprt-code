  ...
  it('defaults the artifacts directory to the current working directory', () => {
    useTempArtifactsDir((dir) => {
      ...
      try {
        ...
        // stdout/stderr under encoding: 'utf8'. Use the non-throwing isRecord
        // guard so a non-object throw value falls back gracefully instead of
        // propagating a new Error out of the catch block.
        const errRecord = isRecord(error) ? error : {};
        const stdout =
          typeof errRecord['stdout'] === 'string' ? errRecord['stdout'] : '';
        result = {
          stdout,
          exitCode:
