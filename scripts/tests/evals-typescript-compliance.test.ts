...
async function loadEvalsEslintBlock() {
  ...
  const block = config.find(
    (entry: unknown) =>
      typeof entry === 'object' &&
      entry !== null &&
      containsExactGlob(asRecord(entry)['files'], EXACT_EVALS_GLOB),
  );
  expect(block, 'must define an eslint block targeting evals').toBeDefined();
  return block;
}

function containsExactGlob(value: unknown, expected: string[]): boolean {
  return (
    Array.isArray(value) &&
    value.some(
      (pattern) => typeof pattern === 'string' && expected.includes(pattern),
    )
  );
}
 ...
 * @param {string[]} out
 */
function collectStringIgnores(entry: unknown, out: string[]) {
  if (
    !(
      entry &&
      typeof entry === 'object' &&
      Array.isArray(asRecord(entry)['ignores'])
    )
  ) {
    return;
  }
  const ignores = asRecord(entry)['ignores'];
 ...
 * `['error', {...}]` forms.
 */
function isRuleError(ruleValue: string | string[]) {
  if (ruleValue === 'error') {
    return true;
  }
...
describe('evals: TypeScript static compliance configuration', () => {
  it('tolerates non-string entries while inspecting config arrays', () => {
    expect(
      containsExactGlob(['evals/**/*.{ts,tsx}', 42], ['evals/**/*.{ts,tsx}']),
    ).toBe(true);
    const ignores: string[] = [];
    collectStringIgnores({ ignores: ['dist/**', 42] }, ignores);
    expect(ignores).toEqual(['dist/**']);
  });
