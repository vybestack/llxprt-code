...
describe('H1: Closed assignment target', () => {
  it('closed issue receives no assignee, label, history, or success feedback', () => {
    ...
    expect(comments.some((c) => /closed|not open/i.test(String(c.body)))).toBe(
      true,
    );
    expect(comments.some((c) => /\[OK\] Assigned/.test(String(c.body)))).toBe(
      false,
    );
  });
  ...
  it('assign.yml workflow gate includes issue.state == open', () => {
    ...
    const assignJob = workflowJob(workflow, 'assign');
    const condition = normalize(asOptionalString(assignJob.if));
