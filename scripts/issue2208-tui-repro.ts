...
function normalizeCapturedLine(line: string) {
  return line
    .replace(/█/g, '')
    .replace(/^\s*│\s?/, '')
    .trimEnd();
...
function normalizeCapturedAssistantText(text: string) {
  return text
    .replace(/\r\n/g, '\n')
    .split('\n')
...
function extractAssistantOutput(scrollback: string) {
...
function validateCapturedOutput(profile: string, outDir: string): boolean {
  const captureFile = readdirSync(outDir).find((fileName: string) =>
...
function createScenario(profile: string) {
...
function runProfile(profile: string): boolean {
  validateProfile(profile);
