 ...
 *
 * Usage:
 *   node scripts/codemods/pse-fix.ts <locations-file>
 *
...
for (const line of lines) {
  ...
  if (entry !== undefined) {
    entry.push({ line: Number(ln), col: Number(col) });
  }
}
...
function processLocation(
  ...
  let pae: Node | undefined = node;
  while (pae && pae.getKind() !== SyntaxKind.PropertyAccessExpression) {
    pae = pae.getParent();
  }
  if (!pae || !Node.isPropertyAccessExpression(pae)) return 'skipped';
...
for (const [filePath, locs] of byFile) {
  ...
  locs.sort((a: Location, b: Location) => b.line - a.line || b.col - a.col);
