 ...
 * Usage:
 *   node scripts/codemods/apply-suggestions.ts <ruleId> <file...>
 *
...
type SuggestionMessage = Linter.LintMessage & {
  suggestions: Array<{ fix: { range: [number, number]; text: string } }>;
};
 ...
 */
async function applyOneSuggestionIteration(
  eslint: ESLint,
  file: string,
  ruleId: string,
  iteration: number,
  ...
  msgs.sort(
    (a: SuggestionMessage, b: SuggestionMessage) =>
      b.suggestions[0].fix.range[0] - a.suggestions[0].fix.range[0],
  );
  const m = msgs[0];
