 ...
 *
 * Usage: node scripts/codemods/no-conditional-expect.ts <file> [file...]
 */

import { IfStatement, Node, Project, SourceFile, SyntaxKind } from 'ts-morph';
...
function invertTest(testNode: Node): string {
  const txt = testNode.getText();
  if (Node.isPrefixUnaryExpression(testNode)) {
  ...
  if (Node.isBinaryExpression(testNode)) {
...
function containsExpectCall(node: Node): boolean {
  let found = false;
  node.forEachDescendant((d: Node, traversal: { stop: () => void }) => {
...
function shapeOkForInversion(testNode: Node): boolean {
 ...
 */
function rewriteIfStatement(sf: SourceFile, ifStmt: IfStatement) {
  ...
  const dedented = innerText
    .split('\n')
    .map((line: string) =>
      line.startsWith(dedentPrefix) ? line.slice(2) : line,
    )
    .join('\n');
