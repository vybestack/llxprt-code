 ...
 *
 * Usage: node scripts/codemods/nce-try-unreachable.ts <file...>
 */

import { Project, SourceFile, SyntaxKind, TryStatement } from 'ts-morph';

const argv = process.argv.slice(2);
if (argv.length === 0) {
  console.error('usage: nce-try-unreachable.ts <file...>');
  process.exit(2);
}
 ...
 */
function rewriteTryStatement(sf: SourceFile, tryStmt: TryStatement) {
  ...
  const keptTryStmts = tryBodyStmts
    .slice(0, -1)
    .map((s: { getText: () => string }) => indent + '  ' + s.getText())
    .join('\n');
