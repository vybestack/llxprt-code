 ...
 *
 * Usage: bun scripts/codemod-import-type-annotations.ts <file> [<file> ...]
 */
...
if (files.length === 0) {
  console.error(
    'Usage: codemod-import-type-annotations.ts <file> [<file> ...]',
  );
  process.exit(1);
}
 ...
 */
function processImportTypeNode(
  node: ImportTypeNode,
  ...
  const entry = needed.get(modulePath);
  if (entry !== undefined) {
    entry.add(typeName);
  }
...
function mergeImportForModule(
  ...
  for (const name of missing) {
    const specifier = existing.addNamedImport(name);
 ...
 */
function processSourceFile(project: Project, resolved: string): number {
