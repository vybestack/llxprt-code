 ...
 *   subpath), one of the declared runtime subpath exports in
 *   ALLOWED_RUNTIME_SUBPATHS, or a declared dependency of packages/a2a-server
 *   (the dependency or its subpaths — derived from package.json so the
 *   allowlist cannot drift from the manifest).
...
/**
 * Declared runtime subpath exports a host may import despite the ROOT rule.
 *
 * The root-only rule exists to stop hosts reaching into runtime *internals*.
 * #3305 introduced one subpath that is not an internal: the MCP host-services
 * seam, published deliberately in packages/mcp/package.json `exports` as
 * "./host/hostServices.js". It is the registration point a host process calls
 * once at startup, and packages/cli and packages/agents import that exact
 * specifier too. Reaching it through the MCP root entrypoint would pull the
 * whole client surface in for a two-field registration, which is the cost the
 * dedicated export exists to avoid.
 *
 * Entries are matched exactly, never by prefix, so this stays fail-closed: no
...
export function evaluateSpecifier(
  ...
  if (ALLOWED_RUNTIME_SUBPATHS.includes(specifier)) {
    return {
      allowed: true,
      reason: 'declared runtime host-wiring subpath export',
    };
  }
