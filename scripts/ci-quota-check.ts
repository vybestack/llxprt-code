...
function extractLimit(data: unknown): number | undefined {
  if (data !== null && typeof data === 'object' && !Array.isArray(data)) {
    const subscription = Reflect.get(data, 'subscription');
    if (
      subscription !== null &&
      typeof subscription === 'object' &&
      !Array.isArray(subscription)
    ) {
      const limit = Reflect.get(subscription, 'limit');
...
function extractRequests(data: unknown): number | undefined {
  if (data !== null && typeof data === 'object' && !Array.isArray(data)) {
    const subscription = Reflect.get(data, 'subscription');
    if (
      subscription !== null &&
      ...
      const requests = Reflect.get(subscription, 'requests');
 ...
 */
function assertKeySafe(key: string, label: string) {
...
function writeSelectedKeyOutput(selectedKeyName: string) {
 ...
 */
function exportSelectedKeyToEnv(keyValue: string, keyName: string): void {
...
async function checkQuota(
  ...
  try {
    ...
    console.log(`${keyName} quota check error: ${message}`);
    return null;
  } finally {
...
export async function selectOptimalKey() {
  ...
  const usable1 = quota1 !== null && quota1.usagePercent <= 90 ? quota1 : null;
  const usable2 = quota2 !== null && quota2.usagePercent <= 90 ? quota2 : null;
  ...
  assertKeySafe(safeKey, 'Selected API key');

  const selectedKeyName = safeKey === key2 ? 'secondary' : 'primary';
  writeSelectedKeyOutput(selectedKeyName);
  exportSelectedKeyToEnv(safeKey, selectedKeyName);
}
...
if (
  ...
  main().catch((e: unknown) => {
    const message = e instanceof Error ? e.message : String(e);
    console.error('Quota selection failed:', message);
    process.exit(1);
  });
}
