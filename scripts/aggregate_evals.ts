 ...
 *
 * Usage:
 *   node scripts/aggregate_evals.ts [artifacts-dir]
 *
...
import {
  ...
  fetchHistoricalData,
  normalizeRunEntry,
} from './aggregate-evals-historical.ts';
import {
 ...
 */
function buildTableHeader(historicalRuns: string[]) {
  const headers = ['Test'];
  const alignments = [':---'];
...
function buildTestRow(
  testName: string,
  currentTestStats: { pass: number; fail: number; total: number } | undefined,
  historicalRuns: string[],
  historicalStats: Map<
    string,
    Map<string, { pass: number; fail: number; total: number }>
...
function generateMarkdown(
  currentStats: Map<string, { pass: number; fail: number; total: number }>,
  historicalStats: Map<
    string,
    Map<string, { pass: number; fail: number; total: number }>
  >,
 ...
 */
function aggregateArtifacts(artifactsDir: string, expectedAttempts: number[]) {
  ...
  try {
    ...
    console.error(
      `Warning: Could not fetch historical data, continuing with current run only: ${err instanceof Error ? err.message : String(err)}`,
    );
  }
