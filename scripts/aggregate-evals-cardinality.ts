 ...
 */
function isCanonicalAttemptString(captured: string) {
 ...
 * @returns {number|null}
 */
export function extractAttemptFromPath(reportPath: string) {
 ...
 * @returns {number|null}
 */
function attemptFromArtifactName(name: string) {
 ...
 */
export function classifyTopLevelEntry(artifactsDir: string, entry: string) {
 ...
 * @returns {string[]} validation errors (empty when cardinality is correct)
 */
export function validateTopLevelArtifactDirs(
  artifactsDir: string,
  expectedAttempts: number[],
) {
  const errors = [];
  ...
  try {
    ...
    errors.push(
      `Could not read artifacts directory ${artifactsDir}: ${message}`,
    );
    return errors;
  }
 ...
 * @returns {number[]|null}
 */
export function parseExpectedAttempts(raw: unknown) {
 ...
 *   malformed.
 */
export function resolveExpectedAttempts(argv: string[]) {
