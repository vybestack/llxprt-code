...
function getArgList(name: string) {
...
`,
  ...
  try {
    ...
    process.stderr.write(
      `Could not read OCR rules file at ${rulesPath}: ${err instanceof Error ? err.message : String(err)}
`,
    );
    process.exit(1);
    return ''; // unreachable — satisfies consistent-return
...
function gitDiffStat(from: string, to: string) {
  try {
...
function parseOcrOutput(stdout: string) {
  ...
  const warnings = Array.isArray(parsed?.warnings)
    ? parsed.warnings.map((w: unknown) => redact(String(w)))
    : [];
...
function runOcrReview(from: string, to: string, concurrency: number) {
...
try {
  ...
  process.stderr.write(
    `Could not create output directory ${outputDir}: ${err instanceof Error ? err.message : String(err)}\n`,
  );
  process.exit(1);
}
