...
import {
  parseCurrentReport,
  aggregateStats,
} from './aggregate-evals-schema.ts';
import {
  DEFAULT_EXPECTED_ATTEMPTS,
  validateTopLevelArtifactDirs,
  validateCardinality,
} from './aggregate-evals-cardinality.ts';
...
type RunSync = (cmd: string, args: readonly string[]) => RunSyncResult;

interface ListRunsPageOptions {
  runSync?: RunSync;
}

interface HistoricalRunResult {
  runId: string;
  stats: Map<string, { pass: number; fail: number; total: number }>;
  omitted: boolean;
  reason?: string;
}
 ...
 * @type {RunSync}
 */
function spawnSyncBound(cmd: string, args: readonly string[]): RunSyncResult {
...
export function isWithinRetentionWindow(
  createdAt: string,
  now: number,
  retentionDays: number = HISTORICAL_RETENTION_DAYS,
) {
 ...
 */
export function selectRunsInWindow<T extends { createdAt: string }>(
  runs: T[],
  now: number,
 ...
 */
export function classifyRunAge(
  createdAt: string,
  now: number,
  retentionDays: number,
) {
 ...
 */
export function classifyRunsPage(
  runs: Array<{ createdAt: string; databaseId?: number }>,
  now: number,
  retentionDays: number,
): {
  inWindow: Array<{ createdAt: string; databaseId?: number }>;
  sawOutOfWindow: boolean;
} {
  const inWindow: Array<{ createdAt: string; databaseId?: number }> = [];
 ...
 */
export function listWorkflowRunsInWindow(
  listRunsPage: (page: number) =>
    | Array<{ createdAt: string; databaseId?: number }>
    | {
        runs: Array<{ createdAt: string; databaseId?: number }>;
        rawCount: number;
        totalCount: number;
  ...
  for (let page = 1; page <= GH_RUN_LIST_MAX_PAGES; page++) {
    ...
    const pageResult: {
      ...
      rawCount?: number;
      totalCount?: number;
    } = Array.isArray(raw) ? { runs: raw } : (raw ?? { runs: [] });
    const runs = pageResult.runs;
 ...
 */
export function buildRunListArgs(page: number) {
  return [
    'api',
 ...
 */
export function normalizeRunEntry(entry: unknown) {
  ...
  const createdAt = Reflect.get(entry, 'created_at');
  if (typeof createdAt !== 'string' || Number.isNaN(Date.parse(createdAt))) {
    return null;
  }
  const conclusion = Reflect.get(entry, 'conclusion');
  if (conclusion !== null && typeof conclusion !== 'string') {
    return null;
  }
  const headSha = Reflect.get(entry, 'head_sha');
 ...
 */
function parseRunListEnvelope(stdout: string) {
 ...
 * @returns {{runs: Array<{databaseId: number, createdAt: string, conclusion: string|null, headSha: string}>, rawCount: number, totalCount: number}}
 */
export function listRunsPageWithGh(
  page: number,
...
function findReportsEntry(
  ...
  try {
    ...
    if (stat.isDirectory()) {
      reports.push(...findReports(String(fullPath)));
    } else if (entry === 'report.json') {
      reports.push(String(fullPath));
    }
  } catch (err) {
 ...
 */
function parseHistoricalReportsStrict(reports: string[]) {
 ...
 */
function downloadRunWithGh(runId: string | number, dir: string) {
 ...
 */
export function processHistoricalRun(
  run: { databaseId?: number | string; createdAt?: string },
  downloadRun?: (
    runId: string | number,
    dir: string,
  ...
  const empty = {
    runId,
    stats: new Map<string, { pass: number; fail: number; total: number }>(),
    omitted: true,
  };
...
export function fetchHistoricalData(
  ...
  }) => HistoricalRunResult = processHistoricalRun,
) {
  const historical = new Map<
    string,
    Map<string, { pass: number; fail: number; total: number }>
  >();
