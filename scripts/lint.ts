...
const TEMP_DIR = join(tmpdir(), 'llxprt-code-linters');
...
const LINTERS: Record<
  string,
  { check: string; installer: string; run: string }
> = {
  actionlint: {
...
function runCommand(command: string, stdio: StdioOptions = 'inherit'): boolean {
  try {
 ...
 */
function reportSensitiveKeywordViolation(
  lines: string[],
  matchIndex: number,
  keyword: string,
  file: string,
 ...
 */
function validateExcludeConfig(file: string, exclude: unknown): boolean {
  ...
  const invalidExcludes = excludeArr.filter((item) => {
    // Flag items that are either non-string OR not in the allowed set.
    // A Set<string> never contains null/number/object, so non-string items
    // are always invalid (matching the original JS behavior).
    if (typeof item !== 'string') return true;
    return !allowedExclude.has(item);
  });
...
export function runTSConfigLinter() {
  ...
  try {
    ...
    console.error('Error finding tsconfig.json files:', message);
    process.exit(1);
  }
  ...
  for (const file of files) {
    ...
    try {
      ...
      console.error(`Error parsing ${tsconfigPath}: ${message}`);
      hasError = true;
    }
  }
