...
export function extractRuleKey(line: string) {
...
export function extractSeverityValue(line: string) {
...
export function normalizeSeverity(raw: string) {
  switch (raw) {
    case 'error':
    case '2':
      return 'error';
...
export function extractThresholdValue(
  line: string,
  ruleKey: string,
): { value: number; form: 'max' | 'numeric' } | null {
 ...
 */
function stripTrailingComment(line: string) {
  const idx = line.indexOf('//');
  return idx === -1 ? line : line.slice(0, idx);
}

export function isMultilineArraySeverityEntry(line: string) {
  return /^[ \t]*['"](?:error|warn|off)['"][ \t]*(?:,[ \t]*)?$/.test(
    stripTrailingComment(line.trim()),
  );
}

export function isMultilineNumericSeverityEntry(line: string) {
  return /^[ \t]*(?:2|1|0)[ \t]*(?:,[ \t]*)?$/.test(
    stripTrailingComment(line.trim()),
  );
}

export function normalizeMultilineSeverity(line: string) {
  const stringMatch = /^[ \t]*['"](error|warn|off)['"]/.exec(line);
  if (stringMatch !== null) {
    return normalizeSeverity(stringMatch[1]);
  }
...
export function isStandaloneMaxLine(line: string) {
  return /^['"]?max['"]?[ \t]*:[ \t]*\d+[ \t]*(?:,[ \t]*)?$/.test(line.trim());
}

export function isObjectFormMaxLine(line: string) {
  return /^\{?[ \t]*['"]?max['"]?[ \t]*:[ \t]*\d+\b/.test(line.trim());
}

export function extractMaxValueFromStandaloneLine(line: string) {
  const match = /^\{?[ \t]*['"]?max['"]?[ \t]*:[ \t]*(\d+)/.exec(line.trim());
  return match !== null ? Number(match[1]) : null;
}

export function isStandaloneNumericThresholdLine(line: string) {
  return /^[ \t]*\d+[ \t]*(?:,[ \t]*)?$/.test(line.trim());
}

export function extractStandaloneNumericThresholdValue(line: string) {
  const match = /^[ \t]*(\d+)[ \t]*(?:,[ \t]*)?$/.exec(line.trim());
  return match !== null ? Number(match[1]) : null;
}

export function isStandaloneOffRuleValue(line: string) {
  return /^\s*(?:['"]off['"]|0),?\s*(?:\/\/.*)?$/.test(line);
}

export function isRuleOffEntry(
  line: string,
  insideRulesBlock: boolean,
...
export function hasAssignmentOperator(line: string) {
...
function isAssignmentOperatorAt(line: string, i: number) {
  if (line[i] !== '=') {
    return false;
  }
  const prev = line[i - 1];
...
export function isRuleSeverityAssignmentShape(line: string) {
  if (extractRuleKey(line) === null) {
    return false;
  }
...
export function buildRuleState(
  content: string,
  ruleKey: string | null,
  isStandaloneSeverity: boolean,
  isStandaloneNumericThreshold: boolean,
  isStandaloneMax: boolean,
) {
  const keyedRuleKey = extractRuleKey(content);
  const effectiveKey = keyedRuleKey !== null ? keyedRuleKey : ruleKey;

  let severity: 'off' | 'warn' | 'error' | null = null;
  let threshold: number | null = null;
  let thresholdForm: 'max' | 'numeric' | null = null;
...
export function compareRuleConfigChanges(
  removedContentLine: string,
  addedContentLine: string,
) {
...
function pushSeverityDowngrade(
  messages: string[],
  removedLine: string,
  addedLine: string,
  ruleKey: string,
...
export function isCompleteSingleLineRuleEntry(content: string) {
...
export function openerExpectsFirstArrayElement(content: string) {
 ...
 */
function advanceQuoteEscapeState(
  ch: string,
  next: string,
  quote: Quote | null,
  escaped: boolean,
): { quote: Quote | null; escaped: boolean; skip: boolean } {
  if (escaped) {
...
function openerBracketExpectsElement(content: string) {
...
function contentAfterBracketIsEmpty(content: string, start: number) {
...
export function countDiffBracketAndBraceDelta(line: string) {
  let delta = 0;
  let quote: Quote | null = null;
  let escaped = false;
