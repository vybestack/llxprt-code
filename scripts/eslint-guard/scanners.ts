...
function scanDirectoryForDirectives(
  rootDir: string,
  modulePath: string,
  issueNumber: string,
): Violation[] {
...
function scanFileForDirectives(
  filePath: string,
  modulePath: string,
  issueNumber: string,
) {
...
function isScannableTextFile(fileName: string) {
 ...
 */
export function scanModuleDirectives(
  modulePath: string,
  issueNumber: string,
  baseDir?: string,
): Violation[] {
 ...
 */
export function scanCoreDirectives(coreDir?: string): Violation[] {
  if (coreDir) {
 ...
 */
export function listTsFiles(rootDir: string): string[] {
 ...
 * .ts/.tsx.
 */
export function listCheckedSourceFiles(rootDir: string) {
  const results: string[] = [];
  if (!existsSync(rootDir)) {
    return results;
  }
  ...
  for (const entry of entries) {
    ...
    if (entry.isDirectory()) {
      ...
      entry.isFile() &&
      /\.(?:cjs|cts|mjs|mts|js|jsx|ts|tsx)$/.test(entry.name)
    ) {
      results.push(fullPath);
    }
  }
 ...
 */
function listGitTrackedCheckedSourceFiles(rootDir: string) {
  let raw: string;
  try {
...
export function isProductionCheckedSourceFile(filePath: string) {
  if (!/\.(?:cjs|cts|mjs|mts|js|jsx|ts|tsx)$/.test(filePath)) {
    return false;
  }
 ...
 */
export function scanPackageDirectives(packageDir: string, issueNumber: string) {
 ...
 */
export function scanPackageTypeScriptSuppressions(
  packageDir: string,
  issueNumber: string,
) {
  const target = packageDir;
 ...
 */
export function scanRootTypeScriptSuppressions(
  rootDir: string,
  issueNumber: string,
) {
