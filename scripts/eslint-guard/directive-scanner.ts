...
type ScannerState = TemplateState & {
  quote: Quote | null;
  escaped: boolean;
};
...
type NestedTemplateStep = {
  done: boolean;
  nExpr: number;
  nQuote: Exclude<Quote, '`'> | null;
  nEscaped: boolean;
  advance: number;
};

type BlockCommentResult =
  | { result: 'FOUND' }
  | { result: 'END' }
  | { result: 'CONTINUE'; nextIndex: number };

type CommentMarkerResult = {
  terminal: boolean | null;
  skipTo: number | null;
};
...
type ExecutableScanResult =
  | { handled: false }
  | { handled: true; result: DirectiveStep };

type TemplateLoopStep = {
  state: ScannerState;
  advance: number;
  terminal: TemplateState | null;
};

type CommentSkipResult =
  | { handled: false }
  | { handled: true; terminal: boolean; advance: number };

type TemplateCharStep = {
  state: ScannerState;
  advance: number;
  done: boolean;
};

export function shouldCheckInlineDirective(file: string) {
  if (
    file === 'scripts/check-eslint-guard.ts' ||
    file === 'scripts/tests/eslint-guard.test.ts'
  ) {
    return false;
  }
  return /\.(?:cjs|cts|mjs|mts|js|jsx|ts|tsx)$/.test(file);
}

export function shouldCheckTypeScriptSuppression(file: string) {
  return /\.(?:cjs|cts|mjs|mts|js|jsx|ts|tsx)$/.test(file);
}

export function previousCodeChar(line: string, index: number) {
...
export function canStartRegex(line: string, index: number) {
  const previous = previousCodeChar(line, index);
  return previous === '' || /[({[=,:;!&|?+\-*%^~<>]/.test(previous);
}

export function skipQuoted(line: string, start: number, quote: Quote) {
...
export function skipRegex(line: string, start: number) {
...
function scanRegexChar(ch: string, inCharacterClass: boolean) {
  if (ch === '\\') {
 ...
 */
function findNestedTemplateEnd(content: string, start: number) {
  let i = start;
  let nExpr = 0;
  let nQuote = null;
  let nEscaped = false;
...
function scanNestedTemplateChar(
  nch: string,
  nnext: string,
  nQuote: Exclude<Quote, '`'> | null,
  nEscaped: boolean,
...
function normalizeIncomingState(
  incoming: boolean | TemplateState,
): TemplateState {
...
export function hasInlineEslintDirective(line: string) {
  return scanLineForDirective(line, DIRECTIVE_PATTERN);
}

export function hasTypeScriptSuppression(line: string) {
 ...
 */
function scanLineForDirective(line: string, pattern: RegExp) {
...
function checkBlockCommentMarker(
  line: string,
  i: number,
  pattern: RegExp,
): CommentMarkerResult {
 ...
 */
export function hasInlineEslintDirectiveInState(
  line: string,
  incoming: boolean | TemplateState,
) {
 ...
 */
export function hasTypeScriptSuppressionInState(
  line: string,
  incoming: boolean | TemplateState,
) {
 ...
 */
function scanLineForDirectiveInState(
  line: string,
  pattern: RegExp,
  incoming: boolean | TemplateState,
  trackBraces: boolean,
...
function scanStateBlockComment(
  line: string,
  i: number,
  pattern: RegExp,
  state: ScannerState,
 ...
 */
function scanTemplateStructure(
  line: string,
  i: number,
  ch: string,
  next: string,
...
function scanOutsideTemplateChar(
  ch: string,
  state: ScannerState,
): DirectiveStep {
  if (ch === '`') {
 ...
 * ${ ... } EXPRESSION.
 */
export function scanTemplateLiteralState(
  content: string,
  incoming: boolean | TemplateState,
) {
  const initialState = normalizeIncomingState(incoming);
...
function advanceTemplateLoopBody(
  content: string,
  i: number,
  ch: string,
  next: string,
...
function advanceTemplateStateChar(
  content: string,
  i: number,
  ch: string,
  next: string,
...
function advanceInsideTemplate(
  content: string,
  i: number,
  ch: string,
  next: string,
...
function advanceOutsideTemplate(
  ch: string,
  state: ScannerState,
  i: number,
): TemplateCharStep {
