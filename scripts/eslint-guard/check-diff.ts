...
import { countDiffBraceDeltaWithBlockState } from './diff-context.ts';
import {
  bufferRemovedConfig,
  updateRemovedStructuralContext,
  updateStructuralContext,
} from './diff-state-tracking.ts';
import {
...
import type { DiffState, Violation } from './types.ts';

type RemovedConfig = DiffState['pendingRemovedConfigs'][number];
...
export function checkDiff(diff: string) {
...
function createDiffState(): DiffState {
  return {
    violations: [],
    policyState: {
...
function flushPendingConfigs(state: DiffState) {
...
function processDiffLine(state: DiffState, line: string) {
...
function processContextLine(state: DiffState, line: string) {
  if (line.startsWith('\\')) {
    return;
  }
...
function processAddedLine(state: DiffState, line: string) {
...
function checkAddedTsSuppression(
  state: DiffState,
  content: string,
  currentLine: number,
) {
...
function checkCliDirectiveScope(
  state: DiffState,
  content: string,
  currentLine: number,
) {
...
function processPolicyStateAdded(state: DiffState, content: string) {
  if (
...
function checkPolicyState(state: DiffState) {
...
function processConfigAddedLine(
  state: DiffState,
  content: string,
  currentLine: number,
) {
...
function checkSameKeyComparison(
  state: DiffState,
  content: string,
  currentLine: number,
  detected: boolean,
...
function findMatchingRemovedConfig(
  state: DiffState,
  addedKey: string | null,
  useZeroContextFallback: boolean,
) {
...
function isConfigMatchEligible(
  state: DiffState,
...
function checkMultilineSeverity(
  state: DiffState,
  content: string,
  currentLine: number,
  detected: boolean,
...
function multilineSeverityMessage(
  ruleKey: string,
  removedSeverity: string,
  addedSeverity: string,
) {
  return (
...
function filterMultilineSeverityByRule(state: DiffState) {
  state.pendingRemovedMultilineSeverity =
...
function checkMultilineMaxThreshold(
  state: DiffState,
  content: string,
  currentLine: number,
) {
...
function checkMultilineNumericThreshold(
  state: DiffState,
  content: string,
  currentLine: number,
) {
...
function filterMultilineNumericByRule(state: DiffState) {
  state.pendingRemovedMultilineNumericThreshold =
...
function checkOffPolicy(
  ...
  content: string,
  currentLine: number,
  severityDowngradeDetected: boolean,
  preUpdateInsideRuleEntry: boolean,
  preUpdateExpectingFirstSeverity: boolean,
) {
...
function computeRuleSeverityOffShape(
  state: DiffState,
  content: string,
  offZeroContextEligible: boolean,
  preUpdateInsideRuleEntry: boolean,
  preUpdateExpectingFirstSeverity: boolean,
) {
...
function processRemovedLine(state: DiffState, line: string) {
...
function checkRemovedMaxWarnings(
  state: DiffState,
  content: string,
  currentLine: number,
) {
...
function processRemovedDirectiveCleanupBlock(
  state: DiffState,
  content: string,
) {
  if (
...
function updateRemovedDirectiveCleanupBlockDepth(
  state: DiffState,
  content: string,
) {
