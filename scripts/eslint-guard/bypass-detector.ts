...
import { isStandaloneOffRuleValue } from './rule-config.ts';
...
interface BypassContext {
  ...
  braceDepth: number;
  moduleObjectDepth: number | null;
  rulesObjectDepth: number | null;
}
...
function scanCommentStripChar(
  char: string,
  quote: Quote | null,
  escaped: boolean,
): { done: boolean; quote: Quote | null; escaped: boolean } {
  if (escaped) {
...
function extractArrayStart(line: string): string | null {
...
function isModulePathLine(line: string, modulePath: string): boolean {
 ...
 * inside arrays.
 */
function isBareStringEntry(line: string): boolean {
  return /^\s*['"`]/.test(line);
}
...
function hasModuleCentralBypassOnSingleLine(
  line: string,
  modulePath: string,
): string | null {
  if (!isModulePathLine(line, modulePath)) {
...
function moduleCentralBypassMessage(
  modulePath: string,
  kind: string,
  issueNumber: string,
) {
...
function scanBraceDeltaChar(
  char: string,
  quote: Quote | null,
  escaped: boolean,
): { quote: Quote | null; escaped: boolean; delta: number } {
  if (escaped) {
...
function countOpeningBraces(line: string): number {
  let count = 0;
  let quote: Quote | null = null;
  let escaped = false;
...
function scanOpeningBracesChar(
  char: string,
  quote: Quote | null,
  escaped: boolean,
): { quote: Quote | null; escaped: boolean; count: number } {
  if (escaped) {
...
function updateCurrentArray(ctx: BypassContext, line: string) {
...
function pushModuleArrayBypass(
  ...
  const currentArray = ctx.currentArray;
  if (currentArray !== null) {
    const arrayKind = getArrayBypassKind(currentArray);
    if (arrayKind !== null) {
      violations.push({
        file: 'eslint.config.js',
        lineNumber,
...
function getArrayBypassKind(currentArray: string): string | null {
...
function updateBraceDepth(ctx: BypassContext, line: string) {
 ...
 * would reintroduce the old suppression pattern outside source files.
 */
export function checkCoreCentralBypassesInConfig(configSource: string) {
 ...
 */
export function checkCoreDirectiveScopesInConfig(configSource: string) {
