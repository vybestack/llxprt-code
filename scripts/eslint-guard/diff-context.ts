...
import { NON_RULE_CONTAINER_KEYS, RULE_ID_CHARS } from './constants.ts';
import {
  extractRuleKey,
...
export function propertyKeyRegex(key: string, openBrace = false) {
  const suffix = openBrace ? '\\s*:\\s*\\{' : '\\s*:';
  return new RegExp('(?:^|[^\\w])[\'"]?' + key + '[\'"]?' + suffix);
}

export function isExportDefaultConfigContext(line: string, matchIndex: number) {
...
export function isRulesInArbitraryContext(line: string, matchIndex: number) {
  const before = line.slice(0, matchIndex);
...
export function isRulesNestedInNonRuleContainer(
  line: string,
  matchIndex: number,
) {
...
function containerKeyNestsRules(line: string, key: string, matchIndex: number) {
...
export function containerOpenAtPosition(
  line: string,
  bracePos: number,
  targetPos: number,
) {
...
export function isNonRuleContainerOpen(line: string) {
  for (const key of NON_RULE_CONTAINER_KEYS) {
...
export function isRulesBlockOpen(line: string) {
 ...
 */
function endsWithOpenBraceNoClose(trimmed: string) {
...
export function isArbitraryObjectOpener(line: string) {
...
export function extractDirectValueAfterKey(segment: string) {
 ...
 */
function findInlineCloseBrace(line: string, start: number) {
  let depth = 1;
  let quote: Quote | null = null;
  let escaped = false;
  let end = line.length;
 ...
 */
function splitInlineSegments(inline: string) {
  const segments: string[] = [];
  let segStart = 0;
  let segDepth = 0;
  let segQuote: Quote | null = null;
...
export function extractInlineRulesEntries(line: string) {
...
function collectRuleEntries(segments: string[]) {
...
function collectSingleRuleEntry(segment: string) {
...
export function countDiffBraceDelta(line: string) {
  return countDiffBraceDeltaWithBlockState(line, false).delta;
}

export function countDiffBraceDeltaWithBlockState(
  line: string,
...
function applyBraceDelta(ch: string, skip: boolean, delta: number) {
  if (skip) {
    return delta;
  }
  if (ch === '{') {
