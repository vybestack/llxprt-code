 ...
 *
 *   bun install && bun scripts/verify-bun-workspace-links.ts
 *
...
interface WorkspaceLinkResult {
  category: 'linked' | 'missingPkgJson' | 'malformed' | 'missing' | 'notLinked';
  detail: string;
}

interface LinkFailures {
  failures: string[];
  workspaceCount: number;
}

interface GroupedResults {
  missingPkgJson: string[];
  missing: string[];
  notLinkedLocally: string[];
  malformed: string[];
}
 ...
 */
function summarizeLinkResults(
  results: WorkspaceLinkResult[],
  workspaceCount: number,
): LinkFailures {
...
// Only run as a CLI when invoked directly (e.g. `bun scripts/verify-bun-
// workspace-links.ts`), so importing `verifyBunWorkspaceLinks` for unit tests
