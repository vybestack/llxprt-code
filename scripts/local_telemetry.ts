...
interface ProcessState {
  jaeger: ChildProcess | null;
  collector: ChildProcess | null;
}

interface LogFdState {
  jaeger: number | null;
...
async function ensureTelemetryBinaries() {
  ...
  const jaegerPath = await ensureBinary(
    ...
    const message = e instanceof Error ? e.message : String(e);
    console.error(`[ERROR] getting jaeger: ${message}`);
    return null;
  });
...
async function main() {
  ...
  const processes: ProcessState = { jaeger: null, collector: null };
  const logFds: LogFdState = { jaeger: null, collector: null };
  ...
  registerCleanup(
    () =>
      [processes.jaeger, processes.collector].filter(
        (p): p is ChildProcess => p !== null,
      ),
    () =>
      [logFds.jaeger, logFds.collector].filter(
        (fd): fd is number => fd !== null,
      ),
...
function attachProcessErrorHandler(proc: ChildProcess | null): void {
  if (proc) {
