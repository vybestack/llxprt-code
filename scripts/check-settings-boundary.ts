 ...
 *
 * Usage:
 *   node scripts/check-settings-boundary.ts                          # run all checks
 *   node scripts/check-settings-boundary.ts --check source-imports  # run specific checks
 *   node scripts/check-settings-boundary.ts --check source-imports,metadata,lockfile
 *   node scripts/check-settings-boundary.ts --phase pre-p08         # report-only for P08+ checks
 *   node scripts/check-settings-boundary.ts --phase pre-p09         # report-only for P09+ checks
 */
 ...
 */
function check7_oldPaths(reportOnly: boolean) {
  const output = runRg(
 ...
 */
function lineExportsMovedSymbol(line: string) {
 ...
 */
function check10_coreReExports(reportOnly: boolean) {
 ...
 */
function check11_modelParamsSubpath(reportOnly: boolean) {
  const corePkgPath = join(ROOT, 'packages/core/package.json');
  if (!existsSync(corePkgPath)) {
 ...
 */
function check12_relativeSettingsImports(reportOnly: boolean) {
  const scanPaths = [
    'packages/agents/src',
    'packages/core/src',
    'packages/cli/test',
 ...
 */
function check13_relativeStorageImports(reportOnly: boolean) {
  const output1 = runRg(
    'from [\'"].*config/storage[\'"]',
    'packages/core/src',
    ['*.ts'],
 ...
 */
function check14_viMockPaths(reportOnly: boolean) {
  const output1 = runRg(
 ...
 */
function check15_dynamicImportPaths(reportOnly: boolean) {
  const output = runRg(
 ...
 */
function collectExportLinesForSymbol(content: string, sym: string) {
  const lines = content.split('\n');
  const result = [];
 ...
 */
function check18_coreBarrelShim(reportOnly: boolean) {
  const barrelPaths = [
    join(ROOT, 'packages/core/src/index.ts'),
    join(ROOT, 'packages/core/index.ts'),
  ];
 ...
 */
function parseMainArgs(args: string[]) {
 ...
 */
const CHECK_HANDLERS: Record<string, (reportOnly?: boolean) => boolean> = {
  ...
  'core-re-exports': (reportOnly?: boolean) =>
    check10_coreReExports(!!reportOnly),
  modelParams: (reportOnly?: boolean) =>
    check11_modelParamsSubpath(!!reportOnly),
  'modelParams-subpath': (reportOnly?: boolean) =>
    check11_modelParamsSubpath(!!reportOnly),
  'relative-settings-imports': (reportOnly?: boolean) =>
    check12_relativeSettingsImports(!!reportOnly),
  'relative-storage-imports': (reportOnly?: boolean) =>
    check13_relativeStorageImports(!!reportOnly),
  'vi-mock-paths': (reportOnly?: boolean) => check14_viMockPaths(!!reportOnly),
  'dynamic-import-paths': (reportOnly?: boolean) =>
  ...
  'core-barrel-shim': (reportOnly?: boolean) =>
    check18_coreBarrelShim(!!reportOnly),
 ...
 */
function runCheckByName(checkName: string, reportOnly: boolean) {
