#!/usr/bin/env node

/**
 * @license
 * Copyright 2025 Google LLC
 * SPDX-License-Identifier: Apache-2.0
 */

import path from 'node:path';
import fs from 'node:fs';
import net from 'node:net';
import os from 'node:os';
import { execSync, type ChildProcess } from 'node:child_process';
import { fileURLToPath } from 'node:url';
import crypto from 'node:crypto';
import { z } from 'zod';
import { isErrnoException, messageOf } from './utils/error-guards.ts';

const __filename = fileURLToPath(import.meta.url);
const __dirname = path.dirname(__filename);

interface GitHubRelease {
  tag_name: string;
  prerelease: boolean;
  assets: GitHubAsset[];
}

interface GitHubAsset {
  name: string;
  browser_download_url: string;
}

interface ReleaseAssetResult {
  release: GitHubRelease;
  asset: GitHubAsset;
}

type AssetNameCallback = (
  version: string,
  platform: string,
  arch: string,
  ext: string,
) => string;

type ProcessGetter = () => ChildProcess[];
type FileDescriptorGetter = () => number[];

const gitHubAssetSchema = z.object({
  name: z.string(),
  browser_download_url: z.string(),
});

const gitHubReleaseSchema = z.object({
  tag_name: z.string(),
  prerelease: z.boolean(),
  assets: z.array(gitHubAssetSchema),
});

const workspaceSettingsSchema = z
  .object({
    sandbox: z.union([z.boolean(), z.string()]).optional(),
    telemetry: z
      .object({
        enabled: z.boolean().optional(),
        otlpEndpoint: z.string().nullable().optional(),
        target: z.union([z.boolean(), z.string()]).optional(),
      })
      .passthrough()
      .optional(),
  })
  .passthrough();

const projectRoot = path.resolve(__dirname, '..');
const projectHash = crypto
  .createHash('sha256')
  .update(projectRoot)
  .digest('hex');

// Use the SINGLE shared path-resolution authority (issue #2606 finding 6):
// no duplicate algorithm. The shared resolver honors the same override
// precedence (LLXPRT_LOG_HOME -> LLXPRT_CONFIG_HOME -> platform log dir) and
// the same absolute-path validity rules as the central Storage contract.
// Dynamic import with fallback so an unrelated packages/storage issue does
// not break telemetry entirely.
let LOG_DIR: string;
try {
  const mod = await import('../packages/storage/src/config/path-resolver.ts');
  LOG_DIR = mod.resolveGlobalLogDir();
} catch (e: unknown) {
  const message = e instanceof Error ? e.message : String(e);
  console.warn(`telemetry_utils: falling back to platform log dir: ${message}`);
  LOG_DIR = path.join(os.homedir(), '.local', 'state', 'llxprt-code');
}
// Project-level .llxprt directory in the workspace
const WORKSPACE_LLXPRT_DIR = path.join(projectRoot, '.llxprt');

// Telemetry artifacts are stored in a hashed directory under the canonical
// log/state directory's tmp subtree.
export const OTEL_DIR = path.join(LOG_DIR, 'tmp', projectHash, 'otel');
export const BIN_DIR = path.join(OTEL_DIR, 'bin');

// Workspace settings remain in the project's .llxprt directory
export const WORKSPACE_SETTINGS_FILE = path.join(
  WORKSPACE_LLXPRT_DIR,
  'settings.json',
);

export function getJson(url: string): GitHubRelease | GitHubRelease[] {
  const tmpFile = path.join(
    os.tmpdir(),
    `llxprt-code-releases-${Date.now()}.json`,
  );
  try {
    execSync(
      `curl -sL -H "User-Agent: llxprt-code-dev-script" -o "${tmpFile}" "${url}"`,
      { stdio: 'pipe' },
    );
    const content = fs.readFileSync(tmpFile, 'utf-8');
    return z
      .union([gitHubReleaseSchema, z.array(gitHubReleaseSchema)])
      .parse(JSON.parse(content));
  } catch (e) {
    console.error(`Failed to fetch or parse JSON from ${url}`);
    throw e;
  } finally {
    if (fs.existsSync(tmpFile)) {
      fs.unlinkSync(tmpFile);
    }
  }
}

export function downloadFile(url: string, dest: string): string {
  try {
    execSync(`curl -fL -sS -o "${dest}" "${url}"`, {
      stdio: 'pipe',
    });
    return dest;
  } catch (e) {
    console.error(`Failed to download file from ${url}`);
    throw e;
  }
}

export function findFile(
  startPath: fs.PathLike,
  filter: (file: string) => boolean,
): string | null {
  const startPathStr = String(startPath);
  if (!fs.existsSync(startPathStr)) {
    return null;
  }
  const files = fs.readdirSync(startPathStr);
  for (const file of files) {
    const filename = path.join(startPathStr, file);
    const stat = fs.lstatSync(filename);
    if (stat.isDirectory()) {
      const result = findFile(filename, filter);
      if (result) return result;
    } else if (filter(file)) {
      return filename;
    }
  }
  return null;
}

export function fileExists(
  filePath: fs.PathLike | fs.PathOrFileDescriptor,
): boolean {
  if (typeof filePath === 'number') {
    try {
      fs.fstatSync(filePath);
      return true;
    } catch {
      return false;
    }
  }
  return fs.existsSync(filePath);
}

export function readJsonFile(filePath: fs.PathOrFileDescriptor): unknown {
  if (!fileExists(filePath)) {
    return {};
  }
  const content = fs.readFileSync(filePath, 'utf-8');
  try {
    return JSON.parse(content);
  } catch (e: unknown) {
    const message = e instanceof Error ? e.message : String(e);
    console.error(`Error parsing JSON from ${filePath}: ${message}`);
    return {};
  }
}

export function writeJsonFile(
  filePath: fs.PathOrFileDescriptor,
  data: unknown,
): void {
  fs.writeFileSync(filePath, JSON.stringify(data, null, 2));
}

export function moveBinary(
  source: fs.PathLike,
  destination: fs.PathLike,
): void {
  try {
    fs.renameSync(source, destination);
  } catch (error: unknown) {
    if (!isErrnoException(error, 'EXDEV')) {
      throw error;
    }
    // Handle a cross-device error: copy-to-temp-then-rename.
    const destDir = path.dirname(String(destination));
    const destFile = path.basename(String(destination));
    const tempDest = path.join(destDir, `${destFile}.tmp`);

    try {
      fs.copyFileSync(source, tempDest);
      fs.renameSync(tempDest, destination);
    } catch (moveError) {
      // If copy or rename fails, clean up the intermediate temp file.
      if (fs.existsSync(tempDest)) {
        fs.unlinkSync(tempDest);
      }
      throw moveError;
    }
    fs.unlinkSync(source);
  }
}

export function waitForPort(port: number, timeout = 10000): Promise<void> {
  return new Promise<void>((resolve, reject) => {
    const startTime = Date.now();
    const tryConnect = () => {
      const socket = new net.Socket();
      socket.once('connect', () => {
        socket.end();
        resolve();
      });
      socket.once('error', (_) => {
        if (Date.now() - startTime > timeout) {
          reject(new Error(`Timeout waiting for port ${port} to open.`));
        } else {
          setTimeout(tryConnect, 500);
        }
      });
      socket.connect(port, 'localhost');
    };
    tryConnect();
  });
}

function findReleaseAsset(
  isJaeger: boolean,
  repo: string,
  platform: string,
  arch: string,
  ext: string,
  assetNameCallback: AssetNameCallback,
): ReleaseAssetResult {
  if (isJaeger) {
    return findJaegerReleaseAsset(repo, platform, arch);
  }
  return findLatestReleaseAsset(repo, platform, arch, ext, assetNameCallback);
}

function findJaegerReleaseAsset(
  repo: string,
  platform: string,
  arch: string,
): ReleaseAssetResult {
  console.log(`Finding latest Jaeger v2+ asset...`);
  const releases = getJson(`https://api.github.com/repos/${repo}/releases`);
  const releaseList = Array.isArray(releases) ? releases : [releases];
  const sortedReleases = releaseList
    .filter((r) => !r.prerelease && r.tag_name.startsWith('v'))
    .sort(compareSemverTags);

  for (const r of sortedReleases) {
    const expectedSuffix =
      platform === 'windows'
        ? `-${platform}-${arch}.zip`
        : `-${platform}-${arch}.tar.gz`;
    const foundAsset = r.assets.find(
      (a) => a.name.startsWith('jaeger-2.') && a.name.endsWith(expectedSuffix),
    );

    if (foundAsset) {
      console.log(
        `Found ${foundAsset.name} in release ${r.tag_name}, downloading...`,
      );
      return { release: r, asset: foundAsset };
    }
  }
  throw new Error(
    `Could not find a suitable Jaeger v2 asset for platform ${platform}/${arch}.`,
  );
}

function findLatestReleaseAsset(
  repo: string,
  platform: string,
  arch: string,
  ext: string,
  assetNameCallback: AssetNameCallback,
): ReleaseAssetResult {
  const release = getJson(
    `https://api.github.com/repos/${repo}/releases/latest`,
  );
  const releaseData = Array.isArray(release) ? release[0] : release;
  const version = releaseData.tag_name.startsWith('v')
    ? releaseData.tag_name.substring(1)
    : releaseData.tag_name;
  const assetName = assetNameCallback(version, platform, arch, ext);
  const asset = releaseData.assets.find((a) => a.name === assetName);
  if (!asset) {
    throw new Error(
      `Could not find a suitable asset for ${repo} (version ${version}) on platform ${platform}/${arch}. Searched for: ${assetName}`,
    );
  }
  return { release: releaseData, asset };
}

function compareSemverTags(a: GitHubRelease, b: GitHubRelease): number {
  const aVersion = a.tag_name.substring(1).split('.').map(Number);
  const bVersion = b.tag_name.substring(1).split('.').map(Number);
  for (let i = 0; i < Math.max(aVersion.length, bVersion.length); i++) {
    if ((aVersion[i] || 0) > (bVersion[i] || 0)) return -1;
    if ((aVersion[i] || 0) < (bVersion[i] || 0)) return 1;
  }
  return 0;
}

function extractArchive(
  archivePath: string,
  tmpDir: string,
  assetName: string,
) {
  const actualExt = assetName.endsWith('.zip') ? 'zip' : 'tar.gz';
  if (actualExt === 'zip') {
    execSync(`unzip -o "${archivePath}" -d "${tmpDir}"`, { stdio: 'pipe' });
  } else {
    execSync(`tar -xzf "${archivePath}" -C "${tmpDir}"`, { stdio: 'pipe' });
  }
}

export async function ensureBinary(
  executableName: string,
  repo: string,
  assetNameCallback: AssetNameCallback,
  binaryNameInArchive: string,
  isJaeger = false,
): Promise<string | null> {
  const executablePath = path.join(BIN_DIR, executableName);
  if (fileExists(executablePath)) {
    console.log(`[OK] ${executableName} already exists at ${executablePath}`);
    return executablePath;
  }

  console.log(`${executableName} not found. Downloading from ${repo}...`);

  const platform = process.platform === 'win32' ? 'windows' : process.platform;
  const arch = process.arch === 'x64' ? 'amd64' : process.arch;
  const ext = platform === 'windows' ? 'zip' : 'tar.gz';

  if (isJaeger && platform === 'windows' && arch === 'arm64') {
    console.warn(
      `WARNING: Jaeger does not have a release for Windows on ARM64. Skipping.`,
    );
    return null;
  }

  const { asset } = findReleaseAsset(
    isJaeger,
    repo,
    platform,
    arch,
    ext,
    assetNameCallback,
  );

  const downloadUrl = asset.browser_download_url;
  const tmpDir = fs.mkdtempSync(
    path.join(os.tmpdir(), 'llxprt-code-telemetry-'),
  );
  const archivePath = path.join(tmpDir, asset.name);

  try {
    console.log(`Downloading ${asset.name}...`);
    downloadFile(downloadUrl, archivePath);
    console.log(`Extracting ${asset.name}...`);

    extractArchive(archivePath, tmpDir, asset.name);

    const nameToFind = binaryNameInArchive || executableName;
    const foundBinaryPath = findFile(tmpDir, (file: string) => {
      if (platform === 'windows') {
        return file === `${nameToFind}.exe`;
      }
      return file === nameToFind;
    });

    if (!foundBinaryPath) {
      throw new Error(
        `Could not find binary "${nameToFind}" in extracted archive at ${tmpDir}. Contents: ${fs.readdirSync(tmpDir).join(', ')}`,
      );
    }

    moveBinary(foundBinaryPath, executablePath);

    if (platform !== 'windows') {
      fs.chmodSync(executablePath, '755');
    }

    console.log(`[OK] ${executableName} installed at ${executablePath}`);
    return executablePath;
  } finally {
    fs.rmSync(tmpDir, { recursive: true, force: true });
    if (fs.existsSync(archivePath)) {
      fs.unlinkSync(archivePath);
    }
  }
}

export function manageTelemetrySettings(
  enable: boolean,
  oTelEndpoint: string | null = 'http://localhost:4317',
  target: boolean | string = 'local',
  originalSandboxSettingToRestore: boolean | string | undefined = undefined,
): boolean | string | undefined {
  const workspaceSettings = workspaceSettingsSchema.parse(
    readJsonFile(WORKSPACE_SETTINGS_FILE),
  );
  const currentSandboxSetting = workspaceSettings.sandbox;
  let settingsModified = false;

  if (typeof workspaceSettings.telemetry !== 'object') {
    workspaceSettings.telemetry = {};
  }

  if (enable) {
    if (workspaceSettings.telemetry.enabled !== true) {
      workspaceSettings.telemetry.enabled = true;
      settingsModified = true;
      console.log('[CONFIG] Enabled telemetry in workspace settings.');
    }
    if (workspaceSettings.sandbox !== false) {
      workspaceSettings.sandbox = false;
      settingsModified = true;
      console.log('[OK] Disabled sandbox mode for telemetry.');
    }
    if (workspaceSettings.telemetry.otlpEndpoint !== oTelEndpoint) {
      workspaceSettings.telemetry.otlpEndpoint = oTelEndpoint;
      settingsModified = true;
      console.log(`[CONFIG] Set telemetry OTLP endpoint to ${oTelEndpoint}.`);
    }
    if (workspaceSettings.telemetry.target !== target) {
      workspaceSettings.telemetry.target = target;
      settingsModified = true;
      console.log(`[CONFIG] Set telemetry target to ${target}.`);
    }
  } else {
    if (workspaceSettings.telemetry.enabled === true) {
      delete workspaceSettings.telemetry.enabled;
      settingsModified = true;
      console.log('[CONFIG] Disabled telemetry in workspace settings.');
    }
    if (workspaceSettings.telemetry.otlpEndpoint) {
      delete workspaceSettings.telemetry.otlpEndpoint;
      settingsModified = true;
      console.log('[CONFIG] Cleared telemetry OTLP endpoint.');
    }
    if (workspaceSettings.telemetry.target) {
      delete workspaceSettings.telemetry.target;
      settingsModified = true;
      console.log('[CONFIG] Cleared telemetry target.');
    }
    if (Object.keys(workspaceSettings.telemetry).length === 0) {
      delete workspaceSettings.telemetry;
    }

    if (
      originalSandboxSettingToRestore !== undefined &&
      workspaceSettings.sandbox !== originalSandboxSettingToRestore
    ) {
      workspaceSettings.sandbox = originalSandboxSettingToRestore;
      settingsModified = true;
      console.log('[OK] Restored original sandbox setting.');
    }
  }

  if (settingsModified) {
    writeJsonFile(WORKSPACE_SETTINGS_FILE, workspaceSettings);
    console.log('[OK] Workspace settings updated.');
  } else {
    console.log(
      enable
        ? '[OK] Workspace settings are already configured for telemetry.'
        : '[OK] Workspace settings already reflect telemetry disabled.',
    );
  }
  return currentSandboxSetting;
}

export function registerCleanup(
  getProcesses: ProcessGetter | null,
  getLogFileDescriptors: FileDescriptorGetter | null,
  originalSandboxSetting: boolean | string | undefined,
): void {
  let cleanedUp = false;
  const cleanup = (): void => {
    if (cleanedUp) return;
    cleanedUp = true;

    console.log('\nShutting down...');

    manageTelemetrySettings(false, null, originalSandboxSetting);

    const processes = getProcesses ? getProcesses() : [];
    processes.forEach((proc) => {
      if (proc && proc.pid) {
        const name = path.basename(proc.spawnfile);
        try {
          console.log(`Stopping ${name} (PID: ${proc.pid})...`);
          process.kill(proc.pid, 'SIGTERM');
          console.log(`[OK] ${name} stopped.`);
        } catch (e: unknown) {
          if (!isErrnoException(e, 'ESRCH')) {
            console.error(`Error stopping ${name}: ${messageOf(e)}`);
          }
        }
      }
    });

    const logFileDescriptors = getLogFileDescriptors
      ? getLogFileDescriptors()
      : [];
    logFileDescriptors.forEach((fd: number | null) => {
      if (fd) {
        try {
          fs.closeSync(fd);
        } catch (_) {
          /* no-op */
        }
      }
    });
  };

  process.on('exit', cleanup);
  process.on('SIGINT', () => process.exit(0));
  process.on('SIGTERM', () => process.exit(0));
  process.on('uncaughtException', (err) => {
    console.error('Uncaught Exception:', err);
    cleanup();
    process.exit(1);
  });
}
