...
import { validateTelemetryRecord } from './ocr-telemetry-schema.ts';
...
interface TelemetryIoOptions {
  cwd?: string;
  env?: NodeJS.ProcessEnv;
}

function readJsonArtifact(path: PathOrFileDescriptor, label: string) {
  try {
...
export function loadTelemetryInput(options: TelemetryIoOptions = {}) {
...
function copyWithRedactedStrings(
  value: JsonValue,
  secrets: string[],
): JsonValue {
...
export function redactTelemetryFile(path: string, secrets: unknown[]) {
...
export function writeTelemetryArtifacts(
  record: unknown,
  markdown: string | Uint8Array,
  options: TelemetryIoOptions = {},
) {
  assertValid(record);
  const cwd = options.cwd ?? process.cwd();
  const env = options.env ?? process.env;
